...
Controlled re-download and validation of real BTC beacon + tick data
"""
...
def download_sep_w2():
    """Download and validate September Week 2 data"""
    print("🗓️ WEEK 2 — SEPTEMBER 8 → SEPTEMBER 14 (2025)")
    print("=" * 80)
    print("TASK: Controlled re-download and validation of real BTC beacon + tick data")
    ...
    print("📊 **Downloading real BTC tick data...**")
    ...
    download_df.to_csv(f'{output_dir}/ingest_sep_w2_summary.csv', index=False)
    ...
    with open(f'{output_dir}/ingest_sep_w2_validity.log', 'w') as f:
        f.write("SEPTEMBER WEEK 2 DOWNLOAD AND VALIDATION LOG\n")
        f.write("=" * 80 + "\n")
        ...
        f.write("-" * 40 + "\n")
        f.write(f"Total attempted: {len(download_results)}\n")
        f.write(f"Successful: {sum(1 for r in download_results if r['status'] == 'SUCCESS')}\n")
        f.write(f"Failed: {sum(1 for r in download_results if r['status'] != 'SUCCESS')}\n\n")
    ...
    if all_ticks and coverage_pct >= 90.0:
        print(f"\n✅ **SEPTEMBER WEEK 2 DOWNLOAD COMPLETE**")
        print(f"✅ Coverage {coverage_pct:.1f}% meets 90% threshold")
    else:
        print(f"\n❌ **SEPTEMBER WEEK 2 DOWNLOAD INCOMPLETE**")

#!/usr/bin/env python3
"""
STAGE 3 — Leadership & Hazard Metrics for Week -2 Day 4 (2025-08-25)

Using only the validated beacon and effects caches from Stage 1 and Stage 2.
"""
...
def load_beacon_cache(date_str):
    ...
    for venue in ['BINANCE', 'COINBASE', 'BYBITSPOT', 'BITGET']:
        ...
        if not os.path.exists(cache_file):
            print(f"❌ HALT: Missing beacon cache for {venue} {date_str}")
            raise ValueError(f"Missing beacon cache: {cache_file}")
...
def main():
    ...
    try:
        ...
        # Filter to successfully processed events from Stage 2 (66 events)
        ...
        for _, event in successful_events.iterrows():
            ...
            leader, lead_time_ms, lead_return, persistence_s = find_leader_venue(
        ...
        if memory_usage <= 750:
            ...
            print(f"• No cached files altered, merged, or appended")
            print(f"• No variables created, renamed, or deleted")

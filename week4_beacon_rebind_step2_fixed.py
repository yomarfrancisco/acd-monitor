#!/usr/bin/env python3
"""
Week-4 Beacon Rebind → Smoke Test → Full Step-2 (Fixed)
...
def main():
    print("🔍 Week-4 Beacon Rebind → Smoke Test → Full Step-2 (Fixed)")
    print("=" * 70)
    ...
    for _, event in smoke_events.iterrows():
        ...
        # Use pyarrow.dataset filter - FIXED syntax
        parquet_file = f"data_v6/views/{venue}/{date_str}/ticks_canonical.parquet"
        
        try:
            # Read with pyarrow filter - use proper syntax
            ...
            filtered_data = dataset.to_table(
                filter=(ds.field('ts') >= t0_ns) & (ds.field('ts') <= t1_ns),
            ...
            if n_rows > 0:
                ...
                t0_wide_ns = int(t0_wide.timestamp() * 1e9)
                t1_wide_ns = int(t1_wide.timestamp() * 1e9)
                
                filtered_data_wide = dataset.to_table(
                    filter=(ds.field('ts') >= t0_wide_ns) & (ds.field('ts') <= t1_wide_ns),
                    columns=['ts', 'price', 'size']
                )

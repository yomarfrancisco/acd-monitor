#!/usr/bin/env python3
"""
Phase 42A-v5: Selective Stress Detection + Persistence Calibration
Calibrate Sticky HMM with compound confirmation and longer persistence
"""
...
def load_dataset_and_verify():
    """Load dataset and verify SHA-256 with 0% NaN"""
    print("🔍 **Phase 42A-v5: Selective Stress Detection + Persistence Calibration**")
    print("=" * 60)
...
def compute_rri_series_v5(df):
    """Compute RRI series with selective methodology"""
    print(f"\n📊 **Computing RRI Series v5**")
    print("=" * 60)
    ...
    for hour, group in df.groupby('hour'):
        ...
        # Also store the raw features for compound stress condition
...
def rebuild_july_baseline_v5(rri_df):
    """Rebuild July baseline with selective composition (≈20% stressed)"""
    print(f"\n📊 **Rebuilding July Baseline v5**")
    print("=" * 60)
    ...
    # Define compound stress condition (AND logic, more selective)
    july_rri['stress_flag'] = (
        (july_rri['rri_z_avg'] > 1.5) &
        (july_rri['entropy'] > 1.5) &
        (np.abs(july_rri['ofi']) > 1.2)
    )
    
    stress_share = july_rri['stress_flag'].mean()
    print(f"📊 Initial stress share: {stress_share:.3f} ({stress_share*100:.1f}%)")
    
    # If baseline < 200h after filtering, extend to Aug 2
    if len(july_rri) < 200:
        print("📊 Extending baseline to August 2...")
        aug_start = pd.Timestamp('2025-08-01 00:00:00', tz='UTC')
        aug_end = pd.Timestamp('2025-08-02 23:00:00', tz='UTC')
        
        aug_rri = rri_df[(rri_df['timestamp'] >= aug_start) & (rri_df['timestamp'] <= aug_end)].copy()
        
        if not aug_rri.empty:
            # Apply same stress condition to August data
            aug_rri['stress_flag'] = (
                (aug_rri['rri_z_avg'] > 1.5) &
                (aug_rri['entropy'] > 1.5) &
                (np.abs(aug_rri['ofi']) > 1.2)
            )
            
            # Merge into July baseline
            july_rri = pd.concat([july_rri, aug_rri], ignore_index=True)
            july_rri = july_rri.sort_values('timestamp').reset_index(drop=True)
            
            print(f"📊 Extended baseline: {len(july_rri)} hours")
    
    # If stress share < 15%, augment with selective August data
    if stress_share < 0.15:
        print("📊 Augmenting July baseline with selective August data...")
        ...
        if not aug_rri.empty:
            ...
            # Add some stressed hours to July to reach ≈20% target
            target_stress_share = 0.20
            n_needed = int(target_stress_share * len(july_rri)) - int(stress_share * len(july_rri))
    ...
    print(f"   - NaN percentage: {july_rri.isnull().sum().sum() / (len(july_rri) * len(july_rri.columns)) * 100:.1f}%")
    
    if july_rri['rri_z_avg'].var() == 0:
        print("❌ HALT: Zero variance in July baseline")
        return None
    
    if july_rri.isnull().sum().sum() / (len(july_rri) * len(july_rri.columns)) > 0.01:
        print("❌ HALT: >1% NaN values in July baseline")
        return None
    
    if len(july_rri) < 200:
        print("❌ HALT: <200 hours coverage in July baseline")
        return None
    
    return july_rri

def define_compound_stress_condition(rri_df):
    """Define compound stress condition with AND logic and 6h rolling window"""
    print(f"\n📊 **Defining Compound Stress Condition**")
    print("=" * 60)
    
    # Apply compound stress condition (AND logic, more selective)
    rri_df['stress_flag'] = (
        (rri_df['rri_z_avg'] > 1.5) &
        (rri_df['entropy'] > 1.5) &
        (np.abs(rri_df['ofi']) > 1.2)
    )
    
    # Apply 6-hour rolling window: mark "stressed" only if ≥6 consecutive hours meet all conditions
    rri_df['stress_rolling'] = False
    
    for i in range(len(rri_df) - 5):
        window = rri_df['stress_flag'].iloc[i:i+6]
        if window.sum() >= 6:  # All 6 hours must be stressed
            rri_df.loc[rri_df.index[i:i+6], 'stress_rolling'] = True
    
    # Define regime labels
    rri_df['regime'] = 'transitional'  # Default
    
    # Calm: all conditions < 0.5 for ≥6 consecutive hours
    calm_mask = (
        (rri_df['rri_z_avg'] < 0.5) &
        (rri_df['entropy'] < 0.5) &
        (np.abs(rri_df['ofi']) < 0.5)
    )
    
    # Apply 6-hour rolling window for calm as well
    rri_df['calm_rolling'] = False
    for i in range(len(rri_df) - 5):
        window = calm_mask.iloc[i:i+6]
        if window.sum() >= 6:  # All 6 hours must be calm
            rri_df.loc[rri_df.index[i:i+6], 'calm_rolling'] = True
    
    rri_df.loc[rri_df['calm_rolling'], 'regime'] = 'calm'
    
    # Stressed: meets rolling condition
    rri_df.loc[rri_df['stress_rolling'], 'regime'] = 'stressed'
    
    # Report regime distribution
    regime_counts = rri_df['regime'].value_counts()
    print(f"📊 Regime distribution:")
    for regime, count in regime_counts.items():
        percentage = (count / len(rri_df)) * 100
        print(f"   {regime}: {count} hours ({percentage:.1f}%)")
    
    return rri_df

def preprocess_for_hmm_v5(rri_df, july_baseline):
    """Preprocess RRI data for HMM fitting with selective features"""
    print(f"\n📊 **Preprocessing for HMM v5**")
    print("=" * 60)
    
    # Use selective features: [rri_z_avg, entropy, ofi]
    features = ['rri_z_avg', 'entropy', 'ofi']
...
class RobustStickyHMMv5:
    """Robust sticky HMM v5 with selective configuration"""
...
def run_sticky_hmm_v5(rri_processed):
    """Run sticky HMM analysis v5"""
    print(f"\n📊 **Running Sticky HMM Analysis v5**")
    print("=" * 60)
    
    # Prepare features for modeling: [rri_z_avg, entropy, ofi]
    features = ['rri_z_avg', 'entropy', 'ofi']
...
def analyze_regime_statistics_v5(model, states, rri_processed):
    """Analyze regime statistics and compute Oct/Jul ratios"""
    print(f"\n📊 **Analyzing Regime Statistics v5**")
    print("=" * 60)
    
    # Add states to dataframe
    rri_processed['state'] = states
    
    # Label states by mean RRI and entropy cross-check
    state_means = []
    for k in range(model.n_states):
        state_mask = states == k
        if state_mask.sum() > 0:
            mean_rri = rri_processed.loc[state_mask, 'rri_z_avg'].mean()
            mean_entropy = rri_processed.loc[state_mask, 'entropy'].mean()
            # Use combined score for labeling
            combined_score = mean_rri + mean_entropy
            state_means.append((k, combined_score, mean_rri, mean_entropy))
    
    state_means.sort(key=lambda x: x[1])  # Sort by combined score
    state_labels = ['calm', 'transitional', 'stressed']
    
    # Create state mapping
    state_mapping = {old_k: new_k for new_k, (old_k, _, _, _) in enumerate(state_means)}
    relabeled_states = np.array([state_mapping[s] for s in states])
    rri_processed['state_relabeled'] = relabeled_states
    
    # Compute state statistics
    state_summary = []
    for new_k, (old_k, combined_score, mean_rri, mean_entropy) in enumerate(state_means):
        ...
        state_summary.append({
            ...
            'mean_rri_z_avg': mean_features[0],
            'mean_entropy': mean_features[1],
            'mean_ofi': mean_features[2],
...
def generate_ascii_timeline_v5(rri_processed, state_labels):
    """Generate ASCII timeline plot v5"""
    print(f"\n📊 **Generating ASCII Timeline v5**")
    print("=" * 60)
...
def compute_null_controls_v5(model, X, rri_processed):
    """Compute null controls for significance testing v5"""
    print(f"\n📊 **Computing Null Controls v5**")
    print("=" * 60)
...
def evaluate_decision_criteria_v5(state_summary, r_real, r_circshift, r_blockperm):
    """Evaluate decision criteria v5"""
    print(f"\n📊 **Evaluating Decision Criteria v5**")
    print("=" * 60)
...
def save_results_v5(state_summary, timeline, decision, reason, r_real, r_circshift, r_blockperm):
    """Save results to files v5"""
    print(f"\n📊 **Saving Results v5**")
    print("=" * 60)
    ...
    json_path = 'results/phase_42A_v5_hmm.json'
    with open(json_path, 'w') as f:
        json.dump(results_json, f, indent=2)
    
    # Save timeline
    timeline_path = 'results/phase_42A_v5_hmm_timeline.txt'
    with open(timeline_path, 'w') as f:
        f.write("Phase 42A-v5 HMM Timeline\n")
        f.write("=" * 30 + "\n")
...
def main():
    ...
    try:
        ...
        df = load_dataset_and_verify()
        if df is None:
            return
        
        # Compute RRI series v5
        rri_df, baseline_stats = compute_rri_series_v5(df)
        if rri_df is None:
            return
        
        # Rebuild July baseline v5
        july_baseline = rebuild_july_baseline_v5(rri_df)
        if july_baseline is None:
            return
        
        # Define compound stress condition
        rri_df = define_compound_stress_condition(rri_df)
        
        # Preprocess for HMM v5
        rri_processed, july_stats = preprocess_for_hmm_v5(rri_df, july_baseline)
        if rri_processed is None:
            return
        
        # Run sticky HMM analysis v5
        model, states, X = run_sticky_hmm_v5(rri_processed)
        
        # Analyze regime statistics v5
        state_summary, r_real, state_labels = analyze_regime_statistics_v5(model, states, rri_processed)
        
        # Generate ASCII timeline v5
        timeline = generate_ascii_timeline_v5(rri_processed, state_labels)
        
        # Compute null controls v5
        r_circshift, r_blockperm = compute_null_controls_v5(model, X, rri_processed)
        
        # Evaluate decision criteria v5
        decision_passed, reason = evaluate_decision_criteria_v5(state_summary, r_real, r_circshift, r_blockperm)
        
        # Save results v5
        save_results_v5(state_summary, timeline, "PASS" if decision_passed else "REJECT", reason, r_real, r_circshift, r_blockperm)
        ...
        print(f"{'State':<5} {'Label':<12} {'Mean(Zavg,Ent,OFI)':<25} {'Duration(h)':<12} {'p_self':<8} {'Share%':<8}")
        print("-" * 70)
        
        for state in state_summary:
            mean_str = f"({state['mean_rri_z_avg']:.2f},{state['mean_entropy']:.2f},{state['mean_ofi']:.2f})"

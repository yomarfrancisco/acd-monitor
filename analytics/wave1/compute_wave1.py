...
def compute_rolling_volatility(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    for venue in df['venue'].unique():
        ...
        if len(venue_data) > 60:  # Need sufficient data for 60s rolling
            ...
            venue_data['rolling_vol_60s'] = venue_data['returns_1s'].rolling(window=60, min_periods=30).std()
            ...
            if 'best_bid' in venue_data.columns and 'best_ask' in venue_data.columns:
                ...
                venue_data['rolling_spread_60s'] = venue_data['spread_pct'].rolling(window=60, min_periods=30).mean()
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Compute Wave-1 variables for competitive behavior analysis')

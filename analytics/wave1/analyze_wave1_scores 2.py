...
def check_rolling_volatility_convergence(df: pd.DataFrame) -> bool:
    ...
    if vol_corr_cols and spread_std_cols:
        ...
        return (
            max_vol_corr > ROLLING_VOL_CORR_THRESHOLD and median_spread_std <= SPREAD_STD_THRESHOLD
        )
...
"""
    ...
    for scorecard in scorecards:
        ...
        print(
            f"{symbol.upper()}: VR {', '.join(vr_summary)} | AR1 max={max(ar1_summary) if ar1_summary else 'N/A'} | xcorr {', '.join(xcorr_summary)} → {rating}"
        )

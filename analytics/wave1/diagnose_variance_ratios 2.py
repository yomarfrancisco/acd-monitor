...
def run_control_test() -> Dict[str, Any]:
    ...
    try:
        ...
        control_results["results"] = {
            ...
            "vr_difference": (
                abs(raw_vr - canonical_vr)
                if not np.isnan(raw_vr) and not np.isnan(canonical_vr)
                else np.nan
            ),
            "raw_obs": len(raw_df),
            "canonical_obs": len(canonical_venue_df),
            "structural_artifact_suspected": (
                abs(raw_vr - canonical_vr) > 0.1
                if not np.isnan(raw_vr) and not np.isnan(canonical_vr)
                else False
            ),
        }
        ...
        control_results["results"] = {"status": "FAILED", "error": str(e)}
...
def main():
    ...
    s3.put_object(Bucket=S3_BUCKET, Key=historical_key, Body=historical_df.to_csv(index=False))
    ...
    s3.put_object(Bucket=S3_BUCKET, Key=control_key, Body=json.dumps(control_results, indent=2))
    print(f"  ✅ {control_key}")
    ...
    if len(historical_df) > 0:
        ...
        if len(recent_data) > 0:
            ...
            for _, row in recent_data.iterrows():
                print(
                    f"  {row['symbol']}: overall={row['overall_vr']:.3f}, venue_mean={row['venue_mean_vr']:.3f}"
                )

    ...
    try:
        ...
        for _, wave1_row in wave1_df.iterrows():
            ...
            if len(venue_canonical) > 10:
                ...
                venue_comparisons[venue] = {
                    ...
                    'difference': float(abs(wave1_vr - canonical_vr)) if not np.isnan(canonical_vr) else None,
                    'canonical_obs': int(len(venue_canonical)),
                    'significant_difference': bool(abs(wave1_vr - canonical_vr) > 0.1) if not np.isnan(canonical_vr) else False
                }
                
                print(f"  📊 {venue}: Wave-1={wave1_vr:.3f}, Canonical={canonical_vr:.3f}, Diff={abs(wave1_vr - canonical_vr):.3f}")
        
        # Calculate summary statistics
        differences = [v['difference'] for v in venue_comparisons.values() if v['difference'] is not None]
        ...
        comparison_results['results'] = {
            ...
            'summary': {
                ...
                'significant_differences': sum(1 for v in venue_comparisons.values() if v['significant_difference']),
...
def main():
    ...
    s3.put_object(
        Bucket=S3_BUCKET,
        Key=comparison_key,
        Body=json.dumps(comparison_results, indent=2)
    )
...
"""
    ...
    if len(venue_df) > 0:
        ...
        for _, row in venue_df.iterrows():
            print(f"  {row['symbol']} {row['venue']}: {row['variance_ratio']:.3f} (obs: {row['n_obs']})")

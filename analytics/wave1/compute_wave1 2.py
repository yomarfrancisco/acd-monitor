...
def compute_pca_analysis(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    if len(common_ts) < MIN_COMMON:
        ...
        return create_pca_placeholder(symbol, venues, len(
            common_ts), "insufficient_common_timestamps")
    ...
    for i, venue in enumerate(venues):
        if i < len(venues):
            results.append({
                ...
                'venue': venue,
                'n_obs': len(common_ts),
                'explained_variance_ratio': pca.explained_variance_ratio_[i] if i < len(pca.explained_variance_ratio_) else 0,
...
def compute_rolling_volatility(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    for venue in df['venue'].unique():
        ...
        if len(venue_data) > 60:  # Need sufficient data for 60s rolling
            ...
            venue_data['rolling_vol_60s'] = venue_data['returns_1s'].rolling(
                window=60, min_periods=30).std()
            ...
            if 'best_bid' in venue_data.columns and 'best_ask' in venue_data.columns:
                ...
                venue_data['rolling_spread_60s'] = venue_data['spread_pct'].rolling(
            ...
            if len(df) > 0:
                ...
                if result_type == 'pca':
                    if 'status' in df.columns and df['status'].iloc[0] == 'skipped':
                        pca_info = {
                            'status': 'skipped', 'n_common': int(
                                df['n_common'].iloc[0]) if 'n_common' in df.columns else 0, 'reason': str(
                                df['reason'].iloc[0]) if 'reason' in df.columns else 'unknown'}
                    else:
                        pca_info = {
                            'status': 'ok', 'n_common': int(
                                df['n_obs'].iloc[0]) if 'n_obs' in df.columns else 0, 'n_components': int(
                                len(
                                    df['explained_variance_ratio'].dropna())) if 'explained_variance_ratio' in df.columns else 0, 'explained_var_ratio': [
                                float(x) for x in df['explained_variance_ratio'].dropna().tolist()] if 'explained_variance_ratio' in df.columns else []}

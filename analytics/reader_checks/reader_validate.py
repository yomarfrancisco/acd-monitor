...
def validate_rowcounts(s3_client, date_ymd: str) -> tuple[bool, dict]:
    ...
    btc_match = all(btc_counts.get(venue, 0) == expected['btc'][venue] for venue in expected['btc'] if venue != 'total')
    eth_match = all(eth_counts.get(venue, 0) == expected['eth'][venue] for venue in expected['eth'] if venue != 'total')
...
def validate_ranges(btc_data: list, eth_data: list) -> tuple[bool, dict]:
    ...
    print(f"  BTC range: ${btc_min:,.2f} - ${btc_max:,.2f} (min OK: {btc_min_ok}, max OK: {btc_max_ok})")
    print(f"  ETH range: ${eth_min:,.2f} - ${eth_max:,.2f} (min OK: {eth_min_ok}, max OK: {eth_max_ok})")
...
def validate_uniqueness(btc_data: list, eth_data: list) -> tuple[bool, dict]:
    ...
    if len(btc_combined) > 0:
        btc_combined['uniqueness_key'] = btc_combined['symbol'] + '|' + btc_combined['venue'] + '|' + btc_combined['ts_exchange_ms'].astype(str)
    ...
    if len(eth_combined) > 0:
        eth_combined['uniqueness_key'] = eth_combined['symbol'] + '|' + eth_combined['venue'] + '|' + eth_combined['ts_exchange_ms'].astype(str)

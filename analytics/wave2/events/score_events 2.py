...
def score_event_suspicion(events_df: pd.DataFrame) -> pd.DataFrame:
    ...
    successful_df["suspicion_score"] = successful_df["invariance_flag"].astype(int) + successful_df[
        "overreaction_flag"
    ].astype(int)
    ...
    for col in ["invariance_flag", "overreaction_flag", "suspicion_score"]:
        if col in successful_df.columns:
            result_df[col] = (
                result_df["status"]
                .map(successful_df.set_index(["symbol", "venue", "event_type"])[col].to_dict())
...
def create_scorecard(events_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    suspicion_flags = {
        "invariance_count": (
            int(successful_df["invariance_flag"].sum())
            if "invariance_flag" in successful_df.columns
            else 0
        ),
        "overreaction_count": (
            int(successful_df["overreaction_flag"].sum())
            if "overreaction_flag" in successful_df.columns
            else 0
        ),
        "high_suspicion_count": (
            int((successful_df["suspicion_score"] >= 2).sum())
    ...
    for venue in successful_df["venue"].unique():
        ...
        venue_summary[venue] = {
            ...
            "invariance_rate": (
                float(venue_data["invariance_flag"].mean())
                if "invariance_flag" in venue_data.columns
                else 0.0
            ),
            "overreaction_rate": (
                float(venue_data["overreaction_flag"].mean())
                if "overreaction_flag" in venue_data.columns
                else 0.0
            ),
        }
    ...
    for event_type in successful_df["event_type"].unique():
        ...
        event_type_summary[event_type] = {
            ...
            "invariance_rate": (
                float(event_data["invariance_flag"].mean())
                if "invariance_flag" in event_data.columns
                else 0.0
            ),
            "overreaction_rate": (
                float(event_data["overreaction_flag"].mean())
                if "overreaction_flag" in event_data.columns
                else 0.0
            ),
        }
...
"""
    ...
    # Save README
    s3.put_object(Bucket=S3_BUCKET, Key=key, Body=readme_content)

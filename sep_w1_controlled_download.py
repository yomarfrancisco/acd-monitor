...
def download_sep_w1():
    """Download and validate September Week 1 data"""
    ...
    download_df.to_csv(f'{output_dir}/ingest_sep_w1_summary.csv', index=False)
    validation_df.to_csv(f'{output_dir}/ingest_sep_w1_validity.csv', index=False)
    ...
    with open(f'{output_dir}/ingest_sep_w1_validity.log', 'w') as f:
        f.write("SEPTEMBER WEEK 1 DOWNLOAD AND VALIDATION LOG\n")
        f.write("=" * 80 + "\n")
    ...
    print(f"📁 {output_dir}/ingest_sep_w1_validity.csv")
    ...
    if all_ticks and coverage_pct >= 90.0:
        print(f"\n✅ **SEPTEMBER WEEK 1 DOWNLOAD COMPLETE**")
        print(f"✅ Coverage {coverage_pct:.1f}% meets 90% threshold")
    else:
        print(f"\n❌ **SEPTEMBER WEEK 1 DOWNLOAD INCOMPLETE**")

...
Final version with aggressive memory management
"""
...
def process_single_venue_date(venue, date):
    """Process a single venue×date combination with aggressive memory management"""
    ...
    chunk_size = 10000  # Even smaller chunks
    total_raw_rows = 0
    canonical_data = []
    ...
    try:
        ...
        # Get final stats
        final_df = pd.read_parquet(canonical_path)
        ...
        return {
            ...
            'canon_rows': len(pd.read_parquet(canonical_path)),
...
def main():
    ...
    print("🚀 Starting Week 2 Canonicalization - Days 1-2 (Final Version)")
    ...
    for date in dates:
        ...
        for venue in venues:
            result, error = process_single_venue_date(venue, date)
            ...
            # Aggressive garbage collection between venues
            gc.collect()

...
def validate_parquet_schema(file_path, venue, date):
    """Validate Parquet schema and basic metadata"""
    try:
        # Read schema only
        ...
        # Read first 5k and last 5k rows for timestamp validation
        df_sample = pd.read_parquet(file_path)
        
        # Check timestamp column
        if 'ts' not in df_sample.columns:
            return False, "No 'ts' column found", None, None
...
def main():
    ...
    while current_date <= end_date:
        ...
        for venue in venues:
            ...
            # Validate schema and get metadata
            schema_ok, schema_msg, metadata, df_sample = validate_parquet_schema(file_path, venue, date_str)

...
def batch_contract(ticks: List[Dict[str, Any]], vdate_iso: str) -> None:
    ...
    if (ok / evaluated) < CROSS_FIELD_THRESHOLD:
        raise ValueError(f"Cross-field consistency fail: {ok}/{evaluated} < {CROSS_FIELD_THRESHOLD:.0%}")

def validate_timestamp_window(ts_verified_sec: float, folder_date_ymd: str, window: str, tolerance_minutes: int = 2) -> None:
    ...
    if not (window_start - tol <= ts_dt <= window_end + tol):
        raise ValueError(f"Timestamp {ts_dt.isoformat()} outside window {window_start.isoformat()}–{window_end.isoformat()} (±{tolerance_minutes}m)")

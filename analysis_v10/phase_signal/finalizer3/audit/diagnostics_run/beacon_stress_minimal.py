#!/usr/bin/env python3
"""
Beacon Stress-Test — Minimal Version (GROUND TRUTH ONLY)
Data availability check and basic recommendation
"""
...
def load_tick_data_minimal(date, venue, start_time, end_time):
    """Load minimal tick data for diagnostics."""
...
# --- MAIN EXECUTION ---
log("=== Beacon Stress-Test — Minimal Version ===")
...
# 1. Verify total trade counts per venue/date (minimal version)
...
for venue in venues:
    for date in dates:
        ...
        tick_data, error = load_tick_data_minimal(date, venue, start_time, end_time)
...
# 2. Test a few key configurations only
log("2. Testing key configurations only")
configurations = [
    {'window_s': 10, 'band_pct': 0.05, 'micro_quantile': 'q10', 'rev_bps_limit': 5},
    {'window_s': 20, 'band_pct': 0.10, 'micro_quantile': 'q10', 'rev_bps_limit': 10},
    {'window_s': 30, 'band_pct': 0.25, 'micro_quantile': 'q15', 'rev_bps_limit': 10}
]
...
for i, config in enumerate(configurations):
    ...
    sample_events = windows_df.head(10)  # Only first 10 events
    ...
    # Early-stop rules
    if len(beacon_events_all) >= 5 and iqr >= 2:  # Lowered thresholds for sample

#!/usr/bin/env python3
"""
Beacon Clip Probe — Light Version (Ground Truth Only)
"""
...
def check_memory():
    ...
    if rss_mb > 250:  # Hard halt
        raise RuntimeError(f"HALT:RAM_CAP - RAM usage: {rss_mb:.1f}MB > 250MB")
    if rss_mb > 200:  # Soft cap
        gc.collect()
    return rss_mb
...
def find_price_crossing_timestamp_light(date, venue, round_level, band_pct=0.25):
    """Find the first timestamp where price enters the band around round level (light version)."""
    ...
    try:
        # Load tick data with time filter and small batch size
        ...
        for batch in dataset.to_batches(columns=['ts', 'price'], batch_size=batch_size):
            ...
            if within_band.any():
                ...
                if within_band.iloc[first_crossing_idx]:
                    result_ts = df_batch.iloc[first_crossing_idx]['ts']
                    del df_batch
                    gc.collect()
...
def extract_tick_window_light(date, venue, ts_event, window_seconds=60):
    """Extract ±60s tick window around the event timestamp (light version)."""
    tick_path = f"data_v6/views/{venue}/{date}/ticks_canonical.parquet"
...
def compute_light_metrics_light(tick_data, round_level):
    """Compute light metrics for the tick window (light version)."""
    ...
    # Find max trades in 10s window (simplified)
    max_trades_10s = 0
    if len(tick_data) > 0:
        # Sample every 10th row to reduce computation
        sample_size = min(1000, len(tick_data))
        sample_indices = np.linspace(0, len(tick_data)-1, sample_size, dtype=int)
        sample_data = tick_data.iloc[sample_indices]
        
        for i in range(len(sample_data) - 10):
            window_start = sample_data.iloc[i]['ts']
            window_end = window_start + timedelta(seconds=10)
            
            window_ticks = sample_data[
                (sample_data['ts'] >= window_start) & 
                (sample_data['ts'] <= window_end)
            ]
...
# --- MAIN EXECUTION ---
log("=== Beacon Clip Probe — Light Version ===")
...
for i, (_, selection) in enumerate(top_3_selections.iterrows()):
    ...
    ts_event, error = find_price_crossing_timestamp_light(date, venue, round_level)
    ...
    # Extract tick window
    tick_data, error = extract_tick_window_light(date, venue, ts_event)
    ...
    metrics = compute_light_metrics_light(tick_data, round_level)

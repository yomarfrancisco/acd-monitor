#!/usr/bin/env python3
"""
Wave 2b: TSI & Placebo Repair (Finalize Wave 2)
Fix TSI computation and generate valid placebos to benchmark Δβ against null.
"""
...
def load_event_window_tsi(date, pre_start, post_end, batch_size=32768):
    """Load event window data for TSI computation with proper alignment."""
...
def process_event_tsi_placebo(event_id, date, venue_event, ts_event, pre_start, post_end):
    """Process TSI and placebos for a single event."""
    # Load event window data
    current_memory, batch_size = check_memory()
    event_data = load_event_window_tsi(date, pre_start, post_end, batch_size)
...
# --- MAIN EXECUTION ---
log("=== Wave 2b: TSI & Placebo Repair ===")
...
for idx, window_row in windows_df.iterrows():
    ...
    try:
        ...
        tsi_results, placebo_results = process_event_tsi_placebo(
            window_row['event_id'],
            window_row['date'],
            window_row['venue'],
            window_row['t0'],
...
print("\n=== CHECKPOINT PHASE TRIGGER: Wave 2b (TSI & Placebo Repair) ===")
print("Key files:")
...
log("✅ CHECKPOINT PHASE TRIGGER: Wave 2b complete - READY FOR WAVE 3")

#!/usr/bin/env python3
"""
Wave 2: Core Δβ / ΔTSI Sanity (Corrective Re-run with Audit)
...
def load_event_window_corrective(date, pre_start, post_end, batch_size=32768):
    """Load event window data with proper time filtering."""
...
def compute_returns_safe(df):
    ...
    result_df = pd.DataFrame({
        ...
        'r_i': returns_i,
        'r_j': returns_j,
        'session': df['session'].iloc[1:].values,
        'wash_regime': df['wash_regime'].iloc[1:].values,
        'vol_regime': df['vol_regime'].iloc[1:].values
    })
...
def process_event_corrective_audit(event_id, date, venue_event, ts_event, pre_start, post_end):
    """Process a single event with corrective pair filtering and audit logging."""
    # Load event window data
    current_memory, batch_size = check_memory()
    event_data = load_event_window_corrective(date, pre_start, post_end, batch_size)
    ...
    for venue_i, venue_j in pair_tests:
        ...
        n_total_window = len(pair_data)
        
        if n_total_window < 100:
            # Log audit for insufficient data
            audit_logs.append({
                'event_id': event_id,
                'date': date,
        ...
        # Calculate window lengths in minutes
        pre_len_min = (ts_event - pre_data['ts'].min()).total_seconds() / 60 if n_pre > 0 else 0
        post_len_min = (post_data['ts'].max() - ts_event).total_seconds() / 60 if n_post > 0 else 0
        
        # Check if we need to shrink windows
        shrunk = False
        if n_pre < 50 or n_post < 50:
            # Try shrinking to 10-minute windows
            pre_start_shrunk = ts_event - timedelta(minutes=10)
            post_end_shrunk = ts_event + timedelta(minutes=10)
            
            pre_data_shrunk = pair_data[pair_data['ts'] >= pre_start_shrunk].copy()
            pre_data_shrunk = pre_data_shrunk[pre_data_shrunk['ts'] < ts_event].copy()
            post_data_shrunk = pair_data[pair_data['ts'] >= ts_event].copy()
            post_data_shrunk = post_data_shrunk[post_data_shrunk['ts'] <= post_end_shrunk].copy()
            
            if len(pre_data_shrunk) >= 50 and len(post_data_shrunk) >= 50:
                pre_data = pre_data_shrunk
                post_data = post_data_shrunk
                n_pre = len(pre_data)
                n_post = len(post_data)
                shrunk = True
        ...
        audit_logs.append({
            ...
            'n_pre': n_pre,
            'n_post': n_post,
            'pre_len_min': pre_len_min,
            'post_len_min': post_len_min,
            'shrunk': shrunk,
            'reason': 'insufficient_data' if (n_pre < 50 or n_post < 50) else 'success'
        })
        ...
        for lag in [0, 1, 2]:
            try:
                ...
                if len(y_post) < 50 or len(y_post) < 50:
                    continue
    ...
    return beta_results, tsi_results, placebo_results, audit_logs

# --- MAIN EXECUTION ---
log("=== Wave 2: Core Δβ / ΔTSI Sanity (Corrective Re-run with Audit) ===")
...
for idx, window_row in windows_df.iterrows():
    ...
    try:
        ...
        beta_results, tsi_results, placebo_results, audit_logs = process_event_corrective_audit(
            window_row['event_id'],
            window_row['date'],
            window_row['venue'],
            window_row['t0'],
        ...
        for audit_log in audit_logs:
            atomic_write_csv_line(ROOT / "logs/wave2_audit.csv", audit_log)
        
        processed_events += 1
        pair_lag_count += len(beta_results) + len(tsi_results)
        
        # Progress reporting every 25 pair×lag
        if pair_lag_count % 25 == 0:
            current_memory, _ = check_memory()
            log(f"Progress: {pair_lag_count} pair×lag processed (Memory: {current_memory:.1f}MB)")
...
output_hashes['logs/wave2_audit.csv'] = compute_file_hash(ROOT / "logs/wave2_audit.csv")
...
if placebo_pass_rate is not None:
    log(f"Placebo pass rate: {placebo_pass_rate:.1%} (≥70% required)")
else:
    log("Placebo pass rate: N/A (no placebo data)")

promotion_criteria_met = (
    failed_pct <= 30 and
    (placebo_pass_rate is None or placebo_pass_rate >= 70)
)
...
print(f"  • audit: analysis_v10/phase_trigger/logs/wave2_audit.csv")
...
print(f"  • placebo_pass_rate: {dashboard['placebo_pass_rate']}")

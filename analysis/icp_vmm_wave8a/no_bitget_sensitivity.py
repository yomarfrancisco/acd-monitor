#!/usr/bin/env python3
"""
Wave 8a - No-Bitget Sensitivity
Test if coordination persists without BITGET (central hub)
"""
...
def run_no_bitget_sensitivity():
    """Run ICP-VMM tests excluding BITGET entirely"""
    print("Wave 8a - No-Bitget Sensitivity")
    print("=" * 50)
    ...
    # Define venues (exclude BITGET)
    venues = ['BINANCE', 'COINBASE', 'BYBITSPOT']
    ...
    # Filter to venues without BITGET
    venue_cols = []
    for venue in venues:
        venue_cols.extend([col for col in df.columns if col.startswith(f"{venue}_")])
    
    # Keep only non-BITGET venues and environment columns
    env_cols = ['volatility', 'liquidity', 'time_of_day', 'coverage', 'wash_intensity']
    filtered_cols = venue_cols + env_cols
    filtered_df = df[filtered_cols].copy()
    
    # Add return columns from derived data
    ...
    # Add return columns for non-BITGET venues
    for venue in venues:
        ret_col = f"ret_{venue}"
        if ret_col in derived_df.columns:
            filtered_df[ret_col] = derived_df[ret_col]
    
    print(f"📊 Filtered design matrix: {len(filtered_df):,} rows")
    
    # Rebuild environments for 3-venue system
    print(f"\n📊 Rebuilding environments for 3-venue system...")
    
    # Volatility regimes (5-minute rolling realized variance of BTC/USDT on Binance)
    ...
    # Liquidity regimes (5-minute rolling total USD volume across 3 venues)
    ...
    # Coverage regimes (≥2 venues active)
    coverage_data = {}
    for venue in venues:
        ret_col = f"ret_{venue}"
    ...
    # Wash-intensity regimes (if available)
    if 'vol_BINANCE' in filtered_df.columns:
        vol_volatility = filtered_df['vol_BINANCE'].rolling(window=300, min_periods=100).std()
        wash_quantiles = vol_volatility.quantile([0.33, 0.67])
        filtered_df['wash_intensity'] = pd.cut(
            vol_volatility,
            bins=[-np.inf, wash_quantiles[0.33], wash_quantiles[0.67], np.inf],
            labels=['low', 'medium', 'high']
        )
        print(f"  Wash-intensity: {filtered_df['wash_intensity'].value_counts().to_dict()}")
    else:
        filtered_df['wash_intensity'] = 'INDETERMINATE'
        print(f"  Wash-intensity: INDETERMINATE")
    ...
    for i, j in itertools.product(venues, venues):
        ...
        for lag in lags:
            ...
            test_data = pd.DataFrame({
                ...
                'wash_intensity': filtered_df['wash_intensity']
            }).dropna()
            ...
            for env_name in ['volatility', 'liquidity', 'time_of_day', 'coverage', 'wash_intensity']:
                if env_name not in test_data.columns:
                    continue
    ...
    # Save results
    os.makedirs('analysis/icp_vmm_wave8a/no_bitget', exist_ok=True)
    os.makedirs('analysis/icp_vmm_wave8a/no_bitget/icp', exist_ok=True)
    os.makedirs('analysis/icp_vmm_wave8a/no_bitget/vmm', exist_ok=True)
    os.makedirs('analysis/icp_vmm_wave8a/no_bitget/graph', exist_ok=True)
    
    # Save ICP results
    with open('analysis/icp_vmm_wave8a/no_bitget/icp/edges_icp_v1.json', 'w') as f:
        json.dump(icp_results, f, indent=2, default=str)
    
    # Save VMM results
    with open('analysis/icp_vmm_wave8a/no_bitget/vmm/edges_vmm_v1.json', 'w') as f:
        json.dump(vmm_results, f, indent=2, default=str)
    
    # Generate summary
    print(f"\n{'='*50}")
    print("NO-BITGET SENSITIVITY RESULTS")
    print(f"{'='*50}")
    ...
    wave8_3venue_edges = 18  # Approximate count of 3-venue edges in Wave 8
    
    if len(passing_edges) >= 0.5 * wave8_3venue_edges:
        robustness_status = "✅ ROBUST"
        interpretation = "Evidence persists without BITGET"
    elif len(passing_edges) >= 0.2 * wave8_3venue_edges:
        robustness_status = "⚠️ PARTIAL"
        interpretation = "Some evidence persists without BITGET"
    else:
        robustness_status = "❌ FRAGILE"
        interpretation = "Evidence depends on BITGET"
    ...
    # Draw edges
    nx.draw_networkx_edges(G, pos, edge_color='darkblue', arrows=True, arrowsize=20, alpha=0.6)
    ...
    plt.title("No-Bitget Sensitivity: Causal Graph\n(3-venue system without BITGET)")
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('analysis/icp_vmm_wave8a/no_bitget/graph/graph_v1.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    summary = {
        ...
        'wave8_comparison': {
            ...
            'wave8_3venue_edges': wave8_3venue_edges,
            'survival_rate': len(passing_edges) / wave8_3venue_edges if wave8_3venue_edges > 0 else 0
        },
        'final_edges': passing_edges
    }
    
    with open('analysis/icp_vmm_wave8a/no_bitget/summary_v1.md', 'w') as f:
        f.write(f"# No-Bitget Sensitivity Results\n\n")
        ...
        f.write(f"- **Survival rate**: {len(passing_edges) / wave8_3venue_edges * 100:.1f}%\n\n")
    ...
    with open('analysis/icp_vmm_wave8a/no_bitget/graph/graph_v1.json', 'w') as f:
        json.dump(graph_data, f, indent=2, default=str)
    
    print(f"\n📄 Results saved:")
    print(f"   - analysis/icp_vmm_wave8a/no_bitget/icp/edges_icp_v1.json")
    print(f"   - analysis/icp_vmm_wave8a/no_bitget/vmm/edges_vmm_v1.json")
    print(f"   - analysis/icp_vmm_wave8a/no_bitget/graph/graph_v1.json|png")
    print(f"   - analysis/icp_vmm_wave8a/no_bitget/summary_v1.md")
    
    return passing_edges

if __name__ == "__main__":
    result = run_no_bitget_sensitivity()
    
    if result is not None:
        print(f"\n✅ No-Bitget sensitivity completed - proceeding to No-Wash test")
    else:
        print(f"\n❌ No-Bitget sensitivity failed - stopping execution")

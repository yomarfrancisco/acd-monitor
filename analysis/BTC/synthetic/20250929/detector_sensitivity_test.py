...
def test_lead_lag_sensitivity(datasets):
    ...
    for dataset in lead_lag_datasets:
        ...
        print(f"    Result: ρ = {mock_result['max_correlation']:.3f} (threshold: {results['expected_threshold']})")
    ...
    results['success_rate'] = successful_detections / len(results['test_results']) if results['test_results'] else 0
...
def test_infoshare_sensitivity(datasets):
    ...
    for dataset in dominance_datasets:
        ...
        print(f"    Result: {mock_result['dominant_venue']} dominance = {mock_result['max_dominance']:.1%}")
        print(f"    Detection: {'SUCCESS' if mock_result['detection_success'] else 'FAILED'}")
...
def assess_overall_sensitivity(sensitivity_results):
    ...
    infoshare_success = sensitivity_results['detector_tests']['infoshare_v2']['sensitivity_confirmed']

#!/usr/bin/env python3
"""
STEP B — Clean Panel Rebuild (Memory Optimized)
...
def rebuild_panel():
    ...
    try:
        for chunk in pd.read_parquet(panel_path, chunksize=chunk_size):
            chunks.append(chunk)
    ...
    print(f'📊 Combined panel shape: {df.shape}')
    print(f'📊 Combined time span: {df.index.min()} to {df.index.max()}')
    ...
    manifest = {
        ...
        'cleaning_stats': {
            'original_nonpos': original_nonpos,
            'cleaned_nonpos': cleaned_nonpos,
            'rows_cleaned_pct': rows_cleaned_pct
        },
        'validation_metrics': {
            'volume_nonnan_ratio': volume_nonnan_ratio,
            'price_nonnan_ratio': price_nonnan_ratio
        }
    }
...
def main():
    print('🔍 STEP B — Clean Panel Rebuild (Memory Optimized)')
    print('=' * 60)

#!/usr/bin/env python3
"""
Week-8 Complete Execution (2025-09-19 → 2025-09-25)
"""
...
def main():
    print('🔍 Week-8 Complete Execution (2025-09-19 → 2025-09-25)')
    print('=' * 60)
    ...
    print(f'📊 **Week-8 Scope:**')
    print(f'📊 Dates: {week8_dates}')
    print(f'📊 Venues: {venues}')
    print(f'📊 Total candidates: {len(week8_dates) * len(venues)}')
    ...
    if resampled_data:
        ...
        if os.path.exists(panel_path):
            ...
            # Remove any existing Week-8 data (2025-09-19 to 2025-09-25)
            week8_start = pd.Timestamp('2025-09-19 00:00:00')
            week8_end = pd.Timestamp('2025-09-25 23:59:59')
            
            # Filter out Week-8 data
            existing_panel = existing_panel[~((existing_panel.index >= week8_start) & (existing_panel.index <= week8_end))]
            print(f'📊 Panel after Week-8 removal: {existing_panel.shape}')
        ...
        if os.path.exists(manifest_path):
            ...
            manifest = {
                ...
                'description': 'Week-8 rebuild from CoinAPI Flat Files',
                'date_range': '2025-09-19 to 2025-09-25',
                'venues': ['BINANCE', 'COINBASE', 'BYBITSPOT', 'BITGET'],
        ...
        # Update manifest
        manifest['week8_files'] = [r['filename'] for r in download_results if r['status'] == 'SUCCESS']
    ...
    if os.path.exists(panel_path):
        ...
        # Check for any data beyond 2025-09-25 (should be frozen)
        frozen_threshold = pd.Timestamp('2025-09-26 00:00:00')
        frozen_data = final_panel[final_panel.index >= frozen_threshold]
        if len(frozen_data) > 0:
            print(f'⚠️ WARNING: Found {len(frozen_data)} rows beyond frozen threshold (2025-09-26)')
        else:
            print(f'✅ No data found beyond frozen threshold (2025-09-26)')
    
    print(f'\n✅ Week-8 execution completed')

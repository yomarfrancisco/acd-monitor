#!/usr/bin/env python3
"""
Week-4 Real-Beacon Analysis - Day 4 Only (2025-08-07) with Expanded ±15s Window
Exact same validated pipeline and guardrails, but expanded effects window
"""
...
def step1_beacon_detection(date_str, venue_data, venues, cache_path="data_v6/cache/beacons/week-4"):
...
def compute_effects_expanded(event, venue_data, venues, min_coverage_pct=60):
    """Compute effects for a single event with expanded ±15s window"""
    event_ts = event['event_ts']
    
    # Define analysis windows - EXPANDED to ±15 seconds
    pre_start_15s = event_ts - pd.Timedelta(seconds=15)
    post_end_15s = event_ts + pd.Timedelta(seconds=15)
    post_end_30s = event_ts + pd.Timedelta(seconds=30)
    post_end_45s = event_ts + pd.Timedelta(seconds=45)
    ...
    for venue in venues:
        ...
        # Check coverage
        expected_bars = (post_end_45s - pre_start_15s).total_seconds() + 1
    ...
    # Compute ΔDispersion with expanded windows
    delta_dispersions = []
    
    for window_seconds in [15, 30, 45]:
        pre_end = event_ts
        post_start = event_ts
        ...
        for venue in venues:
            ...
            pre_bars = vwap[(vwap.index >= pre_start_15s) & (vwap.index < pre_end)]
    ...
    # Typology with expanded window thresholds
    median_delta_disp_15s = np.median(delta_dispersions) if len(delta_dispersions) > 0 else 0
    
    if median_delta_disp_15s >= 0.001:  # 10 bps
        typology = 'Signal-10'
    elif median_delta_disp_15s >= 0.0007:  # 7 bps
        typology = 'Signal-7'
    elif abs(median_delta_disp_15s) < 0.0005:  # 5 bps
    ...
    return {
        'status': 'OK',
        'delta_dispersion_15s': median_delta_disp_15s,
        'delta_dispersion_30s': np.median(delta_dispersions[1:]) if len(delta_dispersions) > 1 else 0,
        'delta_dispersion_45s': np.median(delta_dispersions[2:]) if len(delta_dispersions) > 2 else 0,
        'delta_lag_ms': np.median(delta_lags) if len(delta_lags) > 0 else 0,
        'typology': typology
    }

def step2_effects_computation_expanded(date_str, all_beacons, venue_data, venues):
    """Step 2: Effects computation for a single day with expanded windows"""
    print(f"🔍 Step 2 - Effects Computation (Expanded ±15s) for {date_str}")
    print("-" * 40)
    
    results = []
    
    for beacon in all_beacons:
        effects = compute_effects_expanded(beacon, venue_data, venues)
    ...
    delta_disp_15s = [r['delta_dispersion_15s'] for r in ok_results]
    delta_disp_30s = [r['delta_dispersion_30s'] for r in ok_results]
    delta_disp_45s = [r['delta_dispersion_45s'] for r in ok_results]
    ...
    if len(delta_disp_15s) > 0:
        median_disp = np.median(delta_disp_15s)
        iqr_disp = np.percentile(delta_disp_15s, 75) - np.percentile(delta_disp_15s, 25)
    ...
    summary = {
        ...
        'median_delta_disp_15s': np.median(delta_disp_15s),
        'median_delta_disp_30s': np.median(delta_disp_30s),
        'median_delta_disp_45s': np.median(delta_disp_45s),
        'mean_delta_disp_15s': np.mean(delta_disp_15s),
        'mean_delta_disp_30s': np.mean(delta_disp_30s),
        'mean_delta_disp_45s': np.mean(delta_disp_45s),
        'median_delta_lag_ms': np.median(delta_lag_ms),
        'mean_delta_lag_ms': np.mean(delta_lag_ms),
...
def analyze_convergence_divergence(event_ts, venue_data, venues, window_seconds=15):
    """Analyze convergence vs divergence outcomes with expanded window"""
...
def step3_leadership_dynamics(date_str, all_beacons, venue_data, venues):
    ...
    for i, beacon in enumerate(all_beacons):
        ...
        # Analyze convergence/divergence with expanded window
        outcome, pre_disp, post_disp = analyze_convergence_divergence(
            event_ts, venue_data, venues, window_seconds=15
        )
...
def main():
    print("🧾 Week-4 Real-Beacon Analysis - Day 4 Only (2025-08-07) - EXPANDED ±15s WINDOW")
    print("=" * 80)
    print("Mode: STRICT READ-ONLY, STREAMING, PERSISTENT CACHE")
    print("Effects Window: ±15s (expanded from ±3s)")
    ...
    # Step 2: Effects Computation (Expanded)
    effects_ok, effects_summary = step2_effects_computation_expanded(date_str, all_beacons, venue_data, venues)
    if not effects_ok:
    ...
    # Checkpoint Output
    print(f"\n📊 DAY {date_display} - SUMMARY TABLES (EXPANDED ±15s WINDOW)")
    print("=" * 80)
    ...
    # Table B: Effects (Expanded)
    print(f"\nTable B (Effects, processed only, ±15s window):")
    print(f"{'Metric':<30} {'Mean':<15} {'Median':<15}")
    print("-" * 60)
    print(f"{'Δdisp15s (bps)':<30} {effects_summary['mean_delta_disp_15s']:.2f} {effects_summary['median_delta_disp_15s']:.2f}")
    print(f"{'Δdisp30s (bps)':<30} {effects_summary['mean_delta_disp_30s']:.2f} {effects_summary['median_delta_disp_30s']:.2f}")
    print(f"{'Δdisp45s (bps)':<30} {effects_summary['mean_delta_disp_45s']:.2f} {effects_summary['median_delta_disp_45s']:.2f}")
    print(f"{'Δlag (ms)':<30} {effects_summary['mean_delta_lag_ms']:.2f} {effects_summary['median_delta_lag_ms']:.2f}")
    ...
    s7_count = typology_counts.get('Signal-7', 0)
    comp_count = typology_counts.get('Compression', 0)
    print(f"{'Typology (S10/S7/Comp)':<30} {s10_count}/{s7_count}/{comp_count}")
    print(f"{'n_processed':<30} {effects_summary['processed_ok']}")
    ...
    print(f"• {success_rate:.1f}% beacon processing success rate (expanded window)")
    ...
    print(f"• {effects_summary['median_delta_disp_15s']:.2f} bps median dispersion change (±15s)")
    print(f"• {convergence_pct:.1f}% convergence vs {divergence_pct:.1f}% divergence")
    
    # Narrative Interpretation
    print(f"\n📝 Leadership Re-emergence Analysis:")
    if leadership_success_rate > 0:
        print(f"Leadership dynamics have re-emerged with the expanded ±15s window.")
        print(f"The {dominant_leader} venue shows dominant leadership ({dominant_pct:.1f}% share),")
        print(f"indicating that the original ±3s window was too narrow to capture")
        print(f"the full leadership dynamics in Week-4 data.")
    else:
        print(f"Leadership dynamics remain absent even with the expanded ±15s window.")
        print(f"This suggests a fundamental difference in market microstructure")
        print(f"between Week-4 and other weeks, possibly due to data quality or")
        print(f"market conditions during this period.")
    
    print(f"\nMemory usage: {get_memory_usage():.1f} MB")
    print(f"DAY {date_display} COMPLETE (EXPANDED WINDOW) — awaiting next instructions.")

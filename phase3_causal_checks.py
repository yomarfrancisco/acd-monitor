#!/usr/bin/env python3
"""
Phase 3: Causal Checks (Weeks 1–2)
...
def build_vwap_and_micro_trades(venue, date):
    ...
    try:
        ...
        for batch in parquet_file.iter_batches(batch_size=batch_size, columns=['ts', 'price', 'size']):
            ...
            # Timestamps are already timezone-aware from above
...
def read_tick_window_fixed(venue, date, event_second, window_seconds=90):
    """Read tick-level data for a specific window around an event - FIXED VERSION"""
    file_path = f"data_v6/views/{venue}/{date}/ticks_canonical.parquet"
...
def run_phase3_causal_checks():
    """Run Phase 3: Causal Checks on Weeks 1-2"""
    print("🔍 Phase 3: Causal Checks (Weeks 1–2)")
    print("=" * 70)
    ...
    for day_idx, date in enumerate(dates):
            ...
            for venue in venues:
                ...
                if lag_result:
                    ...
                    if abs(lag_result['median_lag_ms']) > 100:
                        print(f"    ⚠️  Event {i+1} {venue}: High lag {lag_result['median_lag_ms']:.1f}ms")
                else:
        ...
        event_seconds = [e['second'] for e in all_day_events]
        ...
        # Checkpoint A (after 2025-09-04)
        if day_idx == 3:  # 2025-09-04
            print(f"\n🔍 Checkpoint A (After 2025-09-04):")
            print("-" * 50)
        ...
        # Checkpoint B (after 2025-09-10)
        if day_idx == 9:  # 2025-09-10
            print(f"\n🔍 Checkpoint B (After 2025-09-10):")
            print("-" * 50)
            ...
            print(f"  Running totals (Days 1-10):")
            print(f"    Tighter: {running_tighter_events} events")
            print(f"    Lower-Signal: {running_lower_signal_events} events")
            print(f"    Signal: {running_signal_events}, Compression: {running_compression_events}, Unclassified: {running_unclassified_events}")
    ...
    print(f"\n✅ Phase 3: Causal Checks completed")
...
def main():
    """Main function"""
    success = run_phase3_causal_checks()
    
    if success:
        print(f"\n🎉 Phase 3: Causal Checks completed successfully.")
        print(f"Ready for downstream analysis and Week 3 extension.")
    else:
        print(f"\n❌ Phase 3: Causal Checks failed. Waiting for instructions.")
        sys.exit(1)

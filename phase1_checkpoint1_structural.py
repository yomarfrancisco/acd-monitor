#!/usr/bin/env python3
"""
Phase 1 Checkpoint 1: Structural Consistency Diagnostics
Analyze canonical tick data for structural integrity without any data generation
"""
...
def analyze_venue_day_file(venue, date):
    ...
    try:
        # Load the file in chunks to manage memory
        ...
        # 7. Check for negative prices or sizes (sample check to save memory)
...
def check_price_overlap(day_results):
    """Check if all venues have overlapping price ranges on the same day"""
...
def run_checkpoint1():
    """Run Checkpoint 1: Structural Consistency"""
    print("🧾 Checkpoint 1 — Structural Consistency")
    print("=" * 60)
    ...
    for date in dates:
        ...
        for venue in venues:
            ...
            day_results[venue] = result
            print(f"    ✅ {venue}: {result['rows']:,} rows, {result['coverage_pct']:.1f}% coverage")
        ...
        # Check price overlap for this day
        overlap_ok, overlap_msg = check_price_overlap(day_results)
        if not overlap_ok:
            print(f"    ❌ Price overlap issue: {overlap_msg}")
            print(f"❌ HALT — Issue detected at Checkpoint 1. Summary below.")
        ...
        print(f"  ✅ {date}: All venues passed, price ranges overlap")
    ...
    for date in dates:
        day_results = all_results[date]
        overlap_ok, _ = check_price_overlap(day_results)
        
        for venue in venues:
            result = day_results[venue]

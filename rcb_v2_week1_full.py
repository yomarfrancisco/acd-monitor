#!/usr/bin/env python3
"""
RCB v2 — Week 1 Full Run (Dual Configs, Read-Only, Memory-Safe)
Run on 2025-09-01 → 2025-09-07 with Tighter and Lower-Signal configs
"""
...
def build_vwap_and_micro_trades(venue, date):
    ...
    try:
        ...
        # Check coverage threshold (relaxed for some venues)
        if coverage_pct < 40:
...
def run_rcb_v2_week1_full():
    """Run RCB v2 on full Week 1 with dual configs"""
    print("🔍 RCB v2 — Week 1 Full Run (Dual Configs, Read-Only, Memory-Safe)")
    print("=" * 70)
    ...
    print(f"\n✅ RCB v2 Week 1 Full Run completed")
...
def main():
    """Main function"""
    success = run_rcb_v2_week1_full()
    
    if success:
        print(f"\n🎉 RCB v2 Week 1 Full Run completed successfully.")
        print(f"Please review results and provide approval for Week 2.")
    else:
        print(f"\n❌ RCB v2 Week 1 Full Run failed. Waiting for instructions.")
        sys.exit(1)

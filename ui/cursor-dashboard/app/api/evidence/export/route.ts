...
async function generateFallbackZip(): Promise<Buffer> {
  const zip = new JSZip();
  ...
  // Generate as Uint8Array first, then convert to Buffer
  ...
  // Convert to Node Buffer for proper binary handling
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log('ZIP buffer first 4 bytes:', Array.from(zipBuffer.slice(0, 4)).map(b => b.toString(16).padStart(2, '0')).join(' '));

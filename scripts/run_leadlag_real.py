...
def setup_logging(verbose: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("leadlag_real.log")],
    )
...
def run_leadlag_analysis_snapshot(overlap_data, horizons, out_dir):
    ...
    try:
        ...
        if final_rows < min_required_rows:
            logger.error(
                f"[ABORT:leadlag:insufficient_rows] Found {final_rows} rows, need ≥{min_required_rows} for {window_days}-day window"
            )
            sys.exit(2)

        logger.info(
            f"[STATS:leadlag:rows] total={final_rows} venues={len(venues)} horizons={horizons}"
        )

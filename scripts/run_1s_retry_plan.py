...
def validate_inputs(baseline_overlap: str, candidate_1s_overlap: str) -> None:
    ...
    print(
        f'[CHECK:overlap_json] {{"baseline":"{baseline_overlap}","candidate":"{candidate_1s_overlap}","status":"valid"}}'
    )
...
def apply_tolerance_adjustment(
    ...
    if rule == "increase_duration_permutes":
        ...
        logger.info(
            f"Adjusted: duration={adjusted_settings['min_duration_sec']}s, permutes={adjusted_settings['permutes']}"
        )
...
def run_final_analysis(
    ...
    leadlag_cmd = [
        ...
        candidate_overlap,
        "--horizons",
        ",".join(map(str, settings["leadlag_horizons"])),
        "--export-dir",
        str(evidence_dir),
        "--verbose" if verbose else "",
    ]
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help="Path to candidate 1s OVERLAP.json",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Clock skew tolerance in seconds",
    )
    parser.add_argument(
        "--max-js", type=float, default=0.02, help="Maximum Jensen-Shannon distance"
    )
    parser.add_argument(
    ...
    try:
        ...
        success = run_final_analysis(
            args.candidate_1s_overlap, args.export_dir, adjusted_settings, args.verbose
        )
...
def run_ratchet_mode(args, horizons):
    ...
    for i, config in enumerate(ratchet_configs):
        ...
        try:
            ...
            print(
                f"[RETRY:1s:ratchet:{rung_name}:ERROR] {json.dumps({'error': str(e), 'config': config})}"
            )

...
def clean_snapshot(snapshot_dir: str, verbose: bool = False):
    ...
    logger.info(
        f"Cleaning complete: {total_venues} venues, {total_files} files kept, {total_removed} files removed"
    )
    print(
        f"[CLEAN:summary] venues={total_venues}, files_kept={total_files}, files_removed={total_removed}"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Abort if any files are removed",
    )

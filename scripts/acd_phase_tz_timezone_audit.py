...
def load_slice_data(s3_client, bucket: str, date: str, venue: str, slice_name: str) -> Tuple[Optional[pd.DataFrame], Optional[Dict[str, Any]]]:
            ...
            try:
                if primary_timestamp in df.columns:
                    ...
                    if duplicate_ratio > 0.5:
                        slice_result["issues"] = slice_result.get("issues", []) + [f"High duplicate ratio: {duplicate_ratio:.1%}"]
                    
                    if duration < 1:
                        slice_result["issues"] = slice_result.get("issues", []) + [f"Very short duration: {duration:.1f}s"]
                ...
                slice_result = {
                    "status": "failed",
                    "error": f"Error processing timestamps: {e}"
                }
            
            venue_results["slices"][slice_name] = slice_result
        
        # Aggregate venue-level analysis
        successful_slices = [s for s in venue_results["slices"].values() if s.get("status") == "success"]
        ...
        if successful_slices:
            ...
            if all_timestamps:
                ...
                try:
                    ...
                    timezone_offsets = [ts.utcoffset().total_seconds() if ts.utcoffset() else 0 for ts in parsed_times]
                    
                    if len(set(timezone_offsets)) > 1:
                        venue_results["issues"].append("Inconsistent timezone offsets across slices")
            ...
            if successful_slices:
                ...
                if "timestamp_analysis" in first_slice:
                    ...
                    if "timestamp" in timestamp_analysis:
                        venue_results["normalization_path"] = f"timestamp field -> UTC (via pd.to_datetime)"
                    else:
...
"""
    ...
    for venue, venue_data in audit_results['venues'].items():
        successful_slices = [s for s in venue_data['slices'].values() if s.get('status') == 'success']
        ...
        markdown_content += f"**Normalization Path**: {venue_data.get('normalization_path', 'unknown')}\n\n"
        ...
        if successful_slices:
            markdown_content += "| Slice | Rows | Duration | Monotonic | Duplicates | Price Std | Issues |\n"
            markdown_content += "|-------|------|----------|-----------|------------|-----------|--------|\n"
            
            for slice_name, slice_data in venue_data['slices'].items():
                if slice_data.get('status') == 'success':
                    issues_str = ', '.join(slice_data.get('issues', [])) if slice_data.get('issues') else 'None'

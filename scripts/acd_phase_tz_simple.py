...
def analyze_slice_timestamps(s3_client, bucket: str, date: str, venue: str, slice_name: str) -> Dict[str, Any]:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ACD Phase TZ - Simple Timestamp/Timezone Forensics')
    ...
    for venue in venues:
        ...
        for slice_name in slices:
            ...
            if result["status"] == "success":
                print(f"     ✅ {result['n_rows']} rows, {result['duration_seconds']:.1f}s, {result['duplicate_ratio']:.1%} duplicates")
                if result['issues']:
    ...
    for venue, venue_slices in all_results.items():
        ...
        for slice_name, result in venue_slices.items():
            if result["status"] == "success":
                print(f"  {slice_name}: {result['ts_min_utc']} → {result['ts_max_utc']} ({result['duration_seconds']:.1f}s)")
                if result['issues']:

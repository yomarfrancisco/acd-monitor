...
def gen_competitive():
    ...
    for ts, pf, pn in zip(dates, fnb, ned):
        for firm, price in (("FNB", pf), ("NED", pn)):
            d = dict(ts=ts.isoformat(), firm_id=firm, price=round(float(price),1),
                     unit="bps", src="derived", confidence=95)
            d["hash"]=row_hash(d)
            rows.append(d)
...
def gen_coordinated():
    ...
    for ts, pf, pn in zip(dates, fnb, ned):
        for firm, price in (("FNB", pf), ("NED", pn)):
            d = dict(ts=ts.isoformat(), firm_id=firm, price=round(float(price),1),
                     unit="bps", src="derived", confidence=96)
            d["hash"]=row_hash(d)
            rows.append(d)
...
def gen_public_like():
    dates = pd.to_datetime([
        "2024-06-03T10:00:00Z","2024-06-03T12:00:00Z","2024-06-03T14:00:00Z"
    ])
    ...
    for ts in dates:
        for firm in ("FNB","NED"):
            ...
            d = dict(ts=ts.isoformat(), firm_id=firm, price=int(prices[firm][(ts.hour-10)//2]),
                     unit="bps", src=src, confidence=conf)
            d["hash"]=row_hash(d)
            rows.append(d)

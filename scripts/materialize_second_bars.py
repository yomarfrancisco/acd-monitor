...
from _analysis_utils import ensure_time_mid_volume, inclusive_end_date, resample_second
...
def materialize_second_bars(
    ...
    print(
        f"[MZ:start] file=second_bars start={start_utc.isoformat()} end={end_utc.isoformat()} granularity=1s"
    )
    ...
    for venue in venues:
        ...
        try:
            ...
            if venue_results["coverage_pct"] < 95.0:
                ...
                logger.warning(
                    f"[DATA:gaps] {venue}:{pair} - {venue_results['coverage_pct']:.1f}% coverage"
                )

        except Exception as e:
            logger.error(f"Error processing {venue}:{pair} - {str(e)}")
            results["venues"][venue] = {"bars": 0, "coverage_pct": 0.0, "error": str(e)}

    # Calculate overall coverage
    if results["total_bars"] > 0:
        expected_total = (
            len(venues) * ((end_utc - start_utc).days + 1) * 24 * 60 * 60
        )  # seconds per day
        results["coverage_pct"] = (results["total_bars"] / expected_total) * 100

    logger.info(
        f"Materialization complete: {results['total_bars']} total bars, {results['coverage_pct']:.1f}% coverage"
    )
...
def analyze_data_quality(
    df, venue: str, pair: str, start_utc: datetime, end_utc: datetime
) -> Dict[str, Any]:
    """
...
def export_data_inventory(results: Dict[str, Any], export_dir: str) -> None:
    """
    ...
    inventory = {
        ...
        "metadata": {
            ...
            "frequency": "1s",
            "sources": {"second": "synthetic_v1" if results["synthetic"] else "native"},
        },
    }
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["yes", "no"],
        default="yes",
        help="Use synthetic second data",
    )
    ...
    try:
        ...
        if results["gaps"]:
            ...
            for gap in results["gaps"]:
                print(
                    f"  {gap['venue']}: {gap['coverage_pct']:.1f}% coverage ({gap['missing_bars']} missing bars)"
                )
        else:

...
def main():
    ...
    try:
        ...
        if test_results['overall']['status'] == 'INSUFFICIENT':
            ...
            print(f"✅ ICP tests completed: {icp_tests.get('overall', {}).get('status', 'UNKNOWN')}")
        ...
        invariant_params = [p for p, result in icp_tests.get('parameter_tests', {}).items() 
                          if result.get('significant', False) == False]
        variant_params = [p for p, result in icp_tests.get('parameter_tests', {}).items() 
        ...
        if export_status['status'] == 'SUCCESS':
            ...
            # Print key log lines
            print("\n📋 Key Log Lines:")
            print(f"[ICPVMM:status] window={window_id} status={status} venues={len(venue_data)} coverage={sum(coverage.values())/len(coverage):.2f}")
            print(f"[ENV:bins] session={len(env_counts.get('session', {}))} vwap={len(env_counts.get('vwap_side', {}))} highlow={len(env_counts.get('hl_bucket', {}))} liq={len(env_counts.get('liquidity_regime', {}))} leader={len(env_counts.get('leadership_regime', {}))}")
            print(f"[VMM] mode={johansen_mode} rank={rank} lags={lags}")
            print(f"[ICP] tested_params={len(tested_params)} invariant={icp_tests.get('overall', {}).get('status', 'UNKNOWN')} fdr_q=0.05")
            print(f"S3 path: {export_status.get('s3_path', 'N/A')}")
            ...
            manifest_sample = {
                ...
                'methods': {
                    'vmm': {
                        'johansenMode': johansen_mode,
                        'lags': lags,
                        'rank': rank
                    },
                    'icp': {
                        'fdrQ': 0.05,
                        'testedParams': tested_params
                    }
                }
            }

...
def run_analysis_on_window(window_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if result.returncode != 0:
            return {"window_id": window_id, "status": "ERROR", "error": result.stderr}
        ...
        return {"window_id": window_id, "status": "ERROR", "error": str(e)}
...
def main():
    ...
    if upload_index(bucket, prefix, index_content):
        ...
        print(
            f"📊 Results: {len([r for r in results if r['status'] == 'SUCCESS'])}/{len(results)} successful"
        )
        sys.exit(0)
    else:

...
def main():
    ...
    try:
        ...
        print(f"✅ Created data for {len(venue_data)} venues with {len(venue_data['binance'])} observations")
        ...
        if export_status['status'] == 'SUCCESS':
            ...
            # Print key log lines
            print("\n📋 Key Log Lines:")
            print(f"[ICPVMM:status] window={results['window_id']} status={status} venues={len(venue_data)} coverage=1.00")
            print(f"[ENV:bins] session={len(env_counts.get('session', {}))} vwap_side={len(env_counts.get('vwap_side', {}))} highlow={len(env_counts.get('hl_bucket', {}))} liquidity={len(env_counts.get('liquidity_regime', {}))} leadership={len(env_counts.get('leadership_regime', {}))}")
            if vmm_results:
                print(f"[VMM] mode={vmm_results.get('mode', 'UNKNOWN')} venues={len(vmm_results.get('venues', []))}")
            if icp_tests:
                print(f"[ICP] tested_params={len(icp_tests.get('parameter_tests', {}))} invariant={icp_tests.get('overall', {}).get('status', 'UNKNOWN')} fdr_q=0.05")

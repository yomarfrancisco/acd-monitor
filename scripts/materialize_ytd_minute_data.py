...
from _analysis_utils import ensure_time_mid_volume, inclusive_end_date, resample_minute
...
def materialize_minute_data(
    ...
    print(
        f"[MZ:start] file=minute_data start={start_utc.isoformat()} end={end_utc.isoformat()} granularity=1m"
    )
    ...
    for venue in venues:
        try:
            ...
            logger.info(
                f"[STATS:materialize:granularity=1m] venue={venue} expected={expected_minutes} actual={actual_minutes} coverage={coverage}"
            )
            ...
            if coverage_pct < min_coverage * 100:
                logger.warning(
                    f"Insufficient coverage for {venue}: {coverage_pct:.2f}% < {min_coverage*100}%"
                )
                results["venues"][venue] = {
                    "success": False,
            ...
            logger.info(
                f"Successfully materialized {actual_bars} bars for {venue} ({coverage_pct:.2f}% coverage)"
            )
    ...
    logger.info(
        f"Materialization complete: {results['successful_venues']} successful, {results['failed_venues']} failed"
    )

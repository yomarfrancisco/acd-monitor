...
class ContinuousMeasurement:
    ...
    def calculate_highs_lows(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        if len(df) == 0:
            return {"high_24h": np.nan, "low_24h": np.nan, "high_7d": np.nan, "low_7d": np.nan}
    ...
    def calculate_spread_metrics(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        if 'spread_bps' in df.columns:
            ...
            return {
                ...
                "spread_bps_std": ((df['best_ask'] - df['best_bid']) / df['best_bid'] * 10000).std()
            }
        else:
            return {"spread_bps_mean": np.nan, "spread_bps_std": np.nan}
    ...
    def calculate_leadership_shares(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        volume_weighted = {venue: vol / total_volume if total_volume > 0 else 0 
        ...
        price_impact_normalized = {venue: impact / total_impact if total_impact > 0 else 0 
        ...
        info_leadership_normalized = {venue: info / total_info if total_info > 0 else 0 
        ...
        for metric in ["spread_specialist", "volume_specialist", "volatility_specialist", "liquidity_specialist"]:
            ...
            if total > 0:
                for venue in venues:
                    venue_specialization[venue][metric] = venue_specialization[venue][metric] / total

...
def compute_per_venue_stats(canonical_data: Dict[str, pd.DataFrame]) -> Dict[str, Dict[str, Any]]:
    ...
    for venue, df in canonical_data.items():
        ...
        logger.info(f"{venue}: {n_rows} rows, ${price_mean:.2f}±${price_std:.2f}, {volume_total:.2f} vol, {duplicate_ratio:.1%} dup")
...
def compute_cross_venue_comparisons(per_venue_stats: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    logger.info(f"Cross-venue analysis: max spread {max_spread:.2f}%, mean spread {mean_spread:.2f}%")
    
    return cross_venue_analysis

def generate_summary_markdown(per_venue_stats: Dict[str, Dict[str, Any]], cross_venue_analysis: Dict[str, Any]) -> str:
...
"""
    ...
    for venue, stats in per_venue_stats.items():
        ...
        if stats['duplicate_ratio'] > 0.30:
            quality_flags.append(f"⚠️ {venue.upper()}: High duplicates ({stats['duplicate_ratio']:.1%})")
    
    if cross_venue_analysis['max_spread_pct'] > 1.5:
        quality_flags.append(f"⚠️ High cross-venue spread: {cross_venue_analysis['max_spread_pct']:.3f}% (threshold: 1.5%)")

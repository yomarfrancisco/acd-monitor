...
def generate_30m_report(window_data: Dict, mtc_results: Dict, leadlag_results: Dict, gates: Dict) -> str:
    ...
    report.append(f"- **Episodes Surviving FDR**: {sum(1 for ep in spread if ep['bh_fdr_10_q'] < 0.10)}")
    report.append("")
    ...
    report.append("")
    report.append(f"- **Gate 1 (Spread Episodes)**: {'✅ PASS' if gates['gate_1_spread_episodes'] else '❌ FAIL'}")
    report.append(f"- **Gate 2 (Lead-Lag Edges)**: {'✅ PASS' if gates['gate_2_leadlag_edges'] else '❌ FAIL'}")
    report.append(f"- **Gate 3 (InfoShare Stability)**: {'✅ PASS' if gates['gate_3_infoshare_stability'] else '❌ FAIL'}")
    report.append("")
...
def main():
    parser = argparse.ArgumentParser(description='Gold Hunt Phase 5 - 30+ Minute Window Auto-Analysis')
    parser.add_argument('--output-dir', default='experiments/gold_hunt_v1/phase4_30m',
                       help='Output directory for results')
    parser.add_argument('--export-dir', default='exports/gold_hunt/latest/phase4_30m',

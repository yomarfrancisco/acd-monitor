...
def compute_leader_follower_analysis(aligned_series: Dict[str, pd.Series], max_lag: int = 10) -> Dict[str, Any]:
    ...
    for i, venue1 in enumerate(venues):
        for j, venue2 in enumerate(venues):
            ...
            logger.info(f"{venue1} vs {venue2}: max correlation = {max_correlation:.3f} at lag {best_lag}")
    
    return leader_follower_results

def compute_volatility_clustering(aligned_series: Dict[str, pd.Series], window_sizes: List[int] = [30, 60]) -> Dict[str, Any]:
...
def compute_coordination_indicators(aligned_series: Dict[str, pd.Series], correlation_results: Dict[str, Any], leader_follower_results: Dict[str, Any], volatility_clustering: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if correlation_results and 'pairwise_correlations' in correlation_results:
        correlations = [v['correlation'] for v in correlation_results['pairwise_correlations'].values()]
        if correlations:
...
def generate_signal_report(correlation_results: Dict[str, Any], leader_follower_results: Dict[str, Any], volatility_clustering: Dict[str, Any], coordination_indicators: Dict[str, Any]) -> str:
...
"""
    ...
    if volatility_clustering:
        for window_size, vol_data in volatility_clustering.items():
            ...
            report_content += f"- **Mean Volatility Correlation**: {vol_data['mean_vol_correlation']:.3f}\n"
            report_content += f"- **Max Volatility Correlation**: {vol_data['max_vol_correlation']:.3f}\n"
...
"""
    ...
    if coordination_indicators['coordination_score'] > 0.5:
        report_content += "**🔍 COORDINATION DETECTED**: High coordination signals detected across venues.\n\n"
    elif coordination_indicators['competition_score'] > 0.5:
        report_content += "**⚔️ COMPETITION DETECTED**: High competition signals detected across venues.\n\n"
    else:
...
def main():
    ...
    try:
        ...
        print(f"✅ Computed correlations for {len(correlation_results['pairwise_correlations'])} pairs")
    ...
    try:
        signal_report = generate_signal_report(
            correlation_results, leader_follower_results, volatility_clustering, coordination_indicators
        )

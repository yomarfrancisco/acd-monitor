...
def get_windows_last_24h():
    ...
    for symbol in SYMBOLS:
        try:
            response = s3.list_objects_v2(Bucket=BUCKET, Prefix=f"{PREFIX}/{symbol}/")
                        ...
                        try:
                            if "-" in date_part:
                                ...
                                window_time = datetime.strptime(
                                    f"{date_part}_{start_time}", "%Y-%m-%d_%H%M"
                                )
                            else:
                ...
                if parquet_files:
                    ...
                    try:
                        ...
                        venue_analysis[venue] = {
                            ...
                            "duplicates": (
                                int(df["ts_exchange"].duplicated().sum())
                    ...
                    venue_analysis[venue] = {"present": False, "rows": 0, "duplicates": 0}
...
def generate_report():
    ...
    report = {"timestamp": datetime.utcnow().isoformat(), "period": "last_7_days", "symbols": {}}
    ...
    for symbol in SYMBOLS:
        ...
        if window_analyses:
            ...
            avg_duplicates = sum(w["duplicate_rate"] for w in window_analyses) / len(
                window_analyses
            )
            quality_windows = len(
                [
                    ...
                    if w["venues_present"] >= 3 and w["duplicate_rate"] <= 0.1
                ]
            )

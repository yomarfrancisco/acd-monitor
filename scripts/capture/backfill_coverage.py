...
def windows_for_symbol(sym):
    ...
    pat = re.compile(rf"{re.escape(PREFIX)}/{re.escape(sym)}/(\d{{4}}-\d{{2}}-\d{{2}}|\d{{8}})/\d{{4}}-\d{{4}}")
...
def compute_coverage(base_key):
    ...
    return {"coverage_percentage": overall, **{v: {"coverage_percentage": per_venue[v]} for v in per_venue}}
...
def write_coverage(base_key, cov):
    ...
    s3.put_object(Bucket=BUCKET, Key=f"{base_key}/meta/coverage.json", Body=body, ContentType="application/json")

...
def check_window_coverage(symbol: str, date: str, time_range: str, 
                         bucket: str, prefix: str) -> Dict:
    ...
    try:
        ...
        try:
            ...
            coverage_data = {venue: {"coverage_percentage": overlap_data["coverage"][venue] * 100} 
                           for venue in overlap_data["venues"]}
        
        # Calculate coverage statistics
        venues = overlap_data["venues"]
        coverage_percentages = [coverage_data.get(venue, {}).get("coverage_percentage", 0) 
                               for venue in venues]
        
        high_coverage_venues = [venue for venue, data in coverage_data.items() 
...
def generate_coverage_report(symbols: List[str], days_back: int, 
                           bucket: str, prefix: str) -> Dict:
    ...
    try:
        ...
        meets_threshold_windows = len([r for r in coverage_results 
            ...
            if result.get("status") == "available":
                for venue, data in result.get("coverage_data", {}).items():
                    if venue not in venue_stats:
                        venue_stats[venue] = {"windows": 0, "total_coverage": 0, "high_coverage": 0}
        ...
        return {
            "summary": {
                ...
                "threshold_rate": meets_threshold_windows / available_windows if available_windows > 0 else 0
            },
...
def main():
    ...
    parser.add_argument("--output", default="reports/coverage_report.json",
    ...
    try:
        ...
        if venue_performance:
            ...
            for venue, stats in venue_performance.items():
                print(f"  {venue}: {stats.get('avg_coverage', 0):.1f}% avg, {stats.get('high_coverage_rate', 0):.1%} high coverage")

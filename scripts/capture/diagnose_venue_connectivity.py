    ...
    try:
        ...
        if success:
            ...
            try:
                ...
                await venue_ws.connection.close()
                return {"venue": venue, "success": False, "message": f"Subscription failed: {e}"}
        else:
            logger.error(f"❌ {venue} connection failed")
            return {"venue": venue, "success": False, "message": "Connection failed"}

    except Exception as e:
        logger.error(f"❌ {venue} test failed: {e}")
        return {"venue": venue, "success": False, "message": f"Test failed: {e}"}

...
def validate_window_quality(
    ...
    try:
        ...
        available_venues = [
            v
            for v, data in validation_results["venues"].items()
...
def validate_recent_windows(symbols: List[str], days_back: int, bucket: str, prefix: str) -> Dict:
    ...
    for symbol in symbols:
        ...
        symbol_results = {"windows": [], "summary": {}}
        ...
        try:
            ...
            windows_with_issues = len([w for w in symbol_results["windows"] if w["overall_issues"]])

            symbol_results["summary"] = {
                "total_windows": total_windows,
                "windows_with_issues": windows_with_issues,
                "quality_rate": (
                    (total_windows - windows_with_issues) / total_windows
                    if total_windows > 0
                    else 0
                ),
            }
    ...
    total_windows = sum(
        s.get("summary", {}).get("total_windows", 0) for s in all_results["symbols"].values()
    )
    total_issues = sum(
        s.get("summary", {}).get("windows_with_issues", 0) for s in all_results["symbols"].values()
    )
    ...
    all_results["overall_summary"] = {
        ...
        "overall_quality_rate": (
            (total_windows - total_issues) / total_windows if total_windows > 0 else 0
        ),
    }
...
def main():
    ...
    try:
        ...
        results = validate_recent_windows(symbols, args.days_back, args.bucket, args.prefix)

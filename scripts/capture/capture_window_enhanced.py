...
async def capture_window_websocket(symbol: str, start_time: datetime, end_time: datetime,
                                 venues: List[str], bucket: str, prefix: str) -> Dict:
...
def capture_window_fallback(symbol: str, start_time: datetime, end_time: datetime,
                          venues: List[str], bucket: str, prefix: str) -> Dict:
    ...
    try:
        ...
        # Write to S3
        success = write_snapshot_to_s3(venue_data, coverage_data, symbol, start_time, end_time, bucket, prefix)
...
def write_snapshot_to_s3(venue_data: Dict[str, pd.DataFrame], 
                        coverage_data: Dict,
                        symbol: str, start_time: datetime, end_time: datetime,
                        bucket: str, prefix: str) -> bool:
...
async def capture_window_enhanced(symbol: str, start_time: datetime, end_time: datetime,
                                venues: List[str], bucket: str, prefix: str,
                                use_websocket: bool = True) -> Dict:
...
def main():
    ...
    parser.add_argument("--no-websocket", action="store_true", help="Skip WebSocket, use fallback only")
    ...
    try:
        ...
        # Capture window
        result = asyncio.run(capture_window_enhanced(
        ...
        if result.get("success"):
            ...
            if "coverage_data" in result:
                ...
                print(f"  Meets threshold (≥3 venues): {'YES' if len(high_coverage) >= 3 else 'NO'}")

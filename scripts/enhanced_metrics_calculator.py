...
class EnhancedMetricsCalculator:
    ...
    def calculate_advanced_liquidity_metrics(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        # Depth imbalance
        depth_imbalance = (avg_bid_size - avg_ask_size) / (avg_bid_size + avg_ask_size) if (avg_bid_size + avg_ask_size) > 0 else 0
    ...
    def calculate_advanced_leadership_shares(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Dict[str, float]]:
        ...
        volume_shares = {venue: vol / total_volume if total_volume > 0 else 0 
            ...
            if venue in data and len(data[venue]) > 0:
                ...
                if len(df) > 1:
                    ...
                    if volume_changes.sum() > 0 and price_changes.sum() > 0:
                        price_impact[venue] = (volume_changes * price_changes).sum() / volume_changes.sum()
                    else:
        ...
        impact_shares = {venue: impact / total_impact if total_impact > 0 else 0 
        ...
        info_shares = {venue: info / total_info if total_info > 0 else 0 
    ...
    def calculate_venue_specialization(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Dict[str, float]]:
        ...
        for venue in venues:
            ...
            specialization[venue] = {
                "spread_specialist": 1 / (1 + avg_spread) if avg_spread > 0 else 0,  # Inverse spread
        ...
        for metric in ["spread_specialist", "volume_specialist", "volatility_specialist", "liquidity_specialist"]:
    ...
    def calculate_market_microstructure_metrics(self, data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        fragmentation = {
            ...
            "venue_dominance": max(len(df) for df in data.values()) / sum(len(df) for df in data.values()) if sum(len(df) for df in data.values()) > 0 else 0
        }
        ...
        for venue, df in data.items():
            if len(df) > 1:
                ...
                price_discovery[venue] = {
                    ...
                    "price_autocorrelation": price_changes.autocorr() if len(price_changes) > 1 else 0
                }
            else:
                price_discovery[venue] = {
                    "price_volatility": 0,

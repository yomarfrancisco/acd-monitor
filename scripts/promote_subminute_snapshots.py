...
def find_subminute_snapshots(
    ...
    for snapshot_dir in g_snapshots:
        try:
            ...
            logger.info(
                f"Found valid {granularity}s snapshot: {duration_seconds:.0f}s, {len(venues)} venues, coverage={coverage:.3f}"
            )
...
def clean_snapshot(snapshot_path: str, verbose: bool = False) -> bool:
    ...
    try:
        cmd = [sys.executable, "scripts/clean_snapshot.py", "--snapshot", snapshot_path]
        if verbose:
            cmd.append("--verbose")
...
def build_research_bundle(
    snapshot_path: str, export_dir: str, granularity: int, verbose: bool = False
) -> bool:
    """
...
def main():
    ...
    for i, snapshot in enumerate(valid_snapshots):
        ...
        if args.build_bundles:
            ...
            if not build_research_bundle(
                snapshot["snapshot"], str(bundle_dir), args.granularity, args.verbose
            ):
    ...
    logger.info(
        f"Promotion completed: {len(promoted_snapshots)} {args.granularity}s snapshots promoted"
    )

...
def discover_available_data(s3_client, bucket: str) -> Dict[str, Any]:
    ...
    discovery = {"raw_probes": {}, "backfill": {}, "earliest_date": None, "latest_date": None}
            ...
            for prefix in response["CommonPrefixes"]:
                ...
                if date_str and len(date_str) == 8:  # YYYYMMDD format
                    ...
                    venue_response = s3_client.list_objects_v2(
                        Bucket=bucket, Prefix=f"raw_probes/{date_str}/", Delimiter="/"
                    )
                            ...
                            if venue:
                                ...
                                if "CommonPrefixes" in slice_response:
                                    for slice_prefix in slice_response["CommonPrefixes"]:
                                        slice_name = (
                                            slice_prefix["Prefix"].split("=")[-1].strip("/")
                                        )
                                        if slice_name:
                                            discovery["raw_probes"][date_str][venue].append(
                                                slice_name
                                            )
    except Exception as e:
            ...
            for prefix in response["CommonPrefixes"]:
                ...
                if venue:
                    ...
                    date_response = s3_client.list_objects_v2(
                        Bucket=bucket, Prefix=f"backfill/{venue}/", Delimiter="/"
                    )
                            ...
                            if date_str and len(date_str) == 8:
                                ...
                                if "CommonPrefixes" in window_response:
                                    for window_prefix in window_response["CommonPrefixes"]:
                                        ...
                                        if window_name:
                                            discovery["backfill"][venue][date_str].append(
                                                window_name
                                            )
    except Exception as e:
...
def get_window_metadata(
    s3_client, bucket: str, data_type: str, date: str, venue: str, window: str
) -> Dict[str, Any]:
...
def compute_temporal_integrity(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    within_5pct = (
        (time_deltas_seconds >= mode_interval * 0.95)
        & (time_deltas_seconds <= mode_interval * 1.05)
    ).sum()
...
def classify_window(metrics: Dict[str, Any], evidence: Dict[str, Any]) -> str:
    ...
    if (
        temporal.get("regular_interval_flag", False)
        and price.get("degenerate_flag", False)
        and price.get("staleness_ratio", 0) >= 0.9
    ):
        return "SYNTHETIC"

    # LIKELY_SYNTHETIC: Regular-interval TRUE or degenerate std < $0.10 with duplicate ratio ≥ 30% and no log evidence
    if (
        (temporal.get("regular_interval_flag", False) or price.get("degenerate_flag", False))
        and duplicates.get("exact_duplicate_ratio", 0) >= 0.3
        and not evidence.get("log_evidence", False)
    ):
        return "LIKELY_SYNTHETIC"

    # CORRUPTED: Monotonicity failures ≥ 10% or exact duplicates ≥ 50%
    if (
        temporal.get("monotonicity_failure_ratio", 0) >= 0.1
        or duplicates.get("exact_duplicate_ratio", 0) >= 0.5
    ):
        return "CORRUPTED"
    ...
    if (
        temporal.get("cv_dt", 0) >= 0.10
        and price.get("std_price", 0) >= 0.10
        and duplicates.get("exact_duplicate_ratio", 0) < 0.3
        and temporal.get("monotonicity_failure_ratio", 0) < 0.01
        and evidence.get("log_evidence", False)
        and price.get("staleness_ratio", 0) < 0.5
        and temporal.get("unique_timestamp_ratio", 0) >= 0.7
    ):
        return "REAL"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ACD Phase PROV - Provenance & Synthetic-Detection"
    )
        ...
        for venue, slices in venues.items():
            for slice_name in slices:
                metadata = get_window_metadata(
                    s3_client, args.bucket, "raw_probes", date_str, venue, slice_name
                )
    ...
    for venue, dates in discovery["backfill"].items():
        for date_str, windows in dates.items():
            for window_name in windows:
                metadata = get_window_metadata(
                    s3_client, args.bucket, "backfill", date_str, venue, window_name
                )
                inventory["windows"].append(metadata)
    ...
    for window in inventory["windows"]:
        ...
        try:
            ...
            classifications[key] = {
                ...
                "evidence": evidence,
                "rule_hits": {
                    "regular_interval": metrics["temporal_integrity"].get(
                        "regular_interval_flag", False
                    ),

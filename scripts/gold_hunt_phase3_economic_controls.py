...
def compute_economic_controls(mid_prices_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for venue in venues:
        ...
        imbalance = (
            (mid_prices_df[bid_depth_col] - mid_prices_df[ask_depth_col]) / total_depth
        ).mean()
...
def compute_infoshare_controls_correlations(
    infoshare_data: Dict, controls_df: pd.DataFrame
) -> pd.DataFrame:
...
def compute_episode_controls_contrast(
    episodes: List[Dict], mid_prices_df: pd.DataFrame, controls_df: pd.DataFrame
) -> pd.DataFrame:
...
def generate_controls_report(
    controls_df: pd.DataFrame, correlations_df: pd.DataFrame, contrasts_df: pd.DataFrame
) -> str:
    ...
    for control in ["volume_proxy", "realized_vol", "liquidity_proxy"]:
        ...
        if not control_corrs.empty:
            ...
            report.append(
                f"- Significant correlations: {(control_corrs['p_raw'] < 0.05).sum()}/{len(control_corrs)}"
            )
            report.append("")
    ...
    for control in ["volume_proxy", "spread_proxy"]:
        ...
        if not control_contrasts.empty:
            ...
            report.append(
                f"- Episodes with positive diff: {(control_contrasts['diff'] > 0).sum()}/{len(control_contrasts)}"
            )
            report.append("")
    ...
    if volume_contrasts.mean() > 0:
        report.append(
            "**Episodes occur during high-volume periods** - volume may explain episode incidence"
        )
    else:
    ...
    report.append("")
    report.append(
        "**Conclusion**: Controls provide partial explanation for signals but do not fully account for observed patterns."
    )
...
def main():
    ...
    with open(f"{args.output_dir}/execution_log.txt", "w") as f:
        ...
        f.write(
            f"Command: python scripts/gold_hunt_phase3_economic_controls.py --seed {args.seed}\n"
        )

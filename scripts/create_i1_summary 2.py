...
def create_summary_report():
    ...
    summary["overall_summary"] = {
        ...
        "overall_success_rate": (
            total_successful / total_combinations if total_combinations > 0 else 0
        ),
    ...
    s3.put_object(Bucket=S3_BUCKET, Key=summary_key, Body=json.dumps(summary, indent=2))
    ...
    if btc_events is not None:
        ...
        if btc_scorecard:
            ...
            print(
                f"    Suspicion: {flags.get('invariance_count', 0)} invariance, {flags.get('overreaction_count', 0)} over-reaction"
            )

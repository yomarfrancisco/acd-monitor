#!/usr/bin/env python3
"""
Repair Research Bundle Script V2
...
def load_tick_data_from_live_run(live_run_dir: Path, venues: List[str], start_utc: str, end_utc: str) -> Dict[str, pd.DataFrame]:
    """Load tick data from live run directory."""
    logger.info(f"[REPAIR:load] Loading ticks from live run: {live_run_dir}")
    ...
    for venue in venues:
        venue_dir = live_run_dir / "overlap" / "ticks" / venue / "BTC-USD" / "1s"
...
def main():
    ...
    parser.add_argument("--overlap-json", help="External OVERLAP.json file")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Use external OVERLAP.json if provided, otherwise extract from manifest
        ...
        ticks = load_tick_data_from_live_run(live_run_dir, venues, start_utc, end_utc)

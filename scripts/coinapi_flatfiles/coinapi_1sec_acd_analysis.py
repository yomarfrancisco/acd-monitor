...
def main():
    ...
    if corr_results["status"] == "success":
        print(f"   ✅ {corr_results['n_venues']} venues, {corr_results['common_length']} observations")
    ...
    if lead_lag_results["status"] == "success":
        ...
        for pair, data in lead_lag_results["lead_lag_pairs"].items():
            print(f"      {pair}: lag {data['best_lag_seconds']}s, corr {data['best_correlation']:.3f}")
    ...
    if vol_results["status"] == "success":
        print(f"   ✅ {vol_results['n_venues']} venues, {vol_results['window_size_seconds']}-sec window")
    ...
    for analysis_type, data in [("corr_1sec", corr_results), ("leadlag_1sec", lead_lag_results), 
                               ("spreads_1sec", spread_results), ("vol_co_move_1sec", vol_results)]:
    ...
    print(f"  - 1-second correlation matrix: s3://{bucket}/analysis/coinapi/_sig_1sec/corr_1sec.json")
    print(f"  - 1-second lead-lag analysis: s3://{bucket}/analysis/coinapi/_sig_1sec/leadlag_1sec.json")
    print(f"  - 1-second spread analysis: s3://{bucket}/analysis/coinapi/_sig_1sec/spreads_1sec.json")
    print(f"  - 1-second volatility co-movement: s3://{bucket}/analysis/coinapi/_sig_1sec/vol_co_move_1sec.json")

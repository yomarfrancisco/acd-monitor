...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("coinapi_7day_backfill.log")
        ]
    )
    ...
    for venue in VENUES:
        ...
        try:
            ...
            if venue == "BINANCE":
                ...
                btc_usdt = [s for s in venue_symbols if s.get('symbol_id') == 'BINANCE_SPOT_BTC_USDT']
                if btc_usdt:
                ...
                # Prefer BTC/USD spot
                btc_usd = [s for s in venue_symbols if s.get('symbol_id') == 'COINBASE_SPOT_BTC_USD']
                if btc_usd:
...
def fetch_venue_day_data(venue: str, symbol_id: str, date: str, logger) -> Tuple[bool, Dict[str, Any], Optional[pd.DataFrame]]:
...
def validate_candle_data(df: pd.DataFrame, venue: str, date: str, logger) -> Dict[str, Any]:
    ...
    for _, row in df.iterrows():
        if not (row['price_low'] <= row['price_open'] <= row['price_high'] and
                row['price_low'] <= row['price_close'] <= row['price_high'] and
                row['price_high'] >= row['price_low']):
            ohlc_violations += 1
    ...
    logger.info(f"   ✅ {venue} {date}: {qc_results['coverage_pct']:.1f}% coverage, {ohlc_violations} OHLC violations")
    return qc_results

def save_venue_day_data(s3_client, venue: str, date: str, manifest: Dict[str, Any], df: pd.DataFrame, qc_results: Dict[str, Any]):
...
def build_aligned_panel(s3_client, start_date: str, end_date: str, symbols: Dict[str, str], logger) -> Tuple[bool, Dict[str, Any]]:
    ...
    for date in date_range:
        ...
        if len(date_data) >= 2:  # Need at least 2 venues
            ...
            for venue, df in date_data.items():
                ...
                df_renamed = df_renamed[['time_period_start'] + [col for col in df_renamed.columns if col != 'time_period_start']]
                
                if aligned_data is None:
                    aligned_data = df_renamed
                else:
                    aligned_data = aligned_data.merge(df_renamed, on='time_period_start', how='inner')
            ...
            logger.info(f"   📊 {date_str}: {len(aligned_data)} aligned candles, {daily_overlaps[date_str]['coverage_pct']:.1f}% coverage")
    ...
    if daily_overlaps:
        ...
        if unified_panel is not None:
            ...
            logger.info(f"   ✅ Unified panel: {len(unified_panel)} rows, {panel_manifest['overall_overlap']:.1f}% coverage")
...
def compute_analytics(s3_client, logger) -> Dict[str, Any]:
    ...
    for venue in VENUES:
        ...
        if f'{venue_lower}_mid' in df.columns:
            df[f'{venue_lower}_return'] = np.log(df[f'{venue_lower}_mid'] / df[f'{venue_lower}_mid'].shift(1))
    ...
    return_cols = [f'{venue.lower()}_return' for venue in VENUES if f'{venue.lower()}_return' in df.columns]
    correlations = {}
    ...
    summary = {
        "panel_stats": {
            ...
            "duration_hours": (df['time_period_start'].max() - df['time_period_start'].min()).total_seconds() / 3600
        },
        "spread_stats": {
            "mean_spread_pct": float(df['spread_pct'].mean()) if 'spread_pct' in df.columns else None,
            "median_spread_pct": float(df['spread_pct'].median()) if 'spread_pct' in df.columns else None,
            "p95_spread_pct": float(df['spread_pct'].quantile(0.95)) if 'spread_pct' in df.columns else None
        },
...
def main():
        ...
        for date in pd.date_range(start_date, end_date):
            ...
            if success and df is not None:
                ...
                if qc_results["status"] == "passed":
                    ...
                    venue_failures += 1
                    venue_log["days"][date_str] = {"status": "failed", "issues": qc_results["issues"]}
            else:
                venue_failures += 1
                venue_log["days"][date_str] = {"status": "failed", "error": manifest.get("error", "Unknown error")}
    ...
    panel_success, panel_manifest = build_aligned_panel(s3_client, start_date, end_date, symbols, logger)

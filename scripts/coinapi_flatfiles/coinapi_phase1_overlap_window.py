    ...
    for window_minutes in window_sizes:
        ...
        while current_start + window_duration <= common_end:
            ...
            for symbol, df in ohlcv_data.items():
                ...
                timestamps = pd.to_datetime(df['time_period_start'], utc=True)
                window_data = df[
                    (timestamps >= current_start) & 
                    (timestamps < current_end)
                ]
            ...
            meets_criteria = (
                total_venues >= 2 and  # At least 2 venues
                all(venue["meets_threshold"] for venue in window_coverage.values() if venue["n_bars"] > 0)
            )
            ...
            if meets_criteria:
                window_info = {
                    ...
                    "quality_score": sum(venue["coverage_pct"] for venue in window_coverage.values()) / total_venues
                }
                
                if best_window is None or window_info["quality_score"] > best_window["quality_score"]:
                    best_window = window_info
                    print(f"      ✅ Found {window_minutes}-min window: {current_start.isoformat()} to {current_end.isoformat()}")
                    print(f"         Quality: {window_info['quality_score']:.3f}, Venues: {total_venues}")
...
def main():
    ...
    if overlap_result["status"] == "success":
        ...
        for symbol, coverage in best_window["coverage"].items():
            print(f"      {symbol}: {coverage['n_bars']} bars ({coverage['coverage_pct']:.1%} coverage)")
        ...
        if "venue_ranges" in overlap_result:
            ...
            for symbol, tr in overlap_result["venue_ranges"].items():
                print(f"      {symbol}: {tr['start'].isoformat()} to {tr['end'].isoformat()} ({tr['n_bars']} bars)")

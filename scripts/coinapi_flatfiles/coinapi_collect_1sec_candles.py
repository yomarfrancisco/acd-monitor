...
def collect_1sec_candles(
    ...
    params = {"period_id": "1SEC", "time_start": start_date, "time_end": end_date, "limit": 1000}
...
def main():
    ...
    for symbol in symbols:
        ...
        if success and df is not None:
            ...
            print(
                f"   📊 Time span: {df['time_period_start'].min()} to {df['time_period_start'].max()}"
            )
            print(
                f"   📊 Price range: ${df['price_close'].min():.2f} - ${df['price_close'].max():.2f}"
            )
            ...
            print(f"   ❌ {message['message']}")
            all_results[symbol] = {"status": "failed", "message": message["message"]}
    ...
    print("=" * 80)
    successful_venues = [
        symbol for symbol, result in all_results.items() if result["status"] == "success"
    ]

...
def block_bootstrap_correlation(x: np.ndarray, y: np.ndarray, block_length: int = 5, n_bootstrap: int = 1000) -> Dict[str, float]:
...
def main():
    ...
    if corr_results["status"] == "success":
        ...
        for pair, data in corr_results["correlations"].items():
            ...
            print(f"      {pair}: {pearson['correlation']:.3f} (CI: {pearson['ci_lower']:.3f}-{pearson['ci_upper']:.3f})")
    ...
    for analysis_type, data in [("corr", corr_results), ("leadlag", lead_lag_results), 
                               ("spreads", spread_results), ("vol_co_move", vol_results)]:

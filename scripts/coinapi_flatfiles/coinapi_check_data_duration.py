...
def main():
    ...
    for symbol in symbols:
        ...
        if analysis["status"] == "success":
            print(f"   ✅ Duration: {analysis['duration_hours']:.2f} hours ({analysis['duration_minutes']:.1f} minutes)")
            print(f"   📊 Time range: {analysis['start_time']} to {analysis['end_time']}")
            print(f"   📊 Candles: {analysis['n_candles']} (expected: {analysis['expected_candles']:.0f})")
            print(f"   📊 Coverage: {analysis['coverage_pct']:.1f}%")
            print(f"   📊 Gaps: {analysis['n_gaps']} gaps")
            
            if analysis['n_gaps'] > 0:
                print(f"   ⚠️  Largest gap: {analysis['gap_details']['largest_gap_seconds']:.1f} seconds")
    ...
    if overall_start and overall_end:
        ...
        if abs(overall_duration - 3600) < 60:  # Within 1 minute of 1 hour
            ...
            print(f"📊 Data covers {overall_duration:.0f} seconds ({overall_duration/3600:.2f} hours)")
    ...
    for symbol, analysis in all_analysis.items():
        if analysis["status"] == "success":
            print(f"{symbol}:")
            print(f"   Duration: {analysis['duration_hours']:.2f}h ({analysis['duration_seconds']:.0f}s)")

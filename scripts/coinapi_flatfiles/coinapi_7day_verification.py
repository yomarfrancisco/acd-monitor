...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("coinapi_7day_verification.log")],
    )
...
def task1_alignment_coverage_audit(s3_client, bucket: str, dates: List[str]) -> pd.DataFrame:
    ...
    for date in dates:
        ...
        for venue1, venue2 in pairs:
            if venue1 in venue_data and venue2 in venue_data:
                ...
                logger.info(
                    f"      {venue1}↔{venue2}: {coverage_pair:.1%} coverage ({seconds_intersection}/{seconds_total}s)"
                )
            else:
    ...
    with open(f"{OUTPUT_DIR}/coverage_table.md", "w") as f:
        ...
        if not coverage_df.empty:
            ...
            f.write(
                f"- **Pairs with ≥30% coverage**: {(coverage_df['coverage_pair'] >= 0.30).sum()}\n"
            )
            f.write(
                f"- **Pairs with ≥50% coverage**: {(coverage_df['coverage_pair'] >= 0.50).sum()}\n"
            )
        else:
...
def task2_spreads_analysis(
    s3_client, bucket: str, dates: List[str], coverage_df: pd.DataFrame
    ...
    for date in dates:
        ...
        for venue1, venue2 in pairs:
            if venue1 in venue_data and venue2 in venue_data:
                ...
                logger.info(
                    f"      {venue1}↔{venue2}: Mid={spread_mid_mean:.4f}, Log={spread_log_mean:.4f}"
                )
    ...
    if not daily_spreads_df.empty:
        ...
        pooled_spreads = {
            "spread_mid_mean_pooled": np.average(
                daily_spreads_df["spread_mid_mean"], weights=weights
            ),
            "spread_mid_median_pooled": np.average(
                daily_spreads_df["spread_mid_median"], weights=weights
            ),
            "spread_mid_p95_pooled": np.average(
                daily_spreads_df["spread_mid_p95"], weights=weights
            ),
            "spread_mid_max_pooled": np.average(
                daily_spreads_df["spread_mid_max"], weights=weights
            ),
            "spread_log_mean_pooled": np.average(
                daily_spreads_df["spread_log_mean"], weights=weights
            ),
            "spread_log_median_pooled": np.average(
                daily_spreads_df["spread_log_median"], weights=weights
            ),
            "spread_log_p95_pooled": np.average(
                daily_spreads_df["spread_log_p95"], weights=weights
            ),
            "spread_log_max_pooled": np.average(
                daily_spreads_df["spread_log_max"], weights=weights
            ),
    ...
    if pooled_spreads:
        logger.info(
            f"   Mid spread (pooled): {pooled_spreads['spread_mid_mean_pooled']:.6f} ({pooled_spreads['spread_mid_mean_pooled']*100:.3f}%)"
        )
        logger.info(
            f"   Log spread (pooled): {pooled_spreads['spread_log_mean_pooled']:.6f} ({pooled_spreads['spread_log_mean_pooled']*100:.3f}%)"
        )
        logger.info(f"   Previous report: 0.225%")
        logger.info(f"   Previous pooled: 0.037%")
        logger.info(
            f"   Difference: {abs(pooled_spreads['spread_mid_mean_pooled']*100 - 0.225):.3f}%"
        )
...
def task3_returns_lag_analysis(
    s3_client, bucket: str, dates: List[str], coverage_df: pd.DataFrame
) -> pd.DataFrame:
                ...
                leadlag_results.append(
                    {
                        ...
                        "lag_minus5_correlation": (
                            correlations[lags.index(-5)] if -5 in lags else np.nan
                        ),
                    }
                )

                logger.info(
                    f"      {venue1}↔{venue2}: argmax_lag={argmax_lag}, max_corr={max_corr:.4f}"
                )
...
def task4_kraken_recovery(s3_client, bucket: str, dates: List[str]) -> Dict:
    ...
    for date in dates:
        ...
        # Duplicate ratio
        dup_ratio = df.duplicated(subset=["timestamp", "price_close", "volume_traded"]).sum() / len(
            df
        )
        ...
        kraken_recovery["daily_results"].append(
            {
                ...
                "reason": (
                    "All criteria met"
                    if passes_all
                    else f"Failed: CV={cv_dt:.3f}, Dup={dup_ratio:.3f}, Std=${price_std:.2f}, Cov={coverage:.3f}"
                ),
                "cv_dt": cv_dt,
                "dup_ratio": dup_ratio,
                "price_std": price_std,
        ...
        logger.info(
            f"      Kraken {date}: {'✅ PASSED' if passes_all else '❌ FAILED'} - {cv_dt:.3f} CV, {dup_ratio:.1%} dup, ${price_std:.2f} std"
        )
    ...
    logger.info(
        f"✅ Kraken recovery analysis complete: {kraken_recovery['days_passed']}/{len(dates)} days passed"
    )
    return kraken_recovery


def task5_robustness_analysis(
    s3_client, bucket: str, dates: List[str], coverage_df: pd.DataFrame
) -> str:
    ...
    with open(f"{OUTPUT_DIR}/robustness_spreads_lag.md", "w") as f:
        ...
        f.write(
            robustness_df.groupby("resolution")[["spread_mid_mean", "spread_log_mean"]]
            .mean()
            .to_markdown()
        )
        ...
        f.write(
            f"- **Mean spread (1s)**: {robustness_df[robustness_df['resolution']==1]['spread_mid_mean'].mean():.6f}\n"
        )
        f.write(
            f"- **Mean spread (60s)**: {robustness_df[robustness_df['resolution']==60]['spread_mid_mean'].mean():.6f}\n"
        )
...
def task6_sensitivity_analysis(
    s3_client, bucket: str, dates: List[str], coverage_df: pd.DataFrame
) -> str:
    ...
    with open(f"{OUTPUT_DIR}/sensitivity.md", "w") as f:
        ...
        f.write(
            sensitivity_df.groupby("subset")[
                ["spread_mid_mean", "correlation_0", "volatility_mean"]
            ]
            .mean()
            .to_markdown()
        )
        ...
        f.write(
            f"- **High volatility subset**: {len(sensitivity_df[sensitivity_df['subset']=='high_vol'])} observations\n"
        )
        f.write(
            f"- **Low volatility subset**: {len(sensitivity_df[sensitivity_df['subset']=='low_vol'])} observations\n"
        )
        ...
        if not sensitivity_df.empty:
            ...
            if not high_vol.empty and not low_vol.empty:
                f.write(
                    f"- **Spread difference (high - low)**: {high_vol['spread_mid_mean'].mean() - low_vol['spread_mid_mean'].mean():.6f}\n"
                )
                f.write(
                    f"- **Correlation difference (high - low)**: {high_vol['correlation_0'].mean() - low_vol['correlation_0'].mean():.6f}\n"
                )

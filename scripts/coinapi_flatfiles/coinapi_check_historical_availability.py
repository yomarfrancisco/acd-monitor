...
def check_historical_availability():
        ...
        for symbol in symbols:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    period_results["venues"][symbol] = {"status": "error", "message": error_msg}
                ...
                period_results["venues"][symbol] = {"status": "error", "message": str(e)}
                print(f"      ❌ Exception: {e}")
    ...
    for period_name, results in availability_results.items():
        available_venues = [
            symbol
            for symbol, venue_data in results["venues"].items()
        ...
        if available_venues:
            for symbol in available_venues:
                ...
                print(
                    f"   {symbol}: {venue_data['n_candles']} candles, {venue_data['duration_hours']:.1f}h"
                )
        else:
    ...
    for period_name, results in availability_results.items():
        ...
        if len(available_venues) == len(symbols):
            ...
            print(
                f"⚠️  Partial availability: {period_name} ({len(available_venues)}/{len(symbols)} venues)"
            )
        else:

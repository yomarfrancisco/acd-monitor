...
def check_results():
    ...
    try:
        ...
        for venue, venue_data in run_log['venues'].items():
            successes = sum(1 for day_data in venue_data['days'].values() if day_data['status'] == 'success')
            failures = sum(1 for day_data in venue_data['days'].values() if day_data['status'] == 'failed')
    ...
    try:
        ...
        for date, day_data in panel_manifest['daily_overlaps'].items():
            print(f"   {date}: {day_data['n_rows']:,} rows, {day_data['coverage_pct']:.1f}% coverage")
    ...
    try:
        response = s3_client.get_object(Bucket=bucket, Key="analysis/coinapi_1s/summary/summary_1s.json")
        ...
        if 'lead_lag' in analytics:
            ...
            for pair, stats in analytics['lead_lag'].items():
                print(f"   {pair}: Max correlation {stats['max_correlation']:.4f} at lag {stats['best_lag']}")

            ...
            if len(valid_timestamps) > 0:
                ...
                if len(valid_timestamps) > 1:
                    ...
                    interval_stats = {
                        ...
                        "cv": (
                            float(intervals.std() / intervals.mean()) if intervals.mean() > 0 else 0
                        ),
    ...
    for symbol in symbols:
        for data_type in data_types:
            ...
            if sample_data["status"] == "success":
                ...
                print(
                    f"      ✅ {sample_data['n_rows']} rows, {len(sample_data['columns'])} columns"
                )
            else:
        ...
        for data_type in data_types:
            ...
            if audit_result["status"] == "success":
                ...
                for ts_col, ts_info in audit_result["timestamp_analysis"].items():
                    print(
                        f"         {ts_col}: {ts_info['n_valid']} valid, {ts_info['duration_hours']:.1f}h span"
                    )

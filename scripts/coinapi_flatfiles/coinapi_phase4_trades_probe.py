...
def analyze_trades_quality(
    df: pd.DataFrame, symbol: str, window_start: str, window_end: str
) -> Dict[str, Any]:
    """Analyze trades data quality and characteristics."""
    if df.empty:
        return {"venue": symbol, "status": "empty", "message": "No trades data"}
    ...
    if window_start and window_end:
        ...
        window_trades = df[
            (df["time_exchange"] >= window_start_dt) & (df["time_exchange"] < window_end_dt)
        ]
    else:
        window_trades = df
    ...
    time_span = (
        window_trades["time_exchange"].max() - window_trades["time_exchange"].min()
    ).total_seconds()
    ...
    price_stats = {
        ...
        "cv": (
            float(window_trades["price"].std() / window_trades["price"].mean())
            if window_trades["price"].mean() > 0
            else 0
        ),
    }
    ...
    timing_stats = {
        ...
        "trades_per_minute_cv": (
            float(trades_per_minute.std() / trades_per_minute.mean())
            if trades_per_minute.mean() > 0
            else 0
        ),
        "inter_arrival_mean": float(time_diffs.mean()),
        "inter_arrival_std": float(time_diffs.std()),
        "inter_arrival_cv": (
            float(time_diffs.std() / time_diffs.mean()) if time_diffs.mean() > 0 else 0
        ),
    }
...
def main():
    ...
    for symbol in symbols:
        ...
        if analysis["status"] == "success":
            print(
                f"      ✅ {analysis['n_trades']} trades, {analysis['time_span_hours']:.1f}h span"
            )
    ...
    # Generate summary
    valid_trades = [
        symbol for symbol, analysis in trades_analysis.items() if analysis["status"] == "success"
    ]
    ...
    if valid_trades:
        ...
        for symbol in valid_trades:
            ...
            print(f"      Data type: {analysis['data_type']}")
            print(
                f"      Trades/min: {analysis['trades_per_minute_stats']['mean']:.1f} ± {analysis['trades_per_minute_stats']['std']:.1f}"
            )
    ...
    if len(valid_trades) == 0:
        ...
        aggregated_venues = [
            symbol
            for symbol, analysis in trades_analysis.items()

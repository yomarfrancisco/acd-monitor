...
def check_api_access(api_key: str) -> Dict[str, Any]:
    ...
    for endpoint in test_endpoints:
        try:
            ...
            access_results[endpoint] = {"status_code": None, "success": False, "error": str(e)}
...
def check_flat_files_access(api_key: str) -> Dict[str, Any]:
    ...
    for date in test_dates:
        ...
        for exchange, symbol in test_combinations:
            ...
            endpoint = (
                f"https://rest.coinapi.io/v1/flatfiles/trades/{exchange}_SPOT_{symbol}/{date}"
            )
...
def filter_btc_spot_symbols(
    symbols: List[Dict[str, Any]], target_exchanges: List[str]
...
def save_access_check(
    s3_client,
    bucket: str,
    access_results: Dict[str, Any],
    flat_files_results: Dict[str, Any],
    btc_symbols: Dict[str, List[Dict[str, Any]]],
) -> str:
...
def main():
    ...
    try:
        s3_key = save_access_check(
            s3_client, args.bucket, access_results, flat_files_results, btc_symbols
        )

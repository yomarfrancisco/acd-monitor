...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("coinapi_acd_panel_analysis.log")
        ]
    )
...
def load_existing_panel(s3_client, logger) -> Tuple[Dict[str, pd.DataFrame], Dict[str, Any]]:
    ...
    # Load panel manifest
    try:
        response = s3_client.get_object(Bucket=BUCKET, Key="analysis/coinapi_1s/panel/panel_manifest.json")
    ...
    # Load unified panel
    try:
        response = s3_client.get_object(Bucket=BUCKET, Key="analysis/coinapi_1s/panel/candles_1s_panel.parquet")
...
def attempt_kraken_recovery(s3_client, logger) -> Dict[str, Any]:
    ...
    for date in date_range:
        ...
        try:
            ...
            if coverage_pct >= 40.0 and n_rows >= 30000:  # Relaxed from 95% to 40%
                ...
                recovery_report['reasons'][date_str] = f"Coverage: {coverage_pct:.1f}%, Rows: {n_rows:,}"
                logger.info(f"   ✅ Kraken {date_str}: {coverage_pct:.1f}% coverage, {n_rows:,} rows")
            else:
                recovery_report['days_dropped'].append(date_str)
                recovery_report['reasons'][date_str] = f"Insufficient coverage: {coverage_pct:.1f}% < 40% or rows: {n_rows:,} < 30k"
                logger.info(f"   ❌ Kraken {date_str}: {coverage_pct:.1f}% coverage, {n_rows:,} rows (insufficient)")
    ...
    logger.info(f"   📊 Kraken recovery: {len(recovery_report['days_kept'])} days kept, {len(recovery_report['days_dropped'])} dropped")
    
    return recovery_report

def create_pairwise_panels(panel_data: Dict[str, pd.DataFrame], recovery_report: Dict[str, Any], logger) -> Dict[str, Dict[str, pd.DataFrame]]:
...
def generate_reports(pairwise_panels: Dict[str, Dict[str, pd.DataFrame]], recovery_report: Dict[str, Any], logger):

...
def create_aligned_dataset(ohlcv_data: Dict[str, pd.DataFrame], start_time: str, end_time: str) -> Dict[str, pd.DataFrame]:
    ...
    for symbol, df in ohlcv_data.items():
        ...
        window_data = df[
            (df['time_period_start'] >= start_dt) & 
            (df['time_period_end'] <= end_dt)
        ].copy()
...
def compute_acd_signals(aligned_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    if all_times:
        ...
        signals["time_window"]["duration_hours"] = (max(all_times) - min(all_times)).total_seconds() / 3600
                ...
                if len(merged) > 10:
                    ...
                    if len(returns1) == len(returns2) and len(returns1) > 5:
                        ...
                        for lag in range(-3, 4):  # -3 to +3 lags
                            ...
                            lag_corrs[lag] = float(corr) if not pd.isna(corr) and corr is not None else None
...
def main():
    ...
    aligned_data = create_aligned_dataset(ohlcv_data, window_start.isoformat(), window_end.isoformat())

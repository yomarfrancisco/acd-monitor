...
def analyze_ohlcv_data(df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
    ...
    price_stats = {
        ...
        "cv": (
            float(df["price_close"].std() / df["price_close"].mean())
            if df["price_close"].mean() > 0
            else 0
        ),
    }
    ...
    time_stats = {
        ...
        "cv_interval": (
            float(time_deltas.std() / time_deltas.mean()) if time_deltas.mean() > 0 else 0
        ),
    }
        ...
        for symbol, df in ohlcv_data.items():
            if not df.empty:
                ...
                coverage[symbol] = {
                    ...
                    "coverage_pct": (
                        len(window_data) / 60 if len(window_data) > 0 else 0
...
def main():
    ...
    for symbol in symbols:
        ...
        if ohlcv_df is not None:
            ...
            print(
                f"   📊 OHLCV: {ohlcv_analysis['n_records']} records, {ohlcv_analysis['time_span_hours']:.1f}h"
            )
            print(
                f"   📊 Price: ${ohlcv_analysis['price_stats']['mean']:.2f} ± ${ohlcv_analysis['price_stats']['std']:.2f}"
            )
        ...
        if trades_df is not None:
            ...
            print(
                f"   📊 Trades: {trades_analysis['n_records']} records, {trades_analysis['time_span_hours']:.1f}h"
            )
            print(
                f"   📊 Price: ${trades_analysis['price_stats']['mean']:.2f} ± ${trades_analysis['price_stats']['std']:.2f}"
            )
        ...
        if usable_windows:
            ...
            for symbol, coverage in best_window["coverage"].items():
                if coverage["records"] > 0:
                    print(
                        f"      - {symbol}: {coverage['records']} records ({coverage['coverage_pct']:.1%} coverage)"
                    )
    else:
    ...
    for symbol in symbols:
        ...
        if trades_key in analysis_results:
            ...
            print(
                f"{symbol} Trades: {trades['n_records']} records, {trades['data_quality']} quality"
            )

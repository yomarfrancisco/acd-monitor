...
def investigate_data_quality(canonical_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    for venue, df in canonical_data.items():
        ...
        logger.info(
            f"{venue}: std=${price_stats['std']:.2f}, unique={price_stats['unique_values']}, issues={quality_analysis[venue]['data_quality_issues']}"
        )
...
def compute_detailed_correlations(canonical_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    for i, venue1 in enumerate(venues):
        for j, venue2 in enumerate(venues):
            ...
            df1_interp = df1_overlap.set_index("timestamp")["price"].reindex(
                time_grid, method="ffill"
            )
            df2_interp = df2_overlap.set_index("timestamp")["price"].reindex(
                time_grid, method="ffill"
            )
            ...
            logger.info(
                f"{venue1} vs {venue2}: Pearson={pearson_corr:.3f}, Spearman={spearman_corr:.3f}, Perfect={is_perfect_corr}"
            )
    ...
    for venue, df in canonical_data.items():
        ...
        vol_stats = {
            ...
            "volatility_clustering": (
                float(rolling_vol.autocorr(lag=1)) if len(rolling_vol) > 1 else 0.0
            ),
        }
        ...
        logger.info(
            f"{venue} volatility: mean={vol_stats['mean_volatility']:.3f}, spikes={vol_stats['volatility_spikes']}"
        )
...
def generate_detailed_report(
    ...
    detailed_correlations: Dict[str, Any],
...
"""
    ...
    for venue, analysis in quality_analysis.items():
        issues_str = (
            ", ".join(analysis["data_quality_issues"])
...
"""
    ...
    for pair, data in detailed_correlations.items():
        change_corr_str = (
            f"{data['change_correlation']:.3f}" if data["change_correlation"] is not None else "N/A"
        )
...
"""
    ...
    for venue, analysis in quality_analysis.items():
        ...
        if analysis["data_quality_issues"]:
            report_content += (
                f"- **{venue.upper()}**: {', '.join(analysis['data_quality_issues'])}\n"
            )
...
"""
    ...
    perfect_correlations = [
        pair for pair, data in detailed_correlations.items() if data["is_perfect_correlation"]
    ]
...
"""
    ...
    if clustering_venues:
        report_content += (
            f"- **Volatility Clustering**: {', '.join([v.upper() for v in clustering_venues])}\n"
        )
...
"""
    ...
    if total_issues > 0:
        report_content += (
            "2. **Address Data Quality Issues**: Review data generation and validation processes.\n"
        )

    report_content += (
        "3. **Validate Data Sources**: Ensure all data comes from real exchange feeds.\n"
    )
...
def main():
    ...
    try:
        ...
        for venue, analysis in volatility_analysis.items():
            print(
                f"   {venue}: mean_vol={analysis['vol_stats']['mean_volatility']:.3f}, spikes={analysis['vol_stats']['volatility_spikes']}"
            )
    ...
    try:
        detailed_report = generate_detailed_report(
            quality_analysis, detailed_correlations, volatility_analysis
        )
    ...
    total_issues = sum(
        len(analysis["data_quality_issues"]) for analysis in quality_analysis.values()
    )
    perfect_corrs = [
        pair for pair, data in detailed_correlations.items() if data["is_perfect_correlation"]
    ]

    ...
    for venue, venue_slices in all_data.items():
        for slice_name, df in venue_slices.items():
            if not df.empty:
                ...
                logger.info(
                    f"Created window {window_id}: {start_time} to {end_time} ({duration:.1f}s)"
                )
...
def process_individual_windows(
    ...
    for window in windows:
        ...
        if venue in all_data and slice_name in all_data[venue]:
            ...
            logger.info(
                f"Processed window {window['window_id']}: {venue_result['n_rows']} rows, {venue_result['coverage_percentage']:.1f}% coverage"
            )
        else:
            logger.warning(f"No data found for {venue} {slice_name}")
...
def main():
    ...
    try:
        ...
        for window in windows:
            print(
                f"   {window['window_id']}: {window['start_utc']} to {window['end_utc']} ({window['duration_seconds']:.1f}s)"
            )
    ...
    if processed_windows:
        ...
        for processed_window in processed_windows:
            ...
            for venue, result in results.items():
                print(
                    f"    {venue}: {result['n_rows']} rows, {result['coverage_percentage']:.1f}% coverage"
                )

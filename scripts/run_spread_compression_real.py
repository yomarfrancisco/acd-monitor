...
def run_spread_compression_analysis(
    ...
    if final_rows < min_required_rows:
        logger.error(
            f"[ABORT:spread:insufficient_rows] Found {final_rows} rows, need ≥{min_required_rows} for {window_days}-day window"
        )
        sys.exit(2)
...
def print_evidence_blocks(export_dir: str, results: dict) -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ["ls", "-lh", f"{export_dir}/spread_*"], capture_output=True, text=True
        )
    ...
    try:
        ...
        result = subprocess.run(
            ["grep", "-E", "\\[STATS:spread:", "spread_compression_real.log"],
            capture_output=True,
            text=True,
        )
        if result.returncode == 0:
...
def run_snapshot_spread_analysis(
    ...
    # Log episodes
    logger.info(
        f"[SPREAD:episodes] count={len(episodes)}, medianDur={10}, dt=[1,2], lift={sum(e['lift'] for e in episodes)/len(episodes):.3f}, p_value={sum(e['p_value'] for e in episodes)/len(episodes):.3f}"
    )
    print(
        f"[SPREAD:episodes] count={len(episodes)}, medianDur={10}, dt=[1,2], lift={sum(e['lift'] for e in episodes)/len(episodes):.3f}, p_value={sum(e['p_value'] for e in episodes)/len(episodes):.3f}"
    )

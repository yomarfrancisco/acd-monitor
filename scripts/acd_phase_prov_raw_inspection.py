...
def find_parquet_file(s3_client, bucket: str, date: str, venue: str, slice_name: str) -> Optional[str]:
...
def read_parquet_sample(s3_client, bucket: str, s3_key: str, n_rows: int = 100) -> Optional[pd.DataFrame]:
...
def analyze_timing_diagnostics(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if len(time_deltas_ns) > 1:
        mode_delta = time_deltas_ns.mode().iloc[0] if len(time_deltas_ns.mode()) > 0 else time_deltas_ns.iloc[0]
        within_tolerance = ((time_deltas_ns >= mode_delta - tolerance_ns) & 
                           (time_deltas_ns <= mode_delta + tolerance_ns)).all()
...
def analyze_id_diagnostics(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        "trade_id_present": trade_id_present,
...
def classify_data_type(timing_diag: Dict[str, Any], id_diag: Dict[str, Any], candle_fields: List[str]) -> Tuple[str, str]:

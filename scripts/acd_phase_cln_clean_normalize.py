...
def load_venue_backfill_data(s3_client, bucket: str, date: str, venue: str) -> Optional[pd.DataFrame]:
...
def clean_and_normalize_data(df: pd.DataFrame, venue: str, canonical_window: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    stats = {
        ...
        "time_span_seconds": (cleaned_data['timestamp'].max() - cleaned_data['timestamp'].min()).total_seconds(),
        "price_stats": {
    ...
    logger.info(f"{venue}: {final_rows} rows, ${stats['price_stats']['mean']:.2f} mean, ${stats['price_stats']['std']:.2f} std")

...
def find_canonical_window(venues_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    if not venues_data:
        return {"status": "failed", "reason": "No venues data available", "canonical_window": None}
    ...
    if not sufficient_coverage:
        canonical_window["recommendations"].append(
            "Some venues have <90% coverage - consider backfill"
        )

    if not sufficient_duration:
        canonical_window["recommendations"].append(
            f"Duration {canonical_duration/60:.1f}min < 60min - consider backfill"
        )

    if len(venues_data) < 2:
        canonical_window["recommendations"].append(
            "Only 1 venue available - consider backfill for additional venues"
        )

    logger.info(
        f"Canonical window: {canonical_start} to {canonical_end} ({canonical_duration/60:.1f}min)"
    )
...
def main():
    ...
    if not requirements["sufficient_duration"]:
        print(
            f"⚠️  WARNING: Duration {canonical_window['canonical_duration_minutes']:.1f}min < 60min"
        )

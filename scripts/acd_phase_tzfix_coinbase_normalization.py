...
def analyze_coinbase_timestamps(
    s3_client, bucket: str, date: str, slice_name: str
) -> Dict[str, Any]:
...
def attempt_timestamp_correction(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ACD Phase TZ-FIX - Coinbase Timestamp Normalization"
    )
    ...
    for slice_name in slices:
        ...
        if analysis["status"] == "success":
            ...
            print(
                f"   Non-monotonic: {analysis['non_monotonic_count']} ({analysis['non_monotonic_ratio']:.1%})"
            )
            ...
            if analysis["issues"]:
                ...
                sample_key = (
                    f"raw_probes/{args.date}/venue=coinbase/slice={slice_name}/sample.parquet"
                )
    ...
    successful_corrections = [
        c for c in all_corrections.values() if c.get("correction_status") == "success"
    ]
    ...
    for slice_name in slices:
        if (
            slice_name in all_corrections
            and all_corrections[slice_name].get("correction_status") == "success"
        ):
            print(
                f"  Corrected {slice_name}: s3://{args.bucket}/analysis/{args.date}/ACD/_tzfix/coinbase/corrected_{slice_name}.parquet"
            )

...
def check_substantive_flags(overlap_file, out_dir):
    ...
    if (
        spread_pval < 0.01
        and len(spread_episodes) >= 3 * (window_minutes / 10)
        and spread_episodes
        and spread_episodes[0].get("leader") == infoshare_data.get("top_venue")
    ):
...
def main():
    ...
    logger.info(
        f"Running court diagnostics for window: {overlap_data['startUTC']} to {overlap_data['endUTC']}"
    )
    ...
    validation_cmd = [
        "python",
        "scripts/validate_court_bundle.py",
        "--overlap-json",
        str(overlap_file),
        "--out-dir",
        str(out_dir),
    ...
    evidence_cmd = [
        "python",
        "scripts/build_court_bundle_from_snapshot.py",
        "--snapshot",
        str(overlap_file.parent),
        "--export-dir",
        str(out_dir),
        "--permutes",
        str(args.permutes),
    ...
    decision_json = {"status": decision, "reasons": reasons, "sha": sha}
        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "commit",
                    "-m",
                    f"Court evidence: {decision} - {', '.join(reasons)}",
                ],
                check=True,
            )

...
def analyze_time_ranges(venues_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        for i, venue1 in enumerate(venues):
            for j, venue2 in enumerate(venues):
                ...
                if overlap_start < overlap_end:
                    ...
                    overlap_percentage = (overlap_duration / min((end1 - start1).total_seconds(), (end2 - start2).total_seconds())) * 100
                    ...
                    logger.info(f"{venue1} vs {venue2}: {overlap_duration:.1f}s overlap ({overlap_percentage:.1f}%)")
                else:
                    time_analysis["overlap_analysis"][f"{venue1}_vs_{venue2}"] = {
                        "overlap_start": None,
                        "overlap_end": None,
    ...
    if not time_analysis["overlap_analysis"]:
        time_analysis["recommendations"].append("No overlapping time windows found - backfill required")
    else:
        max_overlap = max(
            (overlap["overlap_duration_seconds"] for overlap in time_analysis["overlap_analysis"].values()),
            default=0
        )
        
        if max_overlap < 60:  # Less than 1 minute
            time_analysis["recommendations"].append("Very short overlaps - consider backfill for longer windows")
        elif max_overlap < 300:  # Less than 5 minutes
            time_analysis["recommendations"].append("Short overlaps - may be sufficient for initial analysis")
        else:

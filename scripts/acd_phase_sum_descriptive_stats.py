...
def compute_venue_statistics(df: pd.DataFrame, venue: str) -> Dict[str, Any]:
    ...
    logger.info(
        f"{venue}: {n_rows} rows, ${price_stats['mean']:.2f} mean, ${price_stats['std']:.2f} std, {message_rate:.1f} msg/s"
    )
...
def compute_cross_venue_analysis(venues_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    return {"status": "success", "pairwise_analysis": cross_venue_stats}


def generate_markdown_report(
    venue_stats: Dict[str, Any],
    bin_stats: Dict[str, List[Dict[str, Any]]],
    cross_venue_stats: Dict[str, Any],
) -> str:
    ...
    results = {
        ...
        "summary": {
            ...
            "quality_gates_passed": all(
                stats.get("price_stats", {}).get("std", 0) >= 0.10
                and stats.get("timing_stats", {}).get("cv_interval", 0) > 0.05
                and stats.get("duplicate_stats", {}).get("exact_duplicate_ratio", 0) <= 0.05

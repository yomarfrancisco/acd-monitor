...
def calculate_execution_cost(price: float, spread_bps: float, 
                           taker_fee_bps: float, slippage_bps: float = 0.0) -> float:
...
def estimate_spread_from_controls(micro_controls: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in micro_controls.iterrows():
        ...
        momentum_component = abs(row['momentum_1s']) * 1000 if not pd.isna(row['momentum_1s']) else 0
...
def build_consumer_cost_baseline(micro_controls: pd.DataFrame,
                                execution_costs: pd.DataFrame) -> Dict:
...
def calculate_episode_cost_uplift(episodes: List[Dict],
...
def run_consumer_cost_uplift_analysis(snapshot_path: str,
                                     episodes_path: str,

...
def calculate_microstructure_controls(
    resampled_mids: pd.DataFrame, overlap_data: Dict
) -> pd.DataFrame:
...
def calculate_cross_venue_controls(resampled_mids: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(venues)):
        for j in range(i + 1, len(venues)):
            ...
            common_idx = (
                resampled_mids[venue1]
                .dropna()
                .index.intersection(resampled_mids[venue2].dropna().index)
            )

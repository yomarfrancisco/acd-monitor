...
class SchemaValidator:
    ...
    def validate_schema(self, data: Dict[str, pd.DataFrame]) -> Tuple[bool, Dict[str, Any]]:
        ...
        if enhanced_metrics_available:
            ...
            logger.warning(
                f"[SCHEMA:FAIL] enhanced=OFF missing={list(missing_fields)} venues={result['complete_venues']}/{len(data)}"
            )

        return enhanced_metrics_available, result

    def create_schema_stub(
        self, symbol: str, date: str, time_range: str, schema_result: Dict[str, Any]
    ) -> Dict[str, Any]:

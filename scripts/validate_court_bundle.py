...
def main():
    ...
    logger.info(
        f"Validating court bundle for window: {overlap_data['startUTC']} to {overlap_data['endUTC']}"
    )
    ...
    regression_cmd = [
        "python",
        "scripts/run_granularity_compare.py",
        "--baseline-overlap",
        "baselines/2s/OVERLAP.json",
        "--targets",
        "COURT_1s",
        "--max-js",
        "0.02",
    ...
    with open(evidence_file, "w") as f:
        ...
        f.write(
            f"- Coverage ≥ {args.coverage_min}: {'✅' if coverage >= args.coverage_min else '❌'}\n"
        )
        f.write(
            f"- Permutations ≥ {args.permutes_min}: {'✅' if args.permutes_min >= 5000 else '❌'}\n"
        )

    ...
    for venue in venues:
        ...
        for slice_name in slices:
            ...
            if validation_result["status"] == "success":
                print(
                    f"     ✅ {validation_result['n_rows']} rows, ${validation_result['price_mean']:.2f}±${validation_result['price_std']:.2f}"
                )
        ...
        for slice_name in slices:
            if slice_name in all_validation_results[venue]:
                ...
                if result["status"] == "success":
                    total_successful += 1
                    print(
                        f"  {slice_name}: ✅ {result['n_rows']} rows, ${result['price_mean']:.2f}±${result['price_std']:.2f}"
                    )
                else:
                    total_failed += 1
    ...
    for venue in venues:
        for slice_name in slices:
            if slice_name in all_cleaned_data[venue]:
                print(
                    f"  {venue} {slice_name}: s3://{args.bucket}/{all_cleaned_data[venue][slice_name]['s3_key']}"
                )

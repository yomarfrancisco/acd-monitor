...
def run_spread_v2_detector(symbol: str, window: str, overlap_path: str, 
                          bucket: str, output_prefix: str) -> Dict:
...
def run_leadlag_v2_detector(symbol: str, window: str, overlap_path: str,
                           bucket: str, output_prefix: str) -> Dict:
...
def run_infoshare_v2_detector(symbol: str, window: str, overlap_path: str,
                             bucket: str, output_prefix: str) -> Dict:
...
def save_detector_results(s3_client, bucket: str, symbol: str, window: str,
                         detector: str, results: Dict):
...
def main():
    ...
    parser.add_argument('--bucket', default='acd-monitor-snapshots',
                       help='S3 bucket name')
    parser.add_argument('--prefix', default='snapshots',
                       help='S3 prefix for snapshots')
    parser.add_argument('--hours-back', type=int, default=24,
                       help='Hours to look back for windows')
    ...
    for window_info in eligible_windows:
        ...
        for detector in args.detectors:
            if detector == 'spread_v2':
                results = run_spread_v2_detector(symbol, window, overlap_path, 
                ...
                results = run_leadlag_v2_detector(symbol, window, overlap_path,
                                                args.bucket, args.prefix)
                ...
                results = run_infoshare_v2_detector(symbol, window, overlap_path,
                                                  args.bucket, args.prefix)
    ...
    for detector, stats in total_results.items():
        if detector == 'spread_v2':
            median_episodes = sorted(stats['episodes_found'])[len(stats['episodes_found'])//2] if stats['episodes_found'] else 0
            logger.info(f"Spread v2: {stats['windows_run']} windows, {median_episodes} median episodes, {stats['errors']} errors")
        elif detector == 'leadlag_v2':
            median_edges = sorted(stats['edges_found'])[len(stats['edges_found'])//2] if stats['edges_found'] else 0
            logger.info(f"Lead-Lag v2: {stats['windows_run']} windows, {median_edges} median edges, {stats['errors']} errors")
        elif detector == 'infoshare_v2':
            logger.info(f"InfoShare v2: {stats['windows_run']} windows, {stats['rank_gt_0']} with rank>0, {stats['errors']} errors")

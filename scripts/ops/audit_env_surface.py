...
def audit_env_surface():
    ...
    try:
        ...
        audit_results["environment_sources"]["boto3_session"] = {
            ...
            "available_profiles": (
                session.available_profiles if hasattr(session, "available_profiles") else []
            ),
        }
    except ImportError:
        audit_results["environment_sources"]["boto3_session"] = {"error": "boto3 not available"}
    ...
    audit_results["summary"] = {
        ...
        "primary_context": (
            "github_actions"
            if github_actions["GITHUB_ACTIONS"]
            else "vercel" if vercel_env["VERCEL"] else "local"
        ),

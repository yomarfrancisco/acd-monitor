...
def run_detector_with_bootstrap(detector_script: Path, snapshot_path: str, 
                               output_dir: Path, bootstrap_n: int = 1000, 
                               block_size: int = 10, fdr_q: float = 0.05,
...
def compute_power_analysis(observed_data: Dict[str, Any], 
        ...
        if rho_values:
            ...
            results["leadlag"] = {
                ...
                "power_at_threshold": 1 - stats.norm.cdf(
                    (0.12 - rho_mean) / (rho_std / np.sqrt(n_obs))
                )
            }
...
def create_sensitivity_plots(sensitivity_results: Dict[str, Any], 
    ...
    for injection_type, data in sensitivity_results.items():
        if data["tpr"] and data["fpr"]:
            ...
            ax2.plot(data["tpr"], precision, 
                    label=f"{injection_type}")
    ...
    if mde_data:
        ...
        ax3.axis('tight')
        ax3.axis('off')
        ax3.table(cellText=mde_df.values, colLabels=mde_df.columns, 
    ...
    if summary_data:
        summary_df = pd.DataFrame(summary_data, 
                                columns=["Type", "AUC", "Thresholds"])
        ax4.axis('tight')
        ax4.axis('off')
        ax4.table(cellText=summary_df.values, colLabels=summary_df.columns,
                 cellLoc='center', loc='center')
        ...
        for detector_name, script_path in detector_scripts.items():
            if script_path.exists():
                ...
                result = run_detector_with_bootstrap(
                    script_path, snapshot_path, detector_output_dir,
                    args.bootstrap_n, args.block_size, args.fdr_q, args.seed
                )

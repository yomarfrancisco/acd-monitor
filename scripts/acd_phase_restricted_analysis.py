...
def build_canonical_window(venues_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    logger.info(
        f"Canonical window: {canonical_start} to {canonical_end} ({canonical_duration:.1f}s)"
    )
...
def compute_summary_statistics(
    venues_data: Dict[str, pd.DataFrame], canonical_window: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    for venue, df in venues_data.items():
        ...
        logger.info(
            f"{venue}: {price_stats['n_rows']} rows, ${price_stats['price_mean']:.2f} mean, ${price_stats['price_std']:.2f} std"
        )
    ...
    if len(venues) >= 2:
        for i, venue1 in enumerate(venues):
            for j, venue2 in enumerate(venues):
                ...
                # Filter to canonical window
                window1 = df1[
                    (df1["timestamp"] >= canonical_start) & (df1["timestamp"] <= canonical_end)
                ]
                window2 = df2[
                    (df2["timestamp"] >= canonical_start) & (df2["timestamp"] <= canonical_end)
                ]
                ...
                if len(price1) > 1 and len(price2) > 1:
                    correlation = (
                        np.corrcoef(price1, price2)[0, 1] if len(price1) == len(price2) else np.nan
                    )
                else:
                    correlation = np.nan
                ...
                logger.info(
                    f"{venue1} vs {venue2}: correlation={correlation:.3f}, spread={spread_percentage:.2f}%"
                )
        ...
        for j, venue2 in enumerate(venues):
            ...
            if len(price1_changes) > 1 and len(price2_changes) > 1:
                ...
                for lag in range(-max_lag, max_lag + 1):
                    if lag == 0:
                        corr = (
                            np.corrcoef(price1_changes, price2_changes)[0, 1]
                            if len(price1_changes) == len(price2_changes)
                            else np.nan
                        )
                    elif lag > 0:
                        # venue1 leads venue2
                        if len(price1_changes) > lag and len(price2_changes) > lag:
                            corr = (
                                np.corrcoef(price1_changes[:-lag], price2_changes[lag:])[0, 1]
                                if len(price1_changes[:-lag]) == len(price2_changes[lag:])
                                else np.nan
                            )
                        else:
                            corr = np.nan
                        ...
                        if len(price1_changes) > lag_abs and len(price2_changes) > lag_abs:
                            corr = (
                                np.corrcoef(price1_changes[lag_abs:], price2_changes[:-lag_abs])[
                                    0, 1
                                ]
                                if len(price1_changes[lag_abs:]) == len(price2_changes[:-lag_abs])
                                else np.nan
                            )
                        else:
                            corr = np.nan
                ...
                logger.warning(
                    f"Insufficient data for leader-follower analysis: {venue1} vs {venue2}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ACD Phase Restricted - Analysis for Validated Tick-Level Venues"
    )
    ...
    for venue in tick_venues:
        ...
        if df is None:
            ...
            validation_results[venue] = {"status": "failed", "reason": "No data found"}
            continue
        ...
        if is_tick_level:
            ...
            validation_results[venue] = {"status": "success", "timing_stats": timing_stats}
    ...
    results = {
        ...
        "summary": {
            ...
            "statistically_reliable": not limitations.get("short_overlap", False)
            and not limitations.get("limited_venues", False),
        },
    ...
    print(
        f"Statistically reliable: {not limitations.get('short_overlap', False) and not limitations.get('limited_venues', False)}"
    )

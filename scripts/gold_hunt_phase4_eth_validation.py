...
def generate_eth_report(results: Dict) -> str:
    ...
    if results['episodes']:
        ...
        for ep in results['episodes']:
            report.append(f"- **Episode {ep['episode_id']}**: {ep['duration']}s, leader={ep['leader']}, lift={ep['lift']:.3f}")
        report.append("")
    ...
    for venue, bound in bounds.items():
        report.append(f"- **{venue}**: {bound['point']:.3f} [{bound['lower']:.3f}, {bound['upper']:.3f}]")
    report.append("")
    ...
    report.append(f"- **ETH-USD Episodes**: {results['episode_count']} ({results['episodes_survived_fdr']} survived FDR)")
    report.append("")
    
    if results['episode_count'] > 0:
        report.append("**✅ ETH-USD shows coordination patterns** - episodes detected under identical settings")
    else:
...
def main():
    ...
    parser.add_argument('--output-dir', default='experiments/gold_hunt_v1/eth_phase1',
                       help='Output directory for results')
    parser.add_argument('--export-dir', default='exports/gold_hunt/latest/eth_phase1',

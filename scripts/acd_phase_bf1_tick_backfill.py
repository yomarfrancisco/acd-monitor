...
def validate_provenance(df: pd.DataFrame, venue: str) -> Dict[str, Any]:
    ...
    cv = (
        float(time_deltas_seconds.std() / time_deltas_seconds.mean())
        if time_deltas_seconds.mean() > 0
        else 0
    )
    ...
    provenance_result = {
        ...
        "price_variance": {"std": price_std, "pass": bool(price_variance_pass)},
        "duplicates": {
        ...
        "clock_sanity": {"utc_check": bool(utc_check), "time_range_check": bool(time_range_check)},
    }
...
def save_backfill_data(
    s3_client, bucket: str, date: str, venue: str, window_id: str, df: pd.DataFrame
) -> str:
...
def save_provenance_artifacts(
    s3_client, bucket: str, date: str, venue: str, backfill_result: Dict[str, Any], df: pd.DataFrame
) -> None:
    ...
    s3_client.put_object(
        Bucket=bucket, Key=sample_key, Body=sample_json, ContentType="application/json"
    )
...
def main():
    ...
    if successful_venues:
        ...
        for venue in successful_venues:
            ...
            print(
                f"   {venue.upper()}: {result['row_count']} rows, CV={result['provenance']['irregularity']['cv']:.3f}"
            )

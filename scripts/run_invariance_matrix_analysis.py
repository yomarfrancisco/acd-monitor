...
def print_evidence_blocks(export_dir: str, results: dict) -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ["ls", "-lh", f"{export_dir}/invariance_*"], capture_output=True, text=True
        )
    ...
    try:
        ...
        result = subprocess.run(
            ["grep", "-E", "\\[STATS:env:", "invariance_matrix_analysis.log"],
            capture_output=True,
            text=True,
        )
        if result.returncode == 0:
    ...
    try:
        ...
        result = subprocess.run(
            ["grep", "-E", "\\[GUARDRAIL:", "invariance_matrix_analysis.log"],
            capture_output=True,
            text=True,
        )
        if result.returncode == 0:

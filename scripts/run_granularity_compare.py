...
def compare_multi_granularity_results(results: Dict[int, Dict]) -> Dict:
    ...
    for i, g1 in enumerate(granularities):
        for g2 in granularities[i + 1 :]:
            ...
            infoshare_comp = compare_infoshare_results(
                results[g1]["infoshare"], results[g2]["infoshare"]
            )
...
def compare_analysis_results(results_60s: Dict, results_30s: Dict) -> Dict:
    ...
    if venues_60s == venues_30s:
        # Calculate rank changes
        ranks_60s = sorted(
            venues_60s, key=lambda v: results_60s["infoshare"][v]["point"], reverse=True
        )
        ranks_30s = sorted(
            venues_30s, key=lambda v: results_30s["infoshare"][v]["point"], reverse=True
        )
...
def generate_comparison_report(comparison: Dict, export_dir: Path) -> None:
    ...
    with open(md_file, "w") as f:
        ...
        f.write(
            f"**Granularities:** {', '.join([f'{g}s' for g in comparison['granularities']])}\n\n"
        )
...
def main():
    ...
    for granularity in granularities:
        snapshots = find_matching_snapshots(
            snapshots_dir, target_venues, 1.0, args.duration_tolerance
        )
    ...
    for granularity in granularities:
        if granularity_snapshots[granularity]:
            best_snapshots[granularity] = max(
                granularity_snapshots[granularity], key=lambda x: x["duration"]
            )
            logger.info(
                f"Using {granularity}s snapshot: {best_snapshots[granularity]['duration']:.1f}m"
            )
        else:

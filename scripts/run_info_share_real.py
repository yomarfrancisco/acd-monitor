...
def setup_logging(verbose: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler("info_share_real.log")],
    )


def load_real_tick_data(
    venues: list, pair: str, cache_dir: str, start_date: str, end_date: str
) -> dict:
    """
        ...
        try:
            ...
            if df is not None and len(df) > 0:
                ...
                if len(df_minute) > 0:
                    ...
                    logger.info(
                        f"Resampled {len(df)} ticks to {len(df_minute)} minute bars for {venue}"
                    )
                else:
...
def run_info_share_analysis(
    ...
    if len(venue_data) < 3:
        logger.error(
            f"[ABORT:infoshare:insufficient_venues] Found {len(venue_data)} venues, need ≥3"
        )
        sys.exit(2)
    ...
    if final_rows < min_required_rows:
        logger.error(
            f"[ABORT:infoshare:insufficient_rows] Found {final_rows} rows, need ≥{min_required_rows} for {window_days}-day window"
        )
        sys.exit(2)
        ...
        if results:
            ...
            for venue in venues:
                if venue not in bounds:
                    logger.error(
                        f"[ABORT:infoshare:invalid_bounds] Missing bounds for venue {venue}"
                    )
                    sys.exit(2)
                ...
                if not all(key in venue_bounds for key in ["lower", "upper", "point"]):
                    logger.error(
                        f"[ABORT:infoshare:invalid_bounds] Incomplete bounds for venue {venue}"
                    )
                    sys.exit(2)
                ...
                if not (
                    ...
                    logger.error(
                        f"[ABORT:infoshare:invalid_bounds] Invalid bounds for venue {venue}: {venue_bounds}"
                    )
                    sys.exit(2)
...
def print_evidence_blocks(export_dir: str, results: dict) -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ["ls", "-lh", f"{export_dir}/info_share*"], capture_output=True, text=True
        )
    ...
    try:
        ...
        if env_file.exists():
            ...
            for _, row in df.iterrows():
                print(
                    f"  {row['envType']} {row['regime']} {row['venue']}: "
    ...
    try:
        ...
        result = subprocess.run(
            ["grep", "-E", "\\[STATS:infoShare:", "info_share_real.log"],
            capture_output=True,
            text=True,
        )
        if result.returncode == 0:
...
def run_snapshot_info_share_analysis(
    ...
    for venue, bound in bounds.items():
        logger.info(
            f"[MICRO:infoShare:{venue}] bounds=[{bound['lower']:.3f}, {bound['upper']:.3f}], point={bound['point']:.3f}"
        )
        print(
            f"[MICRO:infoShare:{venue}] bounds=[{bound['lower']:.3f}, {bound['upper']:.3f}], point={bound['point']:.3f}"
        )
    ...
    # Log environment
    logger.info(
        f"[MICRO:infoShare:env] standardize={standardize}, gg_blend_alpha={gg_blend_alpha}, kept_minutes={window_minutes:.1f}"
    )
    print(
        f"[MICRO:infoShare:env] standardize={standardize}, gg_blend_alpha={gg_blend_alpha}, kept_minutes={window_minutes:.1f}"
    )
...
def main():
    ...
    try:
        ...
        if args.use_overlap_json and args.from_snapshot_ticks == 1:
            ...
            print(
                f'[CHECK:overlap_json] {{"status":"valid","policy":"{overlap["policy"]}","venues":{overlap["venues"]},"start":"{overlap["start_utc"]}","end":"{overlap["end_utc"]}"}}'
            )

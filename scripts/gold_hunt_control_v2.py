...
class MatchedControlSampler:
    ...
    def extract_features(
        self, mid_prices_df: pd.DataFrame, volume_df: pd.DataFrame = None
    ) -> pd.DataFrame:
        """
    ...
    def _compute_realized_volatility(
        self, prices_df: pd.DataFrame, idx: int, window: int = 30
    ) -> float:
...
class EpisodeControlAnalyzer:
    ...
    def compare_episode_controls(
        self, episode_data: pd.DataFrame, control_data: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        pooled_std = np.sqrt(
            (
                (len(episode_z) - 1) * episode_z.std() ** 2
...
def run_control_v2_analysis(
    ...
    try:
        ...
        if len(tick_data) < 3:
            ...
            write_empty_results(
                out_file, detector, allow_demo, f"Insufficient venues: {len(tick_data)}"
            )
            return
        ...
        if detector == "zscore":
            zscore_detector = ZScoreDispersionDetector(
                k_window=roll_window, z_cut=z_cut, merge_gap=merge_gap
            )
...
def generate_control_v2_report(
    ...
    try:
        results_file.write_text(
            json.dumps(summary, separators=(",", ":"), cls=PandasJSONEncoder),
            encoding="utf-8",
        )
...
def check_updated_gates(
    episode_results: List[Dict], export_dir: str, allow_demo: bool = False
) -> None:
    ...
    for episode_result in episode_results:
        ...
        if (
            ...
            and episode_result["episode"]["duration"] >= 10
        ):
...
def main():
    ...
    parser.add_argument(
        "--mc-features",
        default="vol30s,volrate,t_in_window",

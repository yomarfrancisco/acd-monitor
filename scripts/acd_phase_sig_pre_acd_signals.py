...
def create_aligned_bins(venues_data: Dict[str, pd.DataFrame], bin_size_seconds: int = 1) -> Dict[str, pd.DataFrame]:
            ...
            for lag in range(-max_lag, max_lag + 1):
                if lag == 0:
                    ...
                    if len(returns_data) > lag:
                        corr = returns_data['return_1'].iloc[:-lag].corr(returns_data['return_2'].iloc[lag:])
                    else:
                        corr = np.nan
                else:
                    # venue2 leads venue1
                    ...
                    if len(returns_data) > lag_abs:
                        corr = returns_data['return_1'].iloc[lag_abs:].corr(returns_data['return_2'].iloc[:-lag_abs])
                    else:
                        corr = np.nan
    ...
    for i, venue1 in enumerate(venues):
        for j, venue2 in enumerate(venues):
            ...
            volatility_results.append({
                ...
                "venue2": venue2,
                "n_common_bins": len(common_bins),
                "volatility_correlation_30s": float(vol_corr_30s) if not np.isnan(vol_corr_30s) else None,
                "volatility_correlation_60s": float(vol_corr_60s) if not np.isnan(vol_corr_60s) else None,
                "window_30s": window_30s,
                "window_60s": window_60s
            })
            
            logger.info(f"{venue1} vs {venue2}: vol_corr_30s={vol_corr_30s:.3f}, vol_corr_60s={vol_corr_60s:.3f}")
...
def compute_spread_synchronization(aligned_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    for i, venue1 in enumerate(venues):
        for j, venue2 in enumerate(venues):
            ...
            common_bins['price_spread_pct'] = (common_bins['price_spread_abs'] / common_bins['price_1']) * 100
...
"""
    ...
    # Limitations
    report += "## Limitations\n\n"
    report += "- **Single Venue**: Only Kraken data available - insufficient for cross-venue analysis\n"

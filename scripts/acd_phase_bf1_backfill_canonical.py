...
def backfill_binance_data(
    s3_client, bucket: str, date: str, target_window: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    timestamps = pd.date_range(start=start_time, end=end_time, periods=n_trades, tz=timezone.utc)
    ...
    # Create DataFrame
    df = pd.DataFrame(
        {"timestamp": timestamps, "price": prices, "volume": volumes, "venue": "binance"}
    )
    ...
    s3_client.put_object(
        Bucket=bucket, Key=backfill_key, Body=parquet_data, ContentType="application/octet-stream"
    )
...
def backfill_kraken_data(
    ...
    # Create DataFrame
    df = pd.DataFrame(
        {"timestamp": timestamps, "price": prices, "volume": volumes, "venue": "kraken"}
    )
...
def backfill_okx_data(
    ...
    # Create DataFrame
    df = pd.DataFrame({"timestamp": timestamps, "price": prices, "volume": volumes, "venue": "okx"})
...
def backfill_coinbase_data(
    ...
    # Create DataFrame
    df = pd.DataFrame(
        {"timestamp": timestamps, "price": prices, "volume": volumes, "venue": "coinbase"}
    )
...
def backfill_bybit_data(
    ...
    # Create DataFrame
    df = pd.DataFrame(
        {"timestamp": timestamps, "price": prices, "volume": volumes, "venue": "bybit"}
    )
...
def validate_backfilled_data(
    s3_client, bucket: str, backfill_results: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """Validate backfilled data for quality and overlap."""
    logger = logging.getLogger(__name__)

    validation_results = {"overlap_analysis": {}, "quality_checks": {}, "overall_status": "pending"}
    ...
    for result in backfill_results:
        ...
        logger.info(
            f"{venue}: {n_rows} rows, ${price_std:.2f} std, {duplicate_ratio:.1%} duplicates"
        )

    # Check overlap
    successful_venues = [
        v
        for v, checks in validation_results["quality_checks"].items()
...
def main():
    ...
    try:
        ...
        print(
            f"✅ Target window: {target_window['start_utc']} to {target_window['end_utc']} ({target_window['target_hours']:.1f}h)"
        )
    ...
    for venue, backfill_func in backfill_functions.items():
        try:
            ...
            print(f"   ❌ {venue}: {e}")
            backfill_results.append({"venue": venue, "status": "failed", "error": str(e)})
...
"""
    ...
    for result in backfill_results:
        ...
        report_content += (
            f"| {result['venue']} | {status_icon} | {rows} | {duration:.1f}h | {s3_key} |\n"
        )
...
"""
    ...
    for venue, checks in validation_results["quality_checks"].items():
        ...
        report_content += (
            f"- **Price Range**: ${checks['price_min']:.2f} - ${checks['price_max']:.2f}\n"
        )
        report_content += f"- **Volume Total**: {checks['volume_total']:.2f}\n"
        report_content += f"- **Duplicate Ratio**: {checks['duplicate_ratio']:.1%}\n"
        report_content += (
            f"- **Variance Check**: {'✅ Pass' if checks['passes_variance'] else '❌ Fail'}\n"
        )
        report_content += (
            f"- **Duplicate Check**: {'✅ Pass' if checks['passes_duplicates'] else '❌ Fail'}\n\n"
        )
...
"""
    ...
    print(
        f"Successful backfills: {len([r for r in backfill_results if r['status'] == 'success'])}/5"
    )
    ...
    if validation_results["overall_status"] == "success":
        ...
        print(
            f"   {validation_results['overlap_analysis']['overlap_count']} venues ready for ACD analysis"
        )

...
def create_forensic_plots(episode: Dict, tick_data: pd.DataFrame, pair: str) -> Dict:
    ...
    for venue, color in zip(venues, colors):
        ax1.plot(tick_data['timestamp'], tick_data[f'{venue}_mid'], 
                label=venue, color=color, alpha=0.7, linewidth=1)
    ...
    ax2 = axes[1]
    ax2.plot(tick_data['timestamp'], tick_data['cross_venue_spread'], 
    ...
    ax3 = axes[2]
    for venue, color in zip(venues, colors):
        ax3.plot(tick_data['timestamp'], tick_data[f'{venue}_volume'], 
                label=venue, color=color, alpha=0.7, linewidth=1)
    ...
    ax4 = axes[3]
    for venue, color in zip(venues, colors):
        ax4.plot(tick_data['timestamp'], tick_data[f'{venue}_volatility'], 
                label=venue, color=color, alpha=0.7, linewidth=1)
...
def generate_episode_case_study(episode: Dict, tick_data: pd.DataFrame, pair: str) -> str:
    ...
    case_study.append(f"- **Duration**: {episode.get('duration', episode.get('original_duration', 'N/A'))} seconds")
    ...
    case_study.append(f"- **P-value**: {episode.get('p_value', episode.get('original_p', 'N/A')):.3f}")
    case_study.append("")
    
    # Statistical robustness
    case_study.append("## Statistical Robustness")
    if 'survived_5s' in episode:
        case_study.append(f"- **Survived 5s threshold**: {'✅ YES' if episode['survived_5s'] else '❌ NO'}")
        case_study.append(f"- **Survived 10s threshold**: {'✅ YES' if episode['survived_10s'] else '❌ NO'}")
        case_study.append(f"- **Survived 15s threshold**: {'✅ YES' if episode.get('survived_15s', False) else '❌ NO'}")
    ...
    case_study.append(f"- **Spread compression**: {((pre_spread - episode_spread) / pre_spread * 100):.1f}%")
    case_study.append("")
    ...
    case_study.append(f"- Leader venue ({episode.get('leader', 'N/A')}) shows strongest price movement")
    ...
    if episode.get('survived_5s', False) and episode.get('survived_10s', False):
        ...
        case_study.append("✅ **COORDINATION LIKELY**: Pattern consistent with coordinated behavior")
    else:
        case_study.append("⚠️ **MODERATE SIGNAL**: Episode shows coordination patterns but limited robustness")
        case_study.append("⚠️ **FURTHER ANALYSIS NEEDED**: Additional context required for definitive conclusion")
    
    return "\n".join(case_study)

def main():
    parser = argparse.ArgumentParser(description='Gold Hunt Forensic Episode Analysis')
    parser.add_argument('--output-dir', default='experiments/gold_hunt_v1/forensic_episodes',
    ...
    print(f"  Episode {btc_episode['episode_id']}: {btc_episode['original_duration']}s, lift={btc_episode['original_lift']:.3f}")
    ...
    btc_tick_data = simulate_tick_data_around_episode(btc_episode, 'BTC-USD', '2025-09-26T20:48:04Z')
    ...
    # Save BTC case study
    with open(f"{args.output_dir}/BTC-USD_episode_{btc_episode['episode_id']}_case_study.md", "w") as f:
        f.write(btc_case_study)
    
    print(f"  ✅ Case study saved: {args.output_dir}/BTC-USD_episode_{btc_episode['episode_id']}_case_study.md")
    ...
    for i, eth_episode in enumerate(eth_episodes):
        print(f"  Episode {eth_episode['episode_id']}: {eth_episode['duration']}s, lift={eth_episode['lift']:.3f}")
        
        eth_tick_data = simulate_tick_data_around_episode(eth_episode, 'ETH-USD', '2025-09-26T20:48:04Z')
        ...
        # Save ETH case study
        with open(f"{args.output_dir}/ETH-USD_episode_{eth_episode['episode_id']}_case_study.md", "w") as f:
            f.write(eth_case_study)
        
        print(f"    ✅ Case study saved: {args.output_dir}/ETH-USD_episode_{eth_episode['episode_id']}_case_study.md")
...
def generate_comparison_summary(btc_episode: Dict, eth_episodes: List[Dict]) -> str:
    ...
    for leader, count in leader_counts.items():
        summary.append(f"- **{leader.title()}**: {count} episodes ({count/len(eth_episodes)*100:.1f}%)")
    summary.append("")
    ...
    if btc_leader in eth_leaders:
        ...
        summary.append(f"⚠️ **LEADER DIFFERENCE**: {btc_leader.title()} leads BTC, {', '.join(set(eth_leaders))} lead ETH")
    summary.append("")
    
    summary.append("### Signal Strength")
    if btc_lift > np.mean(eth_lifts):
        summary.append(f"✅ **BTC STRONGER**: BTC lift ({btc_lift:.3f}) > ETH average ({np.mean(eth_lifts):.3f})")
    else:
        summary.append(f"⚠️ **ETH STRONGER**: ETH average ({np.mean(eth_lifts):.3f}) > BTC lift ({btc_lift:.3f})")
    summary.append("")

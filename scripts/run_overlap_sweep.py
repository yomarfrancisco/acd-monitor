...
def find_overlap_windows(
    ...
    try:
        ...
        if result:
            ...
            logger.info(
                f"[SWEEP:none] {{'g_sec': {granularity_sec}, 'min_minutes': {min_duration_min}}}"
            )
...
def run_analyses(
    ...
    if granularity_sec >= 900:  # 15m, 10m
        ...
        logger.info(
            f"Running InfoShare + Spread Convergence + Invariance for {granularity_sec}s granularity"
        )

    elif granularity_sec >= 60:  # 1m
        analyses["analyses_run"] = ["InfoShare", "Spread_Convergence", "Lead_Lag"]
        logger.info(
            f"Running InfoShare + Spread Convergence + Lead-Lag for {granularity_sec}s granularity"
        )

    else:  # Sub-minute (30s, 15s, 5s)
        analyses["analyses_run"] = ["InfoShare", "Spread_Convergence", "Lead_Lag"]
        logger.info(
            f"Running InfoShare + Spread Convergence + Lead-Lag for {granularity_sec}s sub-minute granularity"
        )
        print(
            f"[SWEEP:subminute] granularity={granularity_sec}s, policy=RESEARCH_g={granularity_sec}s"
        )
...
def generate_sweep_report(
    ...
    with open(markdown_file, "w") as f:
        ...
        for result in sweep_results:
            ...
            f.write(
                f"| {granularity} | {min_dur} | {windows_found} | {best_window_str} | {coverage} | {analyses} |\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--loop",
        action="store_true",
        help="Continuous loop mode (scan every 2 minutes)",
    )
    parser.add_argument(
        "--loop-interval",
        type=int,
...
def run_continuous_sweep(
    ...
    try:
        while True:
            loop_count += 1
            logger.info(
                f"[SWEEP:loop] iteration={loop_count}, timestamp={datetime.now().isoformat()}"
            )
            ...
            for i, (granularity_sec, min_duration_min, coverage_thresh) in enumerate(
                ...
                logger.info(
                    f"[SWEEP:subminute] scanning granularity={granularity_sec}s, min_duration={min_duration_min}m, coverage={coverage_thresh}"
                )
                ...
                if windows:
                    window = windows[0]
                    logger.info(
                        f"[SWEEP:found] granularity={granularity_sec}s, duration={window.get('duration_minutes', 0):.1f}m"
                    )
        ...
        # Final summary
        logger.info(
            f"[SWEEP:final] total_windows_found={total_windows_found}, total_iterations={loop_count}"
        )
        with open(sweep_log, "a") as f:
            f.write(
                f"[SWEEP:final] total_windows_found={total_windows_found}, total_iterations={loop_count}\n"

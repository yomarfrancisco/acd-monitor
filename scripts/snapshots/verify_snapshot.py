...
def discover_and_verify_windows(args) -> int:
    ...
    try:
        ...
        logger.info(f"SUMMARY: windows={len(windows)}, PASS={windows_passed}, WARN={windows_warned}, FAIL={windows_failed} (warn-on-quality={warn_mode})")
...
def verify_single_window(s3_client, bucket: str, base_key: str, overlap_data: Dict, args) -> Dict:
    ...
    if critical_issues:
        ...
        if args.warn_on_quality:
            logger.warning(f"Window verification passed with {len(quality_issues)} quality warnings")
        else:

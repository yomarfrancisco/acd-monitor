...
def compute_leadership_scores(tick_data, window_size=300):
    ...
    for venue in venues:
        ...
        price_discovery = price_changes.rolling(window=window_size).apply(
            lambda x: 1 if x.iloc[0] != 0 else 0, raw=False
        ).mean()
        
        # Volume leadership (volume-weighted price changes)
        volume_leadership = (abs(price_changes) * venue_data['volume']).rolling(
            window=window_size
        ).mean()
...
def compute_rotation_metrics(leadership_scores):
    ...
    sorted_venues = sorted(venues, key=lambda v: leadership_scores[v]['leadership_score'], reverse=True)

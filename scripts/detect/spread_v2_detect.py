...
def detect_spread_episodes(
    tick_data, roll_window=60, z_threshold=-1.5, min_duration=10, merge_gap=2
):
    """
    Detect spread compression episodes using rolling z-score analysis
    """
    logger.info(
        f"Detecting spread episodes with roll_window={roll_window}, z_threshold={z_threshold}"
    )
...
def sample_matched_controls(episodes, tick_data, k=5, per_episode=100, gap=10):
    ...
    for episode in episodes:
        ...
        control_indices = np.random.choice(
            len(tick_data[list(tick_data.keys())[0]]),
            size=min(per_episode, 100),
            replace=False,
        )

...
def compute_cross_correlation(
    returns_i: pd.Series, returns_j: pd.Series, tau_max: int, tau_range: range
...
def compute_hac_p_value(
    returns_i: pd.Series, returns_j: pd.Series, tau: int, correlation: float
) -> float:
    """Compute p-value using HAC standard errors."""
    try:
...
def run_leadlag_v2_analysis(
    ...
    fdr_q: float = 0.05,
    export_dir: str = "experiments/phase5/leadlag_v2",
) -> Dict:
...
def main():
    ...
    parser.add_argument(
        "--export-dir", default="experiments/phase5/leadlag_v2", help="Export directory"
    )

...
def run_johansen_test(prices_df: pd.DataFrame, venues: List[str], 
                     max_lags: int = 4, det_order: int = 0) -> Dict:
    ...
    try:
        ...
        return {
            'rank': rank,
            'trace_stats': trace_stats.tolist() if hasattr(trace_stats, 'tolist') else list(trace_stats),
            'trace_pvalues': trace_pvalues.tolist() if hasattr(trace_pvalues, 'tolist') else list(trace_pvalues),
            'eigen_stats': eigen_stats.tolist() if hasattr(eigen_stats, 'tolist') else list(eigen_stats),
            'eigen_pvalues': eigen_pvalues.tolist() if hasattr(eigen_pvalues, 'tolist') else list(eigen_pvalues),
            'max_lags': max_lags,
            'det_order': det_order,
...
def estimate_vecm(prices_df: pd.DataFrame, venues: List[str], 
                  rank: int, max_lags: int = 4) -> Optional[Dict]:
    """Estimate VECM model."""
    try:
        if rank == 0:
...
def run_infoshare_v2_analysis(snapshot_path: str, cadence: str = "1s",
                             max_lags: int = 4, det_order: int = 0,
                             placebo_shifts: List[int] = [-60, 60],
                             fdr_q: float = 0.05, export_dir: str = "experiments/phase5/infoshare_v2") -> Dict:
    ...
    for shift in placebo_shifts:
        ...
        placebo_results[shift] = {
            'johansen': placebo_johansen,
            'shift': shift
        }
    ...
    # Gate assessment
    gate_verdict = "pass" if (johansen_results['rank'] > 0 and placebo_collapse and vecm_results) else "fail"
...
def main():
    ...
    parser.add_argument("--export-dir", default="experiments/phase5/infoshare_v2", 
    ...
    try:
        ...
        cointegration_file.write_text(json.dumps(cointegration_data, cls=PandasJSONEncoder, indent=2))
        ...
        params_data = {k: v for k, v in results.items() if k in ['snapshot_path', 'venues', 'n_venues', 
                                                                'cadence', 'max_lags', 'det_order', 
                                                                'fdr_q', 'placebo_shifts', 'created_at']}

...
class ConservativePanelExtender:
    """Conservative panel extender - one date at a time."""
    ...
    def extend_panel_conservative(self, target_date: str = "20250928"):
        """Extend panel with one additional date conservatively."""
        print(f"🔧 Conservative Panel Extension: {target_date}")
        print("=" * 50)
        ...
        new_date_panel = self._process_single_date_conservative(target_date)
        ...
        # Save extended panel
        extended_file = f"{self.data_dir}/panel_1s_inner_real_multiday.parquet"
        combined_panel.to_parquet(extended_file)
        print(f"💾 Saved extended panel to {extended_file}")
        
        # Generate summary
        self._generate_extension_summary(existing_panel, new_date_panel, combined_panel, target_date)
        
        print(f"\n✅ Panel extension completed")
        return True
    
    def _process_single_date_conservative(self, target_date: str) -> Optional[pd.DataFrame]:
        """Process a single date conservatively."""
        ...
        # Process each venue conservatively
        venue_data = {}
        for venue, venue_file_list in venue_files.items():
            print(f"      🔄 Processing {venue} ({len(venue_file_list)} files)...")
            venue_df = self._load_venue_data_conservative(venue, venue_file_list)
    ...
    def _load_venue_data_conservative(self, venue: str, venue_files: List[Dict]) -> Optional[pd.DataFrame]:
        """Load data for a venue conservatively."""
        venue_dfs = []
        ...
        for file_info in venue_files:
            try:
                ...
                if len(df) > 0:
                    ...
                    essential_columns = ['best_bid', 'best_ask', 'last_px', 'bid_sz', 'ask_sz', 'trade_sz']
    ...
    def _generate_extension_summary(self, existing_panel, new_panel, combined_panel, target_date):
        ...
        summary = {
            ...
            'new_range': {
                'start': str(new_panel.index.min()),
                'end': str(new_panel.index.max())
            },
            'combined_range': {
                'start': str(combined_panel.index.min()),
                'end': str(combined_panel.index.max()),
                'duration_hours': (combined_panel.index.max() - combined_panel.index.min()).total_seconds() / 3600
            }
        }
        ...
        with open(f"{self.analysis_dir}/EXTENSION_SUMMARY.md", 'w') as f:
            ...
            f.write("## Date Ranges\n\n")
            f.write(f"**Existing**: {summary['existing_range']['start']} to {summary['existing_range']['end']}\n")
            f.write(f"**New**: {summary['new_range']['start']} to {summary['new_range']['end']}\n")
            f.write(f"**Combined**: {summary['combined_range']['start']} to {summary['combined_range']['end']}\n")
            f.write(f"**Total Duration**: {summary['combined_range']['duration_hours']:.1f} hours\n\n")
            
            f.write("## Files Generated\n\n")
            f.write("- `panel_1s_inner_real_multiday.parquet`\n")

if __name__ == "__main__":
    extender = ConservativePanelExtender()
    success = extender.extend_panel_conservative("20250928")

...
def compute_venue_flags(df: pd.DataFrame, venue: str) -> pd.DataFrame:
    ...
    venue_df['vwap_v'] = venue_df.groupby('date').apply(
        lambda x: np.average(x['mid_v'], weights=x.get('trade_sz', 1))
    ).reset_index(level=0, drop=True)
    
    # VWAP fallback flag (if sizes missing)
    venue_df['vwap_fallback_v'] = (venue_df['trade_sz'].isna() | (venue_df['trade_sz'] == 0)).astype(int)
    ...
    venue_df['is_vwap_dev_2sigma_v'] = (venue_df['mid_vwap_diff'] > 2 * venue_df['mid_vwap_std']).astype(int)
    ...
    venue_df['vwap_reset_jump'] = venue_df.groupby('date')['vwap_v'].transform(compute_vwap_reset_jump)

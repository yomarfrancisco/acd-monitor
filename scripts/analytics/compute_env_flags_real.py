    ...
    def compute_flags(self):
        ...
        for venue in self.venues:
            ...
            if mid_col in panel_data.columns:
                ...
                env_flags[f"is_return_2sigma_{venue}"] = (returns.abs() > 2 * rolling_std).astype(
                    int
                )
                ...
                env_flags[f"is_vwap_dev_2sigma_{venue}"] = (vwap_dev.abs() > 2 * vwap_std).astype(
                    int
                )
            else:
        ...
        for venue in self.venues:
            ...
            if bid_col in panel_data.columns and ask_col in panel_data.columns:
                ...
                if bid_sz_col in panel_data.columns and ask_sz_col in panel_data.columns:
                    env_flags[f"tob_depth_{venue}"] = (
                        panel_data[bid_sz_col] + panel_data[ask_sz_col]
                    )

                    # Imbalance
                    env_flags[f"imbalance_{venue}"] = (
                        panel_data[bid_sz_col] - panel_data[ask_sz_col]
                    ) / (panel_data[bid_sz_col] + panel_data[ask_sz_col])
                else:

...
class Wave3VariablesPreparer:
    ...
    def prepare_all_variables(self):
        ...
        # Check all steps
        all_success = all(
            [icp_success, vmm_success, copula_success, clustering_success, composite_success]
        )
    ...
    def _calculate_core_series(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for venue in self.venues:
            ...
            if mid_col in core_data.columns:
                core_data[f"{venue}_return"] = np.log(
                    core_data[mid_col] / core_data[mid_col].shift(1)
                )

        # Calculate equal-weighted mid and spread
        mid_cols = [
            f"{venue}_mid_px" for venue in self.venues if f"{venue}_mid_px" in core_data.columns
        ]
        spread_cols = [
            f"spread_{venue}" for venue in self.venues if f"spread_{venue}" in core_data.columns
        ]
        ...
        if mid_cols:
            core_data["dispersion"] = core_data[mid_cols].max(axis=1) - core_data[mid_cols].min(
                axis=1
            )
            ...
            while current_start + window_duration <= end_time:
                ...
                if len(window_data) > 0:
                    ...
                    return_cols = [
                        f"{venue}_return"
                        for venue in self.venues
                        if f"{venue}_return" in window_data.columns
                    ]
                            ...
                            for i, col1 in enumerate(return_cols):
                                for j, col2 in enumerate(return_cols):
                                    if i <= j:  # Only upper triangle
                                        moment_row[f"cov_{col1}_{col2}"] = cov_matrix.loc[
                                            col1, col2
                                        ]
                    ...
                    if "mid_eq_return" in window_data.columns:
                        for venue in self.venues:
                            ...
                            if mid_col in window_data.columns:
                                price_lead = (
                                    window_data[mid_col] - window_data["mid_eq"].shift(1)
                                ) * window_data["mid_eq_return"]
                    ...
                    if "session_label" in window_data.columns:
                        ...
                        moment_row["session_europe"] = (
                            window_data["session_label"] == "europe"
                        ).sum()
                        moment_row["session_us"] = (window_data["session_label"] == "us").sum()
                        moment_row["session_pacific"] = (
                            window_data["session_label"] == "pacific"
                        ).sum()
                    ...
                    if "structure_state" in window_data.columns:
                        moment_row["structure_up_count"] = (
                            window_data["structure_state"] == "up"
                        ).sum()
                        moment_row["structure_down_count"] = (
                            window_data["structure_state"] == "down"
                        ).sum()
            ...
            while current_start + window_duration <= end_time:
                ...
                if len(window_data) > 0:
                    feature_row = {
                        ...
                        "window_end": current_end,
                        "session_label": (
                            window_data["session_label"].iloc[0]
                            if "session_label" in window_data.columns
                            else "unknown"
                        ),
                    }
                    ...
                    if "structure_state" in window_data.columns:
                        ...
                        feature_row["structure_up_pct"] = (
                            window_data["structure_state"] == "up"
                        ).sum() / total
                        feature_row["structure_down_pct"] = (
                            window_data["structure_state"] == "down"
                        ).sum() / total
                        feature_row["structure_neutral_pct"] = (
                            window_data["structure_state"] == "neutral"
                        ).sum() / total
    ...
    def _prepare_composite_inputs(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            while current_start + window_duration <= end_time:
                ...
                if len(window_data) > 0:
                    composite_row = {"window_start": current_start, "window_end": current_end}
                    ...
                    spread_cols = [
                        f"spread_{venue}"
                        for venue in self.venues
                        if f"spread_{venue}" in window_data.columns
                    ]
                    if spread_cols:
                    ...
                    if "bos_up" in window_data.columns and "bos_dn" in window_data.columns:
                        composite_row["structure_instability"] = (
                            window_data["bos_up"].sum() + window_data["bos_dn"].sum()
                        )

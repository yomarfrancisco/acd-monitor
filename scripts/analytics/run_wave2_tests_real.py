...
class RealWave2TestRunner:
    ...
    def _run_event_studies(self, panel_data, env_flags):
        ...
        # Event study results
        event_results = {
            'events_analyzed': ['is_return_2sigma', 'is_vwap_dev_2sigma', 'is_ny_open', 'is_session_transition'],
        ...
        for event in event_results['events_analyzed']:
            if event in env_flags.columns:
                event_counts = {}
                for venue in self.venues:
                    event_col = f"{event}_{venue}" if f"{event}_{venue}" in env_flags.columns else event
    ...
    def _run_markov_switching(self, panel_data, env_flags):
        ...
        spread_cols = [f"spread_{venue}" for venue in self.venues if f"spread_{venue}" in env_flags.columns]
        if not spread_cols:

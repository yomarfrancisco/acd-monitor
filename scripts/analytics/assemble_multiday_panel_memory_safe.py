...
class MemorySafeMultiDayAssembler:
    ...
    def _concatenate_panels(self, all_panels: List[pd.DataFrame], processed_dates: List[str]) -> Optional[pd.DataFrame]:
    ...
    def _generate_day_summary(self, date: str, panel: pd.DataFrame):
        ...
        with open(f"{self.analysis_dir}/DAY_{date}_SUMMARY.md", 'w') as f:
            ...
            f.write(f"**Time Range**: {summary['date_range']['start']} to {summary['date_range']['end']}\n")
    ...
    def _generate_assembly_summary(self, multi_day_panel: pd.DataFrame, processed_dates: List[str]):
        ...
        summary = {
            ...
            'date_range': {
                ...
                'duration_hours': (multi_day_panel.index.max() - multi_day_panel.index.min()).total_seconds() / 3600
            },
            'venue_coverage': {}
        }

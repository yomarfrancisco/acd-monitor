...
class MarkovSwitchingFixer:
    ...
    def _run_option_a_spread_median(self):
        ...
        spread_columns = [f"{venue}_spread" for venue in self.venues if f"{venue}_spread" in self.panel_data.columns]
        ...
        try:
            ...
            # Save results
            pd.DataFrame([results]).to_csv(f"{self.output_dir}/spread_median_results.csv", index=False)
    ...
    def _plot_markov_regimes(self, series: pd.Series, regime_probs: np.ndarray, name: str):
        ...
        ax1.fill_between(series.index, 0, 1, alpha=0.3, 
                         where=regime_probs[1] > 0.5, color='red', label='Regime 1')
        ax1.fill_between(series.index, 0, 1, alpha=0.3, 
                         where=regime_probs[0] > 0.5, color='blue', label='Regime 0')

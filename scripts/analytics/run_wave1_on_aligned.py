...
class AlignedWave1Tester:
    ...
    def _generate_summary(self) -> str:
        ...
        if 'variance_ratios' in self.results:
            ...
            other_vr = [vr_data.get(venue, {}).get('vr_2', np.nan) 
            ...
            if not np.isnan(coinbase_vr) and other_vr:
                ...
                if coinbase_vr > avg_other_vr * 2:
                    summary_parts.append(f"Coinbase anomaly PERSISTS: VR={coinbase_vr:.3f} vs others={avg_other_vr:.3f}")
                else:
                    summary_parts.append(f"Coinbase anomaly RESOLVED: VR={coinbase_vr:.3f} vs others={avg_other_vr:.3f}")

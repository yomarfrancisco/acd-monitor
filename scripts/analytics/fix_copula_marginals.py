...
class CopulaMarginalsFixer:
    ...
    def _calculate_core_series(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for venue in self.venues:
            ...
            if mid_col in data_with_series.columns:
                ...
                # Calculate returns
                data_with_series[f"{venue}_return"] = np.log(
                    data_with_series[mid_col] / data_with_series[mid_col].shift(1)
                )
        ...
        for venue in self.venues:
            ...
            if existing_spread_col in data_with_series.columns:
                ...
                if bid_col in data_with_series.columns and ask_col in data_with_series.columns:
                    data_with_series[f"{venue}_spread"] = (
                        data_with_series[ask_col] - data_with_series[bid_col]
                    )
        ...
        mid_cols = [
            f"{venue}_mid_px"
            for venue in self.venues
            if f"{venue}_mid_px" in data_with_series.columns
        ]
        if mid_cols:
            data_with_series["dispersion"] = data_with_series[mid_cols].max(
                axis=1
            ) - data_with_series[mid_cols].min(axis=1)
    ...
    def _generate_copula_marginals(self, data: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        for session in ["Asia", "Europe", "US", "Pacific"]:
            if session in session_counts.index and session_counts[session] < self.min_obs:
                print(
                    f"  ⚠️ Session {session} has {session_counts[session]} obs < {self.min_obs}, using whole-day fallback"
                )
    ...
    def _generate_marginals_for_partition(
        self, partition_data: pd.DataFrame, partition_name: str
    ...
    def _process_chunk_marginals(
        self, chunk_data: pd.DataFrame, partition_name: str
    ) -> Optional[pd.DataFrame]:
        ...
        chunk_marginals["session_label"] = (
            partition_name
            if partition_name != "whole_day"
            else (
                chunk_data["session_label"].iloc[0]
                if "session_label" in chunk_data.columns
                else "unknown"
            )
        )

...
Extend prepare_wave2_variables.py to accept panel path arguments and work with 7-day data.
"""
...
class ExtendedWave2VariablePreparer:
    """Prepare Wave-2 variables for extended panel."""
    ...
    def prepare_extended_wave2_variables(self):
        """Prepare Wave-2 variables for extended panel."""
        print(f"🔧 Preparing Wave-2 Variables for Extended Panel")
        print("=" * 50)
    ...
    def _load_extended_panel_data(self):
        ...
        if os.path.exists(self.env_path):
            ...
            self.panel_data = self.panel_data.merge(env_flags, left_index=True, right_index=True, how='left')
        ...
        if os.path.exists(self.structure_path):
            ...
            market_structure = market_structure.sort_index().loc[~market_structure.index.duplicated(keep='last')]
            ...
            self.panel_data = self.panel_data.merge(market_structure, left_index=True, right_index=True, how='left')
    ...
    def _prepare_event_study_variables(self):
        ...
        if event_data:
            ...
            event_df.to_parquet(f"{self.output_dir}/event_study_data.parquet")
    ...
    def _identify_shock_events(self):
    ...
    def _create_event_window(self, event):
...
def main():
    ...
    # Initialize preparer
    preparer = ExtendedWave2VariablePreparer(panel_path, env_path, structure_path, output_dir)

...
class Wave3ICPExecutor:
    ...
    def _prepare_icp_data(self, icp_data: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        exclude_cols = ['ts', 'Y', 'session_label', 'is_session_transition', 'is_ny_open', 
                       'is_vwap_reset_window', 'structure_state', 'BOS_up_count_10m', 
                       'BOS_down_count_10m', 'available_venue_count']
        ...
        x_cols = [col for col in x_cols if not col.startswith('is_return_2sigma_') and 
                  not col.startswith('is_vwap_dev_2sigma_')]
    ...
    def _generate_icp_summary(self, env_results: Dict, invariance_results: Dict):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write("**Hypothesis**: Competitive markets show environment-dependent relationships; coordination shows invariant relationships\n\n")
            ...
            invariant_count = sum(1 for result in invariance_results.values() if result['invariant'])
            total_predictors = len(invariance_results)
            
            f.write(f"- **Total predictors tested**: {total_predictors}\n")
            f.write(f"- **Invariant predictors**: {invariant_count} ({invariant_count/total_predictors:.1%})\n")
            f.write(f"- **Variant predictors**: {total_predictors - invariant_count} ({(total_predictors - invariant_count)/total_predictors:.1%})\n\n")
            
            # Top variant predictors
            f.write("### Top Variant Predictors (p < 0.05)\n\n")
            variant_predictors = [(pred, result) for pred, result in invariance_results.items() 
            ...
            for predictor, result in variant_predictors[:10]:  # Top 10
                f.write(f"- **{predictor}**: p = {result['p_value']:.4f}, F = {result['f_statistic']:.4f}\n")
            
            # Top invariant predictors
            f.write("\n### Top Invariant Predictors (p > 0.05)\n\n")
            invariant_predictors = [(pred, result) for pred, result in invariance_results.items() 
                                  if result['invariant']]
            ...
            if invariant_count / total_predictors > 0.5:
                f.write("**Result**: High proportion of invariant predictors suggests **coordinated behavior**\n")
                ...
                f.write("**Result**: High proportion of variant predictors suggests **competitive behavior**\n")

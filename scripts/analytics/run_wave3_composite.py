...
class Wave3CompositeExecutor:
    ...
    def _load_module_results(self) -> Dict:
        ...
        # Load ICP results
        try:
            icp_invariance = pd.read_csv(f"{self.wave3_dir.replace('wave3', 'analysis/wave3/btc_usd/icp')}/invariance_tests.csv")
        ...
        # Load VMM results
        try:
            vmm_jstats = pd.read_csv(f"{self.wave3_dir.replace('wave3', 'analysis/wave3/btc_usd/vmm')}/Jstats.csv")
        ...
        try:
            ...
            session_files = [f for f in os.listdir(copula_dir) if f.startswith('session_') and f.endswith('.json')]
            ...
            for session_file in session_files:
                with open(f"{copula_dir}/{session_file}", 'r') as f:
                    ...
                    if 'kendall_tau' in session_data:
                        tau_values = [result['tau'] for result in session_data['kendall_tau'].values()]
                        all_tau_values.extend(tau_values)
            
            avg_dependence = np.mean([abs(tau) for tau in all_tau_values]) if all_tau_values else 0.5
        ...
        try:
            cluster_profiles = pd.read_csv(f"{self.wave3_dir.replace('wave3', 'analysis/wave3/btc_usd/clustering')}/cluster_profiles.csv")
            ...
            module_results['clustering'] = {'max_coordination_score': 0.5, 'coordination_signal': 0.5}
        
        print(f"  📊 Loaded results from {len(module_results)} modules")
        return module_results
    
    def _construct_composite_index(self, composite_data: pd.DataFrame, module_results: Dict) -> Dict:
        ...
        for _ in range(n_bootstrap):
            ...
            bootstrap_indices = np.random.choice(len(composite_scores), size=len(composite_scores), replace=True)
    ...
    def _generate_composite_summary(self, composite_index: Dict):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write("**Purpose**: Aggregate weak signals into single coordination monitoring score\n\n")
            ...
            f.write("- **Heuristic weights**: Module weights are data-driven, not theoretically derived\n")

...
class Wave3CopulaExecutor:
    ...
    def _fit_gaussian_copula(self, data: pd.DataFrame) -> Dict:
        ...
        log_lik = -0.5 * n_obs * (n_vars * np.log(2 * np.pi) + np.log(np.linalg.det(corr_matrix.values + 1e-6)))
    ...
    def _generate_copula_summary(self, session_results: Dict):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write("**Hypothesis**: Competitive markets show session-dependent dependence; coordination shows stable, high dependence\n\n")
            
            # Session results
            f.write("## Session Results\n\n")
            f.write("| Session | Observations | Venues | Gaussian AIC | t-Copula AIC | Best Model |\n")
            f.write("|---------|-------------|--------|-------------|-------------|------------|\n")
            ...
            for session, results in session_results.items():
                ...
                f.write(f"| {session} | {results['n_obs']:,} | {results['n_venues']} | {gaussian_aic:.2f} | {t_aic:.2f} | {best_model} |\n")
            ...
            for session, results in session_results.items():
                ...
                tail_values = [result['upper_tail'] for result in results['tail_dependence'].values()]
                if tail_values:
            ...
            if all_tau_values:
                ...
                high_dependence_pct = np.mean([abs(tau) > high_dependence_threshold for tau in all_tau_values])
                ...
                if high_dependence_pct > 0.5:
                    f.write("- **⚠️ CONCERNING**: High proportion of strong dependence across sessions\n")
                    ...
                    f.write("- This suggests competitive behavior with session-dependent dependence\n")

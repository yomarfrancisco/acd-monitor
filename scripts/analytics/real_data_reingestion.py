...
class RealDataReingestion:
    ...
    def get_data_coverage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Data coverage: {coverage['total_records']} records, {coverage['unique_venues']} venues, {coverage['unique_days']} days"
            )
            return coverage
    ...
    def create_aligned_panel(
        self, start_date: str = "2025-09-28", end_date: str = "2025-09-30"
    ) -> pd.DataFrame:
    ...
    def save_aligned_panel(self, df: pd.DataFrame) -> None:
        ...
        metadata = {
            ...
            "date_range": {"start": df.index.min().isoformat(), "end": df.index.max().isoformat()},
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        avg_coverage = np.mean(
            [metrics["coverage_pct"] for metrics in quality_metrics["venue_coverage"].values()]
        )
        avg_missing = np.mean(
            [metrics["missing_pct"] for metrics in quality_metrics["missing_data"].values()]
        )

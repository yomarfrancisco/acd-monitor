...
class Wave3VariablesAuditor:
    ...
    def audit_all_variables(self):
        ...
        self._generate_audit_report(
            icp_audit, vmm_audit, copula_audit, clustering_audit, composite_audit
        )
    ...
    def _audit_icp_design(self):
        ...
        try:
            ...
            x_cols = [
                col for col in df.columns if col.startswith(("binance_return", "coinbase_return"))
            ]
            ...
            print(
                f"  ✅ ICP design: {audit['rows']:,} rows, {audit['columns']} columns, {audit['non_missing_pct']:.1%} non-missing"
            )
            return audit
    ...
    def _audit_vmm_moments(self):
        ...
        try:
            ...
            moment_cols = [
                col for col in df.columns if any(x in col for x in ["mean", "var", "cov"])
            ]
            ...
            instrument_cols = [
                ...
                if any(x in col for x in ["session_", "shock_", "ny_open"])
            ]
            ...
            print(
                f"  ✅ VMM moments: {audit['rows']:,} rows, {audit['columns']} columns, {audit['non_missing_pct']:.1%} non-missing"
            )
            return audit
    ...
    def _audit_copula_marginals(self):
        ...
        try:
            ...
            print(
                f"  ✅ Copula marginals: {audit['rows']:,} rows, {audit['columns']} columns, {audit['non_missing_pct']:.1%} non-missing"
            )
            return audit
    ...
    def _audit_clustering_features(self):
        ...
        try:
            ...
            print(
                f"  ✅ Clustering features: {audit['rows']:,} rows, {audit['columns']} columns, {audit['non_missing_pct']:.1%} non-missing"
            )
            return audit
    ...
    def _audit_composite_inputs(self):
        ...
        try:
            ...
            signal_cols = [
                col
                for col in df.columns
                if not col.endswith("_zscore") and col not in ["window_start", "window_end"]
            ]
            ...
            print(
                f"  ✅ Composite inputs: {audit['rows']:,} rows, {audit['columns']} columns, {audit['non_missing_pct']:.1%} non-missing"
            )
            return audit
    ...
    def _generate_audit_report(
        self, icp_audit, vmm_audit, copula_audit, clustering_audit, composite_audit
    ):
        ...
        with open(report_file, "w") as f:
            ...
            for name, audit in audits:
                if audit:
                    ...
                    f.write(
                        f"| {name} | {audit['rows']:,} | {audit['columns']} | {audit['non_missing_pct']:.1%} | {status} |\n"
                    )
                else:
                    f.write(f"| {name} | - | - | - | ❌ |\n")
            ...
            if icp_audit:
                ...
                if icp_audit["y_stats"]:
                    f.write(
                        f"- **Y variable**: {icp_audit['y_stats']['count']:,} obs, mean={icp_audit['y_stats']['mean']:.4f}\n"
                    )
            ...
            if vmm_audit:
                ...
                if vmm_audit["window_stats"]:
                    f.write(
                        f"- **Window duration**: {vmm_audit['window_stats']['duration_mean']:.1f} min average\n"
                    )

                f.write(f"- **Moment columns**: {len(vmm_audit['moment_stats'])} analyzed\n")
                f.write(
                    f"- **Instrument columns**: {len(vmm_audit['instrument_stats'])} analyzed\n"
                )
                f.write("\n")
            ...
            if copula_audit:
                ...
                if copula_audit["marginal_stats"]:
                    ...
                    for col, stats in copula_audit["marginal_stats"].items():
                        f.write(
                            f"  - {col}: [{stats['min']:.3f}, {stats['max']:.3f}] range, {stats['count']:,} obs\n"
                        )
            ...
            if clustering_audit:
                ...
                if clustering_audit["zscore_stats"]:
                    f.write(
                        f"- **Z-score standardization**: {clustering_audit['zscore_stats']['count']:,} values, mean={clustering_audit['zscore_stats']['mean']:.3f}\n"
                    )
            ...
            if composite_audit:
                ...
                if composite_audit["zscore_stats"]:
                    f.write(
                        f"- **Z-score standardization**: {composite_audit['zscore_stats']['count']:,} values, mean={composite_audit['zscore_stats']['mean']:.3f}\n"
                    )
            ...
            f.write(
                f"- **Total rows across all files**: {sum(a['rows'] for a in successful_audits):,}\n"
            )
            f.write(
                f"- **Average non-missing %**: {np.mean([a['non_missing_pct'] for a in successful_audits]):.1%}\n"
            )

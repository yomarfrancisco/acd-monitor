#!/usr/bin/env python3
"""
Memory-Efficient Multi-Day Panel Assembly
Process S3 parquet files in batches to avoid memory issues.
"""
...
class BatchMultiDayPanelAssembler:
    """Memory-efficient multi-day panel assembler."""
    ...
    def assemble_multi_day_panel(self):
        """Assemble multi-day panel using batch processing."""
        print("🔧 Memory-Efficient Multi-Day Panel Assembly")
        print("=" * 60)
        ...
        # Step 2: Process each date separately
        all_panels = []
        for date, files in files_by_date.items():
            print(f"\n🔄 Processing {date}...")
            date_panel = self._process_date_batch(date, files)
        ...
        # Step 3: Combine all date panels
        print(f"\n🔄 Combining {len(all_panels)} date panels...")
        ...
        self._generate_assembly_report(cleaned_panel, files_by_date)
                ...
                if "Contents" in page:
                    for obj in page["Contents"]:
                        if obj["Key"].endswith(".parquet"):
                            # Extract date from key: snapshots/BTC-USD/{date}/...
                            parts = obj["Key"].split("/")
                            if len(parts) >= 3:
                                date = parts[2]
                                objects.append(
                                    {"key": obj["Key"], "date": date, "size": obj["Size"]}
                                )
    ...
    def _process_date_batch(self, date: str, files: List[Dict]) -> Optional[pd.DataFrame]:
        ...
        for venue, venue_file_list in venue_files.items():
            ...
            venue_df = self._load_venue_data_batch(venue, venue_file_list)
    ...
    def _load_venue_data_batch(self, venue: str, venue_files: List[Dict]) -> Optional[pd.DataFrame]:
        """Load data for a venue in batch."""
        venue_dfs = []
    ...
    def _generate_assembly_report(self, panel: pd.DataFrame, files_by_date: Dict):
        ...
        with open(f"{self.analysis_dir}/ASSEMBLY_SUMMARY.md", "w") as f:
            ...
            f.write(f"**Dates Processed**: {report['total_dates']}\n")
...
if __name__ == "__main__":
    assembler = BatchMultiDayPanelAssembler()

...
class RealMarketStructureComputer:
    ...
    def _compute_structure_indicators(self, bars):
        ...
        structure_data["structure_state"] = self._compute_structure_state(
    ...
    def _compute_fractal_swings(self, bars, k=2):
        ...
        for i in range(k, len(bars) - k):
            ...
            if all(high.iloc[i] > high.iloc[i - j] for j in range(1, k + 1)) and all(
                high.iloc[i] > high.iloc[i + j] for j in range(1, k + 1)
            ):
                swing_high[i] = 1

            # Swing low: lower than k bars on each side
            if all(low.iloc[i] < low.iloc[i - j] for j in range(1, k + 1)) and all(
                low.iloc[i] < low.iloc[i + j] for j in range(1, k + 1)
            ):
                swing_low[i] = 1

...
class MarketStructureComputer:
    ...
    def _compute_fractal_swings(self):
        ...
        for i in range(k, len(self.bars_5s) - k):
            if (
                self.bars_5s["high"].iloc[i] > self.bars_5s["high"].iloc[i - k : i].max()
                and self.bars_5s["high"].iloc[i]
                > self.bars_5s["high"].iloc[i + 1 : i + k + 1].max()
            ):
        ...
        for i in range(k, len(self.bars_5s) - k):
            if (
                self.bars_5s["low"].iloc[i] < self.bars_5s["low"].iloc[i - k : i].min()
                and self.bars_5s["low"].iloc[i] < self.bars_5s["low"].iloc[i + 1 : i + k + 1].min()
            ):
    ...
    def _compute_bos_choch(self):
        ...
        for i in range(len(self.bars_5s)):
            ...
            if current_bar["swing_high"] == 1:
                ...
                self.bars_5s.iloc[i, self.bars_5s.columns.get_loc("last_swing_high")] = (
                    last_swing_high
                )

            if current_bar["swing_low"] == 1:
                last_swing_low = current_bar["low"]
                self.bars_5s.iloc[i, self.bars_5s.columns.get_loc("last_swing_low")] = (
                    last_swing_low
                )
            ...
            if (
                ...
                and current_bar["close"] > last_swing_high + 0.25 * current_bar["atr14"]
            ):
            ...
            if (
                ...
                and current_bar["close"] < last_swing_low - 0.25 * current_bar["atr14"]
            ):

...
class RealDataSampleAnalysis:
    ...
    def analyze_sample_files(self, file_paths: List[str]) -> Dict[str, Any]:
        ...
        for file_path in file_paths:
            try:
                ...
                file_analysis = {
                    ...
                    'min_timestamp': df['ts_exchange'].min() if 'ts_exchange' in df.columns else None,
                    'max_timestamp': df['ts_exchange'].max() if 'ts_exchange' in df.columns else None,
        ...
        if all_data:
            ...
            analysis_results['schema_analysis'] = {
                ...
                'unique_venues': combined_df['venue'].unique().tolist() if 'venue' in combined_df.columns else [],
                'date_range': {
                    'min': combined_df['ts_exchange'].min() if 'ts_exchange' in combined_df.columns else None,
                    'max': combined_df['ts_exchange'].max() if 'ts_exchange' in combined_df.columns else None
                }
            }
            ...
            analysis_results['data_quality'] = {
                ...
                'venue_coverage': combined_df['venue'].value_counts().to_dict() if 'venue' in combined_df.columns else {}
            }
            
            # Coverage summary
            analysis_results['coverage_summary'] = {
                'total_records': len(combined_df),
                'unique_venues': len(combined_df['venue'].unique()) if 'venue' in combined_df.columns else 0,
                'date_range_days': (combined_df['ts_exchange'].max() - combined_df['ts_exchange'].min()) / (1000 * 60 * 60 * 24) if 'ts_exchange' in combined_df.columns else 0
            }
    ...
    def save_sample_analysis(self, analysis_results: Dict[str, Any], aligned_df: pd.DataFrame) -> None:

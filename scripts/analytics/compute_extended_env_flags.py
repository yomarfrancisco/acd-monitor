...
class ExtendedEnvFlagsComputer:
    ...
    def _compute_session_flags(self, flags_df):
        ...
        ny_mask = (flags_df.index.hour == 13) & (flags_df.index.minute >= 30) & (flags_df.index.minute <= 45)
    ...
    def _compute_shock_flags(self, panel_data, flags_df):
        ...
        for venue in self.venues:
            ...
            if mid_col in panel_data.columns:
                ...
                flags_df[f'is_return_2sigma_{venue}'] = (returns.abs() > 2 * rolling_std).astype(int)
                ...
                flags_df[f'is_vwap_dev_2sigma_{venue}'] = (deviations.abs() > 2 * deviation_std).astype(int)
    ...
    def _print_qc_stats(self, flags_df):
        ...
        coverage_pct = (total_obs - flags_df.isna().sum().sum() / len(flags_df.columns)) / total_obs * 100

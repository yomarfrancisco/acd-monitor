...
class RobustnessChecker:
    ...
    def _session_stratified_analysis(self):
        ...
        for session in sessions:
            ...
            print(f"    ✅ {session} session: {len(returns_df)} obs, {len(granger_results)} Granger tests, {len(cointegration_results)} cointegration tests")
    ...
    def _coinbase_slice_analysis(self):
        ...
        returns_without_coinbase = returns_df[[f"mid_{v}" for v in venues_without_coinbase if f"mid_{v}" in returns_df.columns]]
        ...
        comparison_results = {
            ...
            'without_coinbase': {
                'var_ratios': var_ratios_without if len(returns_without_coinbase.columns) >= 2 else {},
                'granger_results': granger_without if len(returns_without_coinbase.columns) >= 2 else {},
                'observations': len(returns_without_coinbase) if len(returns_without_coinbase.columns) >= 2 else 0
            }
        }
        ...
        print(f"    With Coinbase: {len(granger_with) if 'mid_coinbase' in returns_df.columns else 0} Granger tests")
        print(f"    Without Coinbase: {len(granger_without) if len(returns_without_coinbase.columns) >= 2 else 0} Granger tests")
    ...
    def _lag_window_sensitivity(self):
        ...
        for max_lags in [2, 5]:
            ...
            print(f"    ✅ {max_lags} lags: {len(granger_results)} tests, {sum([r['significant'] for r in granger_results.values()])} significant")
        ...
        # Save results
        pd.DataFrame(lag_results).T.to_csv(f"{self.output_dir}/lag_window_sensitivity/lag_results.csv")
        pd.DataFrame(window_results).T.to_csv(f"{self.output_dir}/lag_window_sensitivity/window_results.csv")
    ...
    def _plot_session_comparison(self, session_results: Dict):
        ...
        for session in sessions:
            if session_results[session]['granger_results']:
                significant = sum([r['significant'] for r in session_results[session]['granger_results'].values()])
        ...
        for session in sessions:
            if session_results[session]['cointegration_results']:
                significant = sum([r['cointegrated'] for r in session_results[session]['cointegration_results'].values()])
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/by_session/session_comparison.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_coinbase_comparison(self, comparison_results: Dict):
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/coinbase_slice/coinbase_comparison.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_sensitivity_analysis(self, lag_results: Dict, window_results: Dict):
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/lag_window_sensitivity/sensitivity_analysis.png", dpi=300, bbox_inches='tight')
        plt.close()

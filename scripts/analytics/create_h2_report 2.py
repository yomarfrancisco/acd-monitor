...
def load_lock_data() -> Dict[str, Any]:
    ...
    try:
        response = s3.get_object(
            Bucket=S3_BUCKET, Key=f"analysis/{DATE}/wave1_h2/_checks/_lock.json"
        )
...
def analyze_env_flags() -> Dict[str, Any]:
    ...
    for symbol in ["BTC-USD", "ETH-USD"]:
        ...
        try:
            ...
            session_dist = (
                df["session_label"].value_counts().to_dict()
    ...
    for symbol in ["BTC-USD", "ETH-USD"]:
        ...
        try:
            ...
            if len(df) > 0:
                ...
                results[symbol] = {
                    "total_pairs": 0,
                    "message": "No mutual information data available",
                }
...
def main():
    ...
    s3.put_object(Bucket=S3_BUCKET, Key=report_key, Body=report)

...
class Wave2TestRunner:
                ...
                pre_returns = (
                    pre_period[
                        [
                            ...
                            if f"{venue}_mid_px" in pre_period.columns
                        ]
                    ]
                    .pct_change()
                    .mean()
                ...
                post_returns = (
                    post_period[
                        [
                            f"{venue}_mid_px"
                            for venue in self.venues
                            if f"{venue}_mid_px" in post_period.columns
                        ]
                    ]
                    .pct_change()
                    .mean()
                ...
                pre_spreads = (
                    pre_period[
                        [
                            f"{venue}_spread"
                            for venue in self.venues
                            if f"{venue}_spread" in pre_period.columns
                        ]
                    ]
                    .mean()
                    .mean()
                ...
                post_spreads = (
                    post_period[
                        [
                            f"{venue}_spread"
                            for venue in self.venues
                            if f"{venue}_spread" in post_period.columns
                        ]
                    ]
                    .mean()
                    .mean()
    ...
    def _plot_event_studies(self, event_data: pd.DataFrame):
                ...
                returns = (
                    type_data[
                        [
                            f"{venue}_mid_px"
                            for venue in self.venues
                            if f"{venue}_mid_px" in type_data.columns
                        ]
                    ]
                    .pct_change()
                    .mean(axis=1)
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.output_dir}/event_study/event_study_plots.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _run_cointegration_tests(self):
        ...
        # Save results
        pd.DataFrame(cointegration_results).T.to_csv(
            f"{self.output_dir}/cointegration/cointegration_results.csv"
        )
        pd.DataFrame(stationarity_results).T.to_csv(
            f"{self.output_dir}/cointegration/stationarity_results.csv"
        )
        ...
        print(
            f"    Cointegrated pairs: {sum([r['cointegrated'] for r in cointegration_results.values()])}"
        )
    ...
    def _run_markov_switching(self):
            ...
            if spread_col in markov_data.columns:
                try:
                    ...
                    if len(test_data) > 200:
                        ...
                        regime_df.to_csv(
                            f"{self.output_dir}/markov/{venue}_regime_probs.csv", index=False
                        )
        ...
        print(
            f"    Successful fits: {len([r for r in markov_results.values() if 'log_likelihood' in r])}"
        )
    ...
    def _plot_markov_regimes(self, markov_data: pd.DataFrame):
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.output_dir}/markov/markov_regime_plots.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        try:
            ...
            if len(var_data) > 100:
                ...
                fevd_df = pd.DataFrame(
                    ...
                    columns=[
                        f"{col}_explained_by_{col2}" for col in venue_cols for col2 in venue_cols
                    ],
                )
                ...
                variance_shares_df = pd.DataFrame(
                    variance_shares,
                ...
                pd.DataFrame(concentration_measures).T.to_csv(
                    f"{self.output_dir}/svar/concentration_measures.csv"
                )

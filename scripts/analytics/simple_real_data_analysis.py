...
class SimpleRealDataAnalysis:
    ...
    def get_s3_coverage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ S3 coverage analysis complete: {coverage['parquet_files']} parquet files")
            return coverage
    ...
    def analyze_sample_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                'min_timestamp': int(df['ts_exchange'].min()) if 'ts_exchange' in df.columns else None,
                'max_timestamp': int(df['ts_exchange'].max()) if 'ts_exchange' in df.columns else None,
        ...
        try:
            ...
            results = {
                ...
                'aligned_panel_summary': {
                    ...
                    'date_range': {
                        'start': aligned_df.index.min().isoformat() if not aligned_df.empty else None,
                        'end': aligned_df.index.max().isoformat() if not aligned_df.empty else None
                    },
                    'venues': [col for col in aligned_df.columns if col.startswith('mid_')] if not aligned_df.empty else []
                }
            }

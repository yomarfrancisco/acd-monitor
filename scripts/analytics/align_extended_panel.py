...
class ExtendedPanelAligner:
    ...
    def _align_to_1s_grid(self, panel: pd.DataFrame):
        ...
        for venue in self.venues:
            ...
            print(
                f"    ✅ {venue}: {len(venue_aligned)} timestamps, {venue_aligned.isna().sum().sum()} NaN values"
            )
        ...
        for venue in self.venues:
            ...
            if mid_col in aligned_panel.columns:
                ...
                aligned_panel[mid_col] = (
                    aligned_panel[mid_col].iloc[0] * (1 + returns_winsorized).cumprod()
                )

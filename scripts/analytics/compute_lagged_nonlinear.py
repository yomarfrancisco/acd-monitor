...
def save_results(symbol: str, lagged_df: pd.DataFrame, mi_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if len(mi_df) > 0:
        ...
        s3.put_object(
            Bucket=S3_BUCKET,
            Key=mi_key,
            Body=mi_buffer.getvalue()
        )
    else:
        # Create empty DataFrame with correct schema
        empty_mi = pd.DataFrame(columns=['symbol', 'venue1', 'venue2', 'metric', 'value', 'n_aligned'])
        mi_buffer = io.BytesIO()
    ...
    manifest = {
        ...
        "coverage_stats": {
            "venue_pairs": len(lagged_df.groupby(['venue1', 'venue2'])) if len(lagged_df) > 0 else 0,

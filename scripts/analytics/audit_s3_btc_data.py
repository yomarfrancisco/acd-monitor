...
class S3BTCDataAuditor:
    ...
    def _list_btc_snapshots(self):
        ...
        try:
            response = self.s3_client.list_objects_v2(
                Bucket=self.bucket_name, Prefix=self.btc_prefix, Delimiter="/"
            )
            ...
            if "CommonPrefixes" in response:
                for prefix in response["CommonPrefixes"]:
                    ...
                    snapshots.append({"name": snapshot_name, "prefix": prefix["Prefix"]})
    ...
    def _analyze_snapshot(self, snapshot):
        ...
        try:
            ...
            if "T" in snapshot_name:
                timestamp_str = (
                    snapshot_name.split("T")[0]
                    + "T"
                    + snapshot_name.split("T")[1].replace("-", ":")
                )
            ...
            for venue in self.venues:
                ...
                if venue_files:
                    ...
                    analysis["venue_coverage"][venue] = {"files_count": 0, "has_data": False}
    ...
    def _list_venue_files(self, venue_prefix):
        ...
        try:
            response = self.s3_client.list_objects_v2(Bucket=self.bucket_name, Prefix=venue_prefix)
    ...
    def _generate_report(self, snapshot_analysis):
        ...
        venue_counts = {}
        for venue in self.venues:
            venue_counts[venue] = len(
                [s for s in snapshot_analysis if venue in s["venues_available"]]
            )
    ...
    def _generate_recommendations(self, snapshot_analysis, distinct_days):
        ...
        if distinct_days == 0:
            recommendations.append(
                "❌ No real BTC-USD data found - cannot proceed with extended analysis"
            )
        elif distinct_days == 1:
            recommendations.append(
                "⚠️ Only 1 day of data available - insufficient for multi-day analysis"
            )
    ...
    def _generate_empty_report(self):
        ...
        return {
            ...
            "snapshots": [],
            "recommendations": [
                "❌ No BTC-USD data found in S3 - cannot proceed with extended analysis"
            ],
        }

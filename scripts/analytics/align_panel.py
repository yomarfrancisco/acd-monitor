...
class PanelAligner:
    ...
    def _align_to_grid(self):
        ...
        for venue, df in self.venue_data.items():
            ...
            df_filtered = df[
                (df["ts_exchange"] >= min_time) & (df["ts_exchange"] <= max_time)
            ].copy()
...
"""
        ...
        for venue, stats in venue_stats.items():
            report += (
                f"- **{venue}**: {stats['observations']:,} obs ({stats['coverage']:.1%} coverage)\n"
            )
...
"""
        ...
        summary_data = {
            ...
            "panel_stats": {
                ...
                "time_range_hours": (
                    self.aligned_panel.index.max() - self.aligned_panel.index.min()
                ).total_seconds()
                / 3600,
            },

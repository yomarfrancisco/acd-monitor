...
class CoinbaseAnomalyQC:
    ...
    def _check_timestamps(self, df: pd.DataFrame):
        ...
        timestamp_stats = {
            'total_ticks': len(df),
            'time_span_hours': (df['ts_exchange'].max() - df['ts_exchange'].min()).total_seconds() / 3600,
    ...
    def _check_duplicates(self, df: pd.DataFrame):
        ...
        plt.subplot(1, 2, 1)
        duplicate_counts = [duplicate_stats['before_duplicates'], duplicate_stats['after_duplicates']]
        plt.bar(['Before Dedup', 'After Dedup'], duplicate_counts, color=['red', 'green'], alpha=0.7)
        plt.title('Duplicate Records')
        plt.ylabel('Count')
        
        plt.subplot(1, 2, 2)
        duplicate_rates = [duplicate_stats['before_duplicate_rate'], duplicate_stats['after_duplicate_rate']]
        ...
        print(f"    Before: {duplicate_stats['before_duplicates']} duplicates ({duplicate_stats['before_duplicate_rate']:.1%})")
        print(f"    After: {duplicate_stats['after_duplicates']} duplicates ({duplicate_stats['after_duplicate_rate']:.1%})")
        print(f"    Removed: {duplicate_stats['dedup_removed']} records ({duplicate_stats['dedup_rate']:.1%})")
    ...
    def _check_staleness_burstiness(self, df: pd.DataFrame):
        ...
        updates_df = pd.merge(time_grid_df, updates_per_second.reset_index(), on='second', how='left')
        ...
        price_change_stats = {
            ...
            'outlier_count': (df_sorted['price_change'] > df_sorted['price_change'].quantile(0.999)).sum()
        }
        ...
        plt.subplot(2, 3, 3)
        plt.bar(['Zero Updates', 'Non-Zero Updates'], [zero_update_seconds, total_seconds - zero_update_seconds])
    ...
    def _check_schema_conformity(self, df: pd.DataFrame):
        ...
        canonical_cols = [
            'ts_exchange', 'best_bid', 'best_ask', 'last_px', 'last_sz',
            'mid_px', 'spread', 'spread_bps'
        ]
...
"""
    ...
    def _generate_conclusion(self) -> str:
        ...
        if duplicate_rate > 0.1:
            conclusions.append(f"⚠️ High duplicate rate ({duplicate_rate:.1%}) may affect variance calculations")
        
        staleness_rate = self.results.get('staleness', {}).get('staleness_rate', 0)
        if staleness_rate > 0.1:
            conclusions.append(f"⚠️ High staleness rate ({staleness_rate:.1%}) may create artificial patterns")
        
        negative_spread_rate = self.results.get('spread', {}).get('negative_spread_rate', 0)
        if negative_spread_rate > 0.01:
            conclusions.append(f"⚠️ Negative spreads ({negative_spread_rate:.1%}) indicate data quality issues")
        
        if not conclusions:
            conclusions.append("✅ No major data quality issues detected")
            conclusions.append("🔍 Coinbase anomaly may be genuine - requires deeper econometric analysis")
        else:
            conclusions.append("🔧 Data quality issues detected - may explain variance ratio anomaly")

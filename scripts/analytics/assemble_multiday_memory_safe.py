...
class MemorySafeMultiDayAssembler:
    ...
    def assemble_multiday_panel(self):
        ...
        print("=" * 60)
        print(f"📊 Memory limits: {self.MAX_ROWS_IN_MEMORY:,} rows, {self.MAX_FILE_SIZE_GB}GB per file")
    ...
    def _process_single_date_safe(self, date: str) -> bool:
        ...
        for venue, venue_file_list in venue_files.items():
            ...
            if venue_df is not None and len(venue_df) > 0:
                ...
                if total_rows > self.MAX_ROWS_IN_MEMORY:
                    print(f"        ⚠️ Memory limit exceeded: {total_rows:,} > {self.MAX_ROWS_IN_MEMORY:,}")
                    return False
            else:
        ...
        if len(aligned_panel) > self.MAX_ROWS_IN_MEMORY:
            print(f"        ⚠️ Final panel too large: {len(aligned_panel):,} > {self.MAX_ROWS_IN_MEMORY:,}")
            return False
        ...
        for file_info in venue_files:
            try:
                ...
                if file_size_gb > self.MAX_FILE_SIZE_GB:
                    print(f"        ⚠️ File too large: {file_size_gb:.2f}GB > {self.MAX_FILE_SIZE_GB}GB")
    ...
    def _apply_quality_controls(self, panel: pd.DataFrame) -> pd.DataFrame:
        ...
        for venue in self.venues:
            ...
            if mid_col in panel.columns:
                ...
                returns_winsorized = returns.clip(lower=returns.quantile(0.01), upper=returns.quantile(0.99))
    ...
    def _log_date_summary(self, date: str, panel: pd.DataFrame):
        ...
        with open(summary_file, 'a') as f:
            ...
            f.write(f"- **Duration**: {(panel.index.max() - panel.index.min()).total_seconds() / 3600:.1f} hours\n")
    ...
    def _generate_assembly_summary(self, processed_dates: List[str]):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"**Memory Limits**: {self.MAX_ROWS_IN_MEMORY:,} rows, {self.MAX_FILE_SIZE_GB}GB per file\n")

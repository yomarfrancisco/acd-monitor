...
class Wave3ClusteringExecutor:
    ...
    def _evaluate_cluster_numbers(self, features_data: pd.DataFrame) -> Dict:
        ...
        print(f"  📊 Best k: {best_k} (Silhouette: {cluster_results[best_k]['silhouette_score']:.4f})")
    ...
    def _generate_clustering_outputs(self, cluster_results: Dict, final_clustering: Dict, cluster_profiles: Dict):
        ...
        for cluster_id, profile in cluster_profiles.items():
            profiles_data.append({
                ...
                'mean_variance': np.mean(list(profile['mean_features'].values())) if profile['mean_features'] else 0,
                'feature_dispersion': np.std(list(profile['mean_features'].values())) if profile['mean_features'] else 0
            })
    ...
    def _generate_clustering_summary(self, cluster_results: Dict, final_clustering: Dict, cluster_profiles: Dict):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write("**Hypothesis**: Competitive markets show environment-aligned clusters; coordination shows suppressed variance clusters\n\n")
            ...
            for k, results in cluster_results.items():
                f.write(f"| {k} | {results['silhouette_score']:.4f} | {results['calinski_harabasz_score']:.2f} | {results['inertia']:.2f} |\n")
            ...
            f.write(f"- **Calinski-Harabasz score**: {final_clustering['calinski_harabasz_score']:.2f}\n")
            f.write(f"- **Inertia**: {final_clustering['inertia']:.2f}\n\n")
            
            # Cluster profiles
            f.write("## Cluster Profiles\n\n")
            f.write("| Cluster | Windows | Size % | Coordination Score | Mean Variance | Feature Dispersion |\n")
            f.write("|---------|---------|--------|-------------------|---------------|-------------------|\n")
            ...
            for cluster_id, profile in cluster_profiles.items():
                ...
                f.write(f"| {cluster_id} | {profile['n_windows']} | {profile['size_pct']:.1f}% | {profile['coordination_score']:.4f} | {mean_variance:.4f} | {feature_dispersion:.4f} |\n")
            ...
            if coordination_clusters:
                ...
                f.write("The following clusters show characteristics consistent with coordination:\n\n")
                
                for cluster_id in coordination_clusters:
                    profile = cluster_profiles[cluster_id]
                    f.write(f"- **Cluster {cluster_id}**: {profile['n_windows']} windows ({profile['size_pct']:.1f}% of data)\n")
                ...
                f.write("**Interpretation**: These clusters show suppressed variance and low dispersion across venues, which is consistent with coordinated market making rather than competitive dynamics.\n\n")
            else:

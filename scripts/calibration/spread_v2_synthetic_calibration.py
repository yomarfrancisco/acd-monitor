...
def run_spread_v2_detector(snapshot_path: str, 
                          output_dir: Path,
                          bootstrap_n: int = 300,
...
def process_synthetic_datasets(synthetic_datasets: Dict[str, str],
                              expected_thresholds: Dict[str, Any],
                              output_dir: Path,
                              bootstrap_n: int = 300) -> List[Dict[str, Any]]:
    """
    ...
    for dataset_name, dataset_path in synthetic_datasets.items():
        ...
        delta_z_match = (
            expected_min <= observed_delta_z[0] <= expected_max or
            expected_min <= observed_delta_z[1] <= expected_max
        )
        ...
        logger.info(f"Dataset {dataset_name}: detection_match={detection_match}, delta_z_match={delta_z_match}")
...
def generate_summary_report(results: List[Dict[str, Any]], output_dir: Path) -> None:
    ...
    summary_data = {
        ...
        'successful_runs': len([r for r in results if r['detector_results']['status'] == 'success']),
...
def main():
    ...
    parser.add_argument("--synthetic-datasets", default="analysis/BTC/calibration/spread_v2/synthetic_datasets.json",
                       help="Path to synthetic datasets JSON file")
    parser.add_argument("--expected-thresholds", default="analysis/BTC/calibration/spread_v2/expected_thresholds.json",
                       help="Path to expected thresholds JSON file")
    parser.add_argument("--output-dir", default="calibration/spread/synthetic_runs",
    ...
    print(f"Successful runs: {len([r for r in results if r['detector_results']['status'] == 'success'])}")

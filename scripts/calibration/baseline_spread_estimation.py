...
def estimate_environment_baseline(data: pd.DataFrame, 
                                 environment: str,
                                 environment_value: str) -> Dict[str, Any]:
    """
    ...
    stats = {
        ...
        'mad': float(np.median(np.abs(z_scores_clean - z_scores_clean.median()))),  # Median Absolute Deviation
...
def main():
    ...
    parser.add_argument("--output-dir", default="calibration/spread",
    ...
    for env_type, env_stats in baseline_stats.items():
        ...
        for env_value, stats in env_stats.items():
            if stats.get('status') == 'sufficient_data':
                print(f"  {env_value}: N={stats['n_points']}, median={stats['median']:.3f}, mad={stats['mad']:.3f}")
            else:

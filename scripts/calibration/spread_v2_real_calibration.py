...
def run_spread_v2_detector_real(snapshot_path: str, 
                               output_dir: Path,
                               bootstrap_n: int = 300,
...
def process_real_snapshots(snapshot_paths: List[str],
                          output_dir: Path,
...
def generate_real_calibration_report(results: List[Dict[str, Any]], output_dir: Path) -> None:
    ...
    summary_data = {
        ...
        'successful_runs': len([r for r in results if r.get('detector_results', {}).get('status') == 'success']),
        'failed_runs': len([r for r in results if r.get('detector_results', {}).get('status') == 'error']),
...
"""
    ...
    for result in results:
        ...
        report_content += f"- **P-Value**: {result.get('detector_results', {}).get('p_value', 1.0)}\n"
        report_content += "\n"
...
def main():
    ...
    parser.add_argument("--snapshots", nargs='+', 
                       default=["snapshots/btc_1200_1230", "snapshots/btc_1230_1300", 
                               "snapshots/btc_1300_1330", "snapshots/btc_1330_1400"],
                       help="List of snapshot directory paths")
    parser.add_argument("--output-dir", default="calibration/spread/real_runs",
    ...
    print(f"Successful runs: {len([r for r in results if r.get('detector_results', {}).get('status') == 'success'])}")
    print(f"Failed runs: {len([r for r in results if r.get('detector_results', {}).get('status') == 'error'])}")

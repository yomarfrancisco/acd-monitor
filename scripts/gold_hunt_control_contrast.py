...
def calculate_episode_metrics(episode_data: pd.DataFrame) -> Dict:
    ...
    metrics['volume_change'] = (metrics['episode_volume'] - metrics['pre_volume']) / metrics['pre_volume'] * 100
    ...
    metrics['volatility_change'] = (metrics['episode_volatility'] - metrics['pre_volatility']) / metrics['pre_volatility'] * 100
    ...
    metrics['spread_change'] = (metrics['episode_spread'] - metrics['pre_spread']) / metrics['pre_spread'] * 100
...
def calculate_control_metrics(control_data: pd.DataFrame) -> Dict:
    ...
    metrics['pre_volume'] = np.mean(pre_volumes)
    metrics['post_volume'] = np.mean(post_volumes)
    metrics['volume_change'] = (metrics['control_volume'] - metrics['pre_volume']) / metrics['pre_volume'] * 100
    ...
    metrics['pre_volatility'] = np.mean(pre_volatilities)
    metrics['post_volatility'] = np.mean(post_volatilities)
    metrics['volatility_change'] = (metrics['control_volatility'] - metrics['pre_volatility']) / metrics['pre_volatility'] * 100
    
    # Spread metrics
    metrics['control_spread'] = control_period['cross_venue_spread'].mean()
    metrics['pre_spread'] = pre_period['cross_venue_spread'].mean()
    metrics['post_spread'] = post_period['cross_venue_spread'].mean()
    metrics['spread_change'] = (metrics['control_spread'] - metrics['pre_spread']) / metrics['pre_spread'] * 100
...
def run_statistical_contrasts(episode_metrics: List[Dict], control_metrics: List[Dict]) -> Dict:
    ...
    contrasts['volume'] = {
        ...
        'difference_pct': ((np.mean(episode_volumes) - np.mean(control_volumes)) / np.mean(control_volumes)) * 100,
    ...
    contrasts['volatility'] = {
        ...
        'difference_pct': ((np.mean(episode_volatilities) - np.mean(control_volatilities)) / np.mean(control_volatilities)) * 100,
    ...
    contrasts['spread'] = {
        ...
        'difference_pct': ((np.mean(episode_spreads) - np.mean(control_spreads)) / np.mean(control_spreads)) * 100,
...
def generate_control_contrast_report(contrasts: Dict, episode_count: int, control_count: int) -> str:
    ...
    report.append(f"- **Statistical Significance**: {'✅ YES' if vol['significant'] else '❌ NO'} (p={vol['p_value']:.3f})")
    report.append("")
    ...
    report.append(f"- **Statistical Significance**: {'✅ YES' if vol_vol['significant'] else '❌ NO'} (p={vol_vol['p_value']:.3f})")
    report.append("")
    ...
    report.append(f"- **Statistical Significance**: {'✅ YES' if spread['significant'] else '❌ NO'} (p={spread['p_value']:.3f})")
    report.append("")
    ...
    if significant_metrics >= 2:
        report.append("✅ **STRONG VALIDATION**: Episodes show statistically significant differences from control periods")
        report.append("✅ **COORDINATION CONFIRMED**: Volume/volatility patterns are abnormal, not background noise")
        ...
        report.append("❌ **WEAK VALIDATION**: Episodes show no significant differences from controls")
...
def main():
    ...
    parser.add_argument('--output-dir', default='experiments/gold_hunt_v1/control_contrast',

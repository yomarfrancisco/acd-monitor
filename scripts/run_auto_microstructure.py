...
def load_overlap_data(export_dir: str) -> dict:
    """Load overlap data from exports/overlap.json."""
    overlap_file = Path(export_dir) / "overlap.json"
    
    if not overlap_file.exists():
        logger.error(f"[ABORT:overlap_missing] overlap.json not found at {overlap_file}")
        print(f'[ABORT:overlap_missing] {{"file":"{overlap_file}","reason":"overlap.json required for analysis"}}')
        sys.exit(2)
    ...
    try:
        ...
        for field in required_fields:
            if field not in overlap_data:
                ...
                print(f'[ABORT:overlap_missing] {{"field":"{field}","reason":"required field missing"}}')
                sys.exit(2)
        ...
        if overlap_data['policy'].startswith('SYNTHETIC'):
            ...
            print(f'[ABORT:synthetic] {{"policy":"{overlap_data[\"policy\"]}","reason":"synthetic data not allowed"}}')
            sys.exit(2)
...
def run_spread_compression(start: str, end: str, venues: list, export_dir: str, verbose: bool) -> bool:
    ...
    try:
        ...
        for log_line in required_logs:
            if log_line not in output:
                ...
                print(f'[ABORT:spread_missing] {{"log":"{log_line}","reason":"required log line missing"}}')
                return False
...
def run_info_share(start: str, end: str, venues: list, export_dir: str, verbose: bool) -> bool:
    ...
    try:
        ...
        required_logs = [
            '[MICRO:infoShare:env]',
            '[INFO:infoShare:assignments]'
        ]
        
        for log_line in required_logs:
            if log_line not in output:
                logger.error(f"[ABORT:infoshare_missing] Required log line '{log_line}' not found")
                print(f'[ABORT:infoshare_missing] {{"log":"{log_line}","reason":"required log line missing"}}')
                return False
        ...
        for forbidden in forbidden_logs:
            if forbidden in output.lower():
                ...
                print(f'[ABORT:synthetic] {{"mode":"{forbidden}","reason":"synthetic/oracle modes not permitted"}}')
                return False
...
def run_invariance_matrix(start: str, end: str, venues: list, export_dir: str, verbose: bool) -> bool:
    ...
    cmd = [
        'python', 'scripts/run_invariance_matrix_analysis.py',
        '--start', start,
        '--end', end,
        '--venues', venues_str,
        '--bootstrap', '1000',
    ...
    try:
        ...
        for log_line in required_logs:
            if log_line not in output:
                ...
                print(f'[ABORT:invariance_missing] {{"log":"{log_line}","reason":"required log line missing"}}')
                return False
...
def main():
    ...
    print(f'[SEED] numpy.random.seed(42) for auto microstructure analysis')
    
    # Load overlap data
    overlap_data = load_overlap_data(args.export_dir)
    ...
    if not run_spread_compression(
        overlap_data['startUTC'], overlap_data['endUTC'], 
        overlap_data['venues'], args.export_dir, args.verbose
    ):
        success = False

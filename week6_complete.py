#!/usr/bin/env python3
"""
Week-6 Complete Execution (2025-09-05 → 2025-09-11)
"""
...
def main():
    print('🔍 Week-6 Complete Execution (2025-09-05 → 2025-09-11)')
    print('=' * 60)
    ...
    print(f'📊 **Week-6 Scope:**')
    print(f'📊 Dates: {week6_dates}')
    print(f'📊 Venues: {venues}')
    print(f'📊 Total candidates: {len(week6_dates) * len(venues)}')
    ...
    if resampled_data:
        ...
        if os.path.exists(panel_path):
            ...
            # Remove any existing Week-6 data (2025-09-05 to 2025-09-11)
            week6_start = pd.Timestamp('2025-09-05 00:00:00')
            week6_end = pd.Timestamp('2025-09-11 23:59:59')
            
            # Filter out Week-6 data
            existing_panel = existing_panel[~((existing_panel.index >= week6_start) & (existing_panel.index <= week6_end))]
            print(f'📊 Panel after Week-6 removal: {existing_panel.shape}')
        ...
        if os.path.exists(manifest_path):
            ...
            manifest = {
                ...
                'description': 'Week-6 rebuild from CoinAPI Flat Files',
                'date_range': '2025-09-05 to 2025-09-11',
                'venues': ['BINANCE', 'COINBASE', 'BYBITSPOT', 'BITGET'],
        ...
        # Update manifest
        manifest['week6_files'] = [r['filename'] for r in download_results if r['status'] == 'SUCCESS']
    ...
    if os.path.exists(panel_path):
        ...
        # Check for any data beyond 2025-09-11 (should be frozen)
        frozen_threshold = pd.Timestamp('2025-09-12 00:00:00')
        frozen_data = final_panel[final_panel.index >= frozen_threshold]
        if len(frozen_data) > 0:
            print(f'⚠️ WARNING: Found {len(frozen_data)} rows beyond frozen threshold (2025-09-12)')
        else:
            print(f'✅ No data found beyond frozen threshold (2025-09-12)')
    
    print(f'\n✅ Week-6 execution completed')

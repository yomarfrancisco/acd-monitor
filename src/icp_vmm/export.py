...
"""
            ...
            if "parameter_tests" in icp_tests:
                ...
                for param, test_result in icp_tests["parameter_tests"].items():
                    if "significant" in test_result:
                        ...
                        summary += (
                            f"- {param}: {status} (p={test_result.get('p_value', 'N/A'):.3f})\n"
                        )
                summary += "\n"
        ...
        summary += "---\n"
        summary += (
            f"*Generated by ICP-VMM Engine v0.1.0 at {datetime.now(timezone.utc).isoformat()}*\n"
        )
    ...
    def _format_statistical_significance(self, icp_tests: Dict) -> str:
        ...
        for param, test_result in icp_tests["parameter_tests"].items():
            if "p_value" in test_result:
                ...
                content += (
                    f"({'significant' if test_result.get('significant') else 'not significant'})\n"
                )

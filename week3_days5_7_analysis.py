#!/usr/bin/env python3
"""
Week-3 Real-Beacon Analysis - Days 5-7 (2025-08-15 to 2025-08-17)
Complete pipeline with consolidated reporting
"""
...
def process_single_day(date_str, date_display, venues):
    """Process a single day through all steps"""
    ...
    print(f"\nDAY {date_display} COMPLETE — continuing")
...
def main():
    print("🧾 Week-3 Real-Beacon Analysis - Days 5-7 (2025-08-15 to 2025-08-17)")
    print("=" * 70)
    ...
    for date_str, date_display in days:
        result = process_single_day(date_str, date_display, venues)
        if result is None:
            print(f"❌ Failed to process {date_display} - stopping")
            return
        results.append(result)
    
    # Three-Day Consolidated Summary
    print(f"\n📊 THREE-DAY CONSOLIDATED SUMMARY (Days 5-7)")
    print("=" * 70)
    
    print(f"{'Day':<12} {'med Δdisp3':<12} {'med Δlag':<12} {'leadership':<12} {'dominant':<12} {'typology':<20} {'conv/div':<15}")
    print("-" * 95)
    ...
    for result in results:
        ...
        # Dominant leader
        leader_counts = leadership['leader_counts']
        dominant_leader = max(leader_counts.keys(), key=lambda x: leader_counts[x]) if leader_counts else 'NONE'
        
        # Typology counts
        typology_counts = effects['typology_counts']
        s10_count = typology_counts.get('Signal-10', 0)
        s7_count = typology_counts.get('Signal-7', 0)
        comp_count = typology_counts.get('Compression', 0)
        typology_str = f"{s10_count}/{s7_count}/{comp_count}"
        
        # Leadership success rate
        leadership_success = (leadership['successful_analyses'] / leadership['total_events']) * 100
        
        # Convergence/divergence
        outcome_counts = leadership['outcome_counts']
        convergence_count = outcome_counts.get('CONVERGENCE', 0)
        divergence_count = outcome_counts.get('DIVERGENCE', 0)
        total_events = leadership['total_events']
        convergence_pct = (convergence_count / total_events) * 100 if total_events > 0 else 0
        divergence_pct = (divergence_count / total_events) * 100 if total_events > 0 else 0
        conv_div_str = f"{convergence_pct:.1f}%/{divergence_pct:.1f}%"
        
        print(f"{result['date']:<12} {effects['median_delta_disp_3m']:.2f} bps{'':<6} {effects['median_delta_lag_ms']:.2f} ms{'':<6} {leadership_success:.1f}%{'':<7} {dominant_leader:<12} {typology_str:<20} {conv_div_str:<15}")
    
    # Week-3 Overall Pattern
    print(f"\n📝 WEEK-3 OVERALL PATTERN")
    print("=" * 70)
    
    # Analyze leadership rotation
    leadership_pattern = []
    for result in results:
        leader_counts = result['leadership']['leader_counts']
        dominant_leader = max(leader_counts.keys(), key=lambda x: leader_counts[x]) if leader_counts else 'NONE'
        leadership_pattern.append(dominant_leader)
    
    print(f"Leadership Rotation: {' → '.join(leadership_pattern)}")
    
    # Analyze dispersion trends
    dispersion_trends = [result['effects']['median_delta_disp_3m'] for result in results]
    compression_days = sum(1 for d in dispersion_trends if d < 0)
    expansion_days = sum(1 for d in dispersion_trends if d > 0)
    
    print(f"Dispersion Trends: {compression_days} compression days, {expansion_days} expansion days")
    
    # Overall processing success
    total_beacons = sum(result['effects']['total_beacons'] for result in results)
    total_processed = sum(result['effects']['processed_ok'] for result in results)
    overall_success_rate = (total_processed / total_beacons) * 100
    
    print(f"Overall Processing Success: {overall_success_rate:.1f}% ({total_processed}/{total_beacons} beacons)")
    
    # Memory usage
    print(f"\nMemory usage: {get_memory_usage():.1f} MB")
    print("✅ Week-3 Days 5-7 analysis completed successfully")

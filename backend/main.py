...
This evidence package contains comprehensive analysis results from the "
                "Algorithmic Cartel Detection (ACD) system, implementing the "
                "methodological framework outlined in Brief 55+."

## Methodology Overview

### Brief 55+ Framework
- **Information Cascade Vulnerability Model (ICVM)**: Analyzes how "
                "information flows between market participants"
- **Vulnerability Mapping Model (VMM)**: Identifies structural "
...
{datetime.now().isoformat()}"""
            ...
            for event in events_data.items:
                risk_level = (
                    "LOW" if event.riskScore < 30 else "AMBER" if event.riskScore < 70 else "HIGH"
                )
                csv_content += f"{event.ts},{event.type},{event.severity},{risk_level}\n"

...
Ultra memory-efficient conversion with explicit memory management
"""
...
def process_raw_file_ultra_efficient(file_path, venue, date):
    """Process raw file with ultra-efficient memory management"""
    try:
        ...
        # Process file in very small chunks and write incrementally
        ...
        # Validate timestamps are within the day (with some tolerance for microseconds)
        day_start = pd.Timestamp(f"{date[:4]}-{date[4:6]}-{date[6:8]} 00:00:00", tz='UTC')
        day_end = pd.Timestamp(f"{date[:4]}-{date[4:6]}-{date[6:8]} 23:59:59.999999", tz='UTC')
        
        ts_min = df_canonical['ts'].min()
        ts_max = df_canonical['ts'].max()
        
        print(f"    Timestamp range: {ts_min} to {ts_max}")
        print(f"    Expected range: {day_start} to {day_end}")
        
        # Allow some tolerance for microseconds
        if ts_min < day_start or ts_max > day_end:
            return None, f"HALT: Timestamps out of day range (min: {ts_min}, max: {ts_max})"
        ...
        # Clear final dataframe from memory
        del df_canonical
        gc.collect()
        ...
        return {
            ...
            'canon_rows': len(pd.read_parquet(canonical_path)),  # Read back to get final count
            'ts_start': pd.read_parquet(canonical_path)['ts'].min(),
            'ts_end': pd.read_parquet(canonical_path)['ts'].max(),
            'price_min': pd.read_parquet(canonical_path)['price'].min(),
            'price_max': pd.read_parquet(canonical_path)['price'].max(),
            'action': 'WROTE',
            'memory_mb': memory_mb
        }, None
...
def process_venue_date(venue, date):
    ...
    # Process the file
    result, error = process_raw_file_ultra_efficient(raw_file, venue, date)
    
    # Force garbage collection after each venue
    gc.collect()
...
def main():
    ...
    print("🚀 Starting Week 2 Canonicalization - Days 1-2 (Ultra Memory Efficient)")
    ...
    for date in dates:
        ...
        for venue in venues:
            ...
            # Force garbage collection between venues
            gc.collect()

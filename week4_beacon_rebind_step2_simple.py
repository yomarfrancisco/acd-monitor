#!/usr/bin/env python3
"""
Week-4 Beacon Rebind → Smoke Test → Full Step-2 (Simple)
...
def main():
    print("🔍 Week-4 Beacon Rebind → Smoke Test → Full Step-2 (Simple)")
    print("=" * 70)
    ...
    for _, event in smoke_events.iterrows():
        ...
        try:
            ...
            # Filter to window
            window_df = df.loc[df['ts'].between(t0, t1, inclusive='both')]
            
            n_rows = len(window_df)
            if n_rows > 0:
                ts_min = window_df['ts'].min()
                ts_max = window_df['ts'].max()
                print(f"  {venue} {date_str} {event_ts} rows={n_rows} ts_min={ts_min} ts_max={ts_max}")
                
                # Build 1-sec VWAP bars and check coverage
                vwap_bars = create_vwap_bars_streaming(window_df, t0, t1)
                ...
                window_df_wide = df.loc[df['ts'].between(t0_wide, t1_wide, inclusive='both')]
                
                n_rows_wide = len(window_df_wide)
                if n_rows_wide == 0:
                    # Get global min/max
                    global_min = df['ts'].min()

...
Crypto markets remain fragmented and fast-moving. The question persists:
👉 Do venues coordinate (collusion) or adapt competitively?

⸻

## 2. Solution

The ACD pipeline now runs through Phase 3 statistical validation:
- **Detectors**: Lead-Lag v2, InfoShare v2, Spread v2 (pending calibration), Leadership Rotation.
- **Evidence Outputs**: JSON, CSV, Markdown.
- **Validation**: Extended capture, synthetic controls, statistical rigor (bootstrap, power analysis, reproducibility).
- **Court-Mode Orchestrator**: Active, but results gated until governance.

⸻

## 3. What's Required

(No change — remains as is: data, environments, leadership metrics, invariance tests, logging.)

⸻

## 4. What's Missing
- **Policy/Regulatory Event Environments**: Not yet integrated.
- **Spread v2 Validation**: Requires synthetic positive controls to confirm sensitivity.
- **Tick-Level Validation**: Still required for exchange-facing credibility.
- **Interpretation Layer**: Pending LLM integration (see Agent Plan).

⸻

## 5. Phase Status

✅ **Phase 1 – Extended Capture + Synthetic Controls**
- 16 BTC windows analyzed; 4 high-quality.
- Synthetic injections (lead-lag, sync, dominance) validated detectors.

✅ **Phase 2 – Detector Suite Completion**
- Spread v2, Leadership Rotation, Lead-Lag v2, InfoShare v2 all implemented.
- Sensitivity validation passed (synthetic signals).

✅ **Phase 3 – Statistical Rigor & Calibration**
- **Power Analysis**:
  - Lead-Lag v2: 75% power, MDE=0.124 → Ready.
  - InfoShare v2: 98% power, dominance + sync stable → Ready.
  - Spread v2: 0% power at threshold → Needs synthetic calibration.
- **Bootstrap**: 1000 iterations with FDR correction.
- **Confidence Intervals**: 95% CI generated for all detector outputs.
- **Reproducibility**: SHA256 hashes + rerun validation logs.
- **Artifacts**:
  - BTC_PHASE3_VALIDATION.md
  - power_analysis.json
...
⏳ **Phase 4 – Policy/Regulatory Events**
- ETF approvals, lawsuits, SEC/FCA/FSCA rulings, outages.
- Event-window invariance analysis.

⏳ **Interpretation Layer**
- LLM-generated phrasing for economists, regulators, courts.

⏳ **Robustness Guardrails**
- Tick-level and order-book validation.
- ≥30-day sample per regime.
- Tie/outlier handling.

⸻

## 7. Governance & Guardrails
- **Court-Mode Orchestrator**: Active with strict 1s gap policy.
- **Baseline**: 2s pinned, 1s strict pass achieved with remedies.
- **Instruction**: Do not promote baselines or make coordination claims without explicit governance decision.

⸻

## 8. Next Steps (Short-Term Q4 2025)
1. Finalize Spread v2 calibration via synthetic injections.
2. Add policy/regulatory event environments.
3. Continue ETH-USD integration.
4. Stabilize court-mode orchestrator monitoring.

⸻

✅ This draft reflects Phases 1–3 as completed and highlights the calibration gap (Spread v2), pending policy/regulatory environments, and LLM interpretation layer.

⸻

## 9. Data Sufficiency Note
...
**6.A Detection**: market-structure, volume profile, order-flow/ICT flags → episode list.

**6.B Validation**: sessions, policy/regulator events, infra shocks, global liquidity regimes → invariance/ICP + court bundles.
...
## 7.a Environment Classification (Addendum — Detection vs Validation)
...
**Implementation status (pointer to code/artifacts)**
...
**Reference**: See `docs/ENV_TAXONOMY.md` for single source of truth.

⸻

## 8. Initial Environment Definitions for BTC-USD

Volatility/liquidity/funding regimes are used in the Detection layer for triage; exogenous session/policy/infra/liquidity-regime bins are used in the Validation layer for ICP.
...
### S3 Data Capture Safety Protocol
**IMPORTANT**: Never stop S3 data capture without explicit verification. If prompted to stop S3 processes:
1. **STOP** and ask for clarification
2. **VERIFY** the request is intentional
3. **CONFIRM** the reason for stopping
4. **DOCUMENT** any stop requests and reasons

**Current Status**: S3 data capture is ACTIVE and should remain so unless explicitly authorized to stop.

### Data Capture Status
- **BTC-USD**: 16 windows captured (07:30-17:54 UTC, 2025-09-29)
- **ETH-USD**: 16 windows captured (07:30-17:54 UTC, 2025-09-29)
- **Coverage**: All windows ≥95% coverage, all 5 venues present
- **Quality**: High-quality tick-level data with full provenance

#!/usr/bin/env python3
"""
Step 2 Real Beacons Re-run - Week -4
Fix synthetic beacon contamination and use real Step-1 beacon detection
"""
...
def process_event_real(event, venue, date_str, all_venues_data):
    """Process a single beacon event with real beacon data"""
    try:
...
def main():
    print("🔍 Step 2 Real Beacons Re-run - Week -4")
    print("=" * 60)
    print("Mode: Read-only, no synthetic beacons")
    ...
    # 1️⃣ Clear any in-memory DataFrames
    print("1️⃣ Clearing in-memory DataFrames...")
    # No specific DataFrames to clear in this script
    
    # 2️⃣ Reload true Step-1 beacon cache from disk (regenerate real beacons)
    print("2️⃣ Regenerating real Step-1 beacon cache...")
    ...
    # 3️⃣ Confirm ts dtype and verify total count
    print("\n3️⃣ Verifying beacon cache...")
    ...
    # Print first 5 rows
    print("First 5 beacon events:")
    print("date       | venue     | event_ts")
    print("-" * 40)
    count = 0
    for date_str in sorted(beacons.keys()):
        for venue in venues:
            for beacon in beacons[date_str][venue][:2]:  # Take first 2 from each venue
                if count < 5:
                    print(f"{date_str} | {venue:>9} | {beacon['t_event']}")
                    count += 1
                else:
                    break
            if count >= 5:
    ...
    # 4️⃣ Rerun Step-2 using verified cache
    print("\n4️⃣ Rerunning Step-2 with real beacons...")
    ...
    while current_date <= end_date:
        ...
        for venue in venues:
            ...
            for event in venue_beacons:
                result = process_event_real(event, venue, date_str, all_venues_data)
    ...
    # 5️⃣ Check for 0 signals and 0 leaders
    print("\n5️⃣ Checking for 0 signals and 0 leaders...")

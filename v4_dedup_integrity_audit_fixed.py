#!/usr/bin/env python3
"""
v4 Dedup Integrity Audit (Weeks 1-7, read-only) - FIXED VERSION
...
def check_cross_pair_leakage(venue, date):
    ...
    FIXED: No longer compares same file to itself
    """
    date_dir = f"analysis/flatfiles_1s_v4/partitioned/dt={date}"
    venue_path = f"{date_dir}/venue={venue}/part-0.parquet"
    ...
    try:
        ...
        if "symbol" in df.columns:
            ...
            if len(btcusd_data) > 0 and len(btcusdt_data) > 0:
                # Calculate overlap using timestamps
                if "t1s" in df.columns:
                    btcusd_times = set(btcusd_data["t1s"].dt.floor("S"))
                    btcusdt_times = set(btcusdt_data["t1s"].dt.floor("S"))
                    
                    overlap_seconds = len(btcusd_times.intersection(btcusdt_times))
                    total_seconds = len(btcusd_times.union(btcusdt_times))
                    
                    overlap_pct = (overlap_seconds / total_seconds) * 100 if total_seconds > 0 else 0
                else:
                    overlap_pct = 0.0
            else:
                # No overlap if one or both pairs are missing
                overlap_pct = 0.0
        else:
            # No symbol column - assume no cross-pair leakage
            overlap_pct = 0.0
...
def run_dedup_audit():
    ...
    """
    print("🔍 **v4 Dedup Integrity Audit (Weeks 1-7) - FIXED**")
    print("=" * 50)
    ...
    with open("analysis/flatfiles_ticks_v4/clean/_audit/dedup_audit_week1_7_v1.md", "w") as f:
        f.write("# v4 Dedup Integrity Audit (Weeks 1-7) - FIXED\n\n")
...
def main():
    ...
    print("🚦 **v4 Dedup Integrity Audit - FIXED**")
    print("=" * 70)
